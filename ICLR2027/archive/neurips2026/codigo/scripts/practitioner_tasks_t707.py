...
def s4_params(X_train, target_p95=0.70710678):
    mu = X_train.mean(0)
...
def task_E(X_tr, y_tr, X_te, y_te, ds, n_episodes=1000, n_way=5, k_shot=5, q_query=15, seed=42):
    ...
    scale = 2 * np.arctanh(0.70710678) / max(p95, 1e-7)
...
def main():
    ...
    for model in ALL_MODELS:
        for ds in ALL_DATASETS:
            ...
            row = {**base,
                   ...
                   "time_s":time.time()-t0}
            rows.append(row)
            pd.DataFrame(rows).to_csv(OUT/"tasks_t707.csv", index=False)
    ...
    df.to_csv(OUT/"tasks_t707.csv", index=False)
    print(f"\nSaved: {OUT/'tasks_t707.csv'}")
