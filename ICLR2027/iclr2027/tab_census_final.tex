...
\bottomrule
\end{tabular}
\caption{\textbf{Structure beyond the second moments is the rule at the class level.} Per backbone: raw ImageNet reading $\hat\delta_{99.9}$, excess over the Haar null mean, rank $r$/200 with left-tail $p$ ($^{\circ}$: not genuine at BH $p\le0.05$ over 72 cells; bold: above the null), the CIFAR-100 and CIFAR-10 excess, and the Spearman correlation $\rho_{\text{WN}}$ with WordNet distances. % expR75_census_centered_haar.csv, exp3_alignment.csv
}
\label{tab:census}
\end{table}
