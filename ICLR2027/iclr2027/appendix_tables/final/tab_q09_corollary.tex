...
\par\vspace{2pt}
\endgroup
\caption{\textbf{The zero-cost gains are modest and limited to prototype tasks.} Per cell: nearest-centroid and few-shot accuracy under the Euclidean and the Poincar\'{e} readout, the advantage of each zero-cost metric, and a paired test.
}
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{The raw reading predicts the gain within datasets.} Per dataset and task: the correlation between the raw supremum reading and the metric gain, with bootstrap intervals and the family-demeaned value.
}
\label{tab:q9-corollary-b}
\end{table}
\begin{table}[H]
\centering
\scriptsize
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{Cosine collects the gain for the self-supervised models.} Per backbone: the best zero-cost advantage over the Euclidean readout, averaged over the hierarchical datasets, and which metric collects it.
}
\label{tab:q9-corollary-c}
\end{table}
\begin{table}[H]
\centering
\scriptsize
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{The calibrated reading predicts the gain as well as the raw one, and the hierarchy verdict predicts none.} Per dataset and task: the same correlations for the raw statistic, the excess and the hierarchy-test $z$.
}
