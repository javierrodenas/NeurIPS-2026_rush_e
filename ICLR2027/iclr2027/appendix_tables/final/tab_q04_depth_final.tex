...
\toprule
 & \multicolumn{2}{c}{C100, $K{=}20$} & \multicolumn{4}{c}{ImageNet, matched star} & bal.\ frame \\
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{Clusters are oriented toward their hubs in 4 of 12 backbones under the star with Gaussian hubs.} Per backbone: the depth statistic and its $z$ on CIFAR-100 and on ImageNet, the WordNet cut at 3 sizes, and the balanced frame.
}
\label{tab:q4-depth}
\end{table}
\begin{table}[H]
...
\toprule
model & star $e_{\mathrm{hub}}$ & depth & $z$ & $r_{\text{star}}$ & decoupled $z$ & cert. \\
\midrule
...
flat control, balanced frame & -- & -- & $-0.46$ & 0/5 & $-0.65$ & 0/50 \\
flat control, frame mismatch & -- & -- & $-3.13$ & 5/5 & $-2.33$ & 38/50 \\
\bottomrule
\end{tabular}
\par\vspace{2pt}
\endgroup
\caption{\textbf{The same 4 backbones are certified under the star whose hubs are a Haar resample of the real ones.} Per backbone: the star's excess, the depth $z$, the star spread and the decoupling control over 10 seeds.
}
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{Supervision on leaf labels can produce the alignment but does not guarantee it.} The 2 ViT-B/16 trained on ImageNet-1k leaf labels, through the census protocol: excess, depth $z$ and agreement with WordNet.
}
...
\end{tabular}
\par\vspace{2pt}
\caption{\textbf{Training in hyperbolic space shows no detected hub structure.} MERU and its Euclidean twin at 3 sizes: the census excess, the depth $z$ and the radii of the embeddings in units of the curvature scale. $^{\circ}$: not genuine at the corrected threshold.
}
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{The trained control separates the objectives by degree.} ViT-B/16 fine-tuned with leaf cross-entropy and with an added hierarchical term, 2 seeds: excess, depth $z$ intact and decoupled, on both frames.
}
