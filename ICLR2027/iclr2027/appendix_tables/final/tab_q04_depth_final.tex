...
\toprule
 & \multicolumn{2}{c}{C100, $K{=}20$} & \multicolumn{4}{c}{ImageNet, matched star} & bal.\ grouping \\
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{Clusters are oriented toward their hubs in 4 of 12 backbones under the star with Gaussian hubs.} Per backbone: the depth statistic and its $z$ on CIFAR-100 and on ImageNet, the WordNet cut at 3 sizes, and the balanced grouping.
}
\label{tab:q4-depth}
\end{table}
\begin{table}[H]
...
\toprule
model & star $e_{\mathrm{hub}}$ & depth & $z$ & $r_{\text{star}}$ & $z$ rotated & cert. \\
\midrule
...
flat control, balanced grouping & -- & -- & $-0.46$ & 0/5 & $-0.65$ & 0/50 \\
flat control, grouping mismatch & -- & -- & $-3.13$ & 5/5 & $-2.33$ & 38/50 \\
\bottomrule
\end{tabular}
\par\vspace{2pt}
\endgroup
\caption{\textbf{The same 4 backbones are certified under the star whose hubs are a Haar resample of the real ones.} Per backbone: the star's excess, the depth $z$, the star spread and the same test with clusters rotated, over 10 seeds.
}
\label{tab:q4-depth-ap}
\end{table}
\begin{table}[H]
\centering
\scriptsize
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{Supervision on leaf labels can produce the alignment but does not guarantee it.} The 2 ViT-B/16 trained on ImageNet-1k leaf labels, the excess and its rank on ImageNet and CIFAR-100, and the depth $z$ under both stars (WordNet agreement: Table~\ref{tab:q7-wordnet}).
}
\label{tab:q4-depth-b}
\end{table}
\begin{table}[H]
\centering
\scriptsize
...
\toprule
 & & \multicolumn{4}{c}{CIFAR-100 ($K{=}20$, $n{=}100$: false alarms uncontrolled)} & \multicolumn{4}{c}{ImageNet ($K{=}30$, $n{=}1000$: false alarms controlled)} \\
...
\end{tabular}
\par\vspace{2pt}
\caption{\textbf{Training in hyperbolic space shows no detected hub structure.} MERU and its Euclidean twin at 3 sizes, on images and on prompts: the excess, the rank $r$ of 200, the reading in each model's own metric and the depth $z$. $^{\circ}$: not genuine at the corrected threshold.
}
\label{tab:q4-depth-c}
\end{table}
\begin{table}[H]
\centering
\scriptsize
...
\begin{tabular}{lcccccccc}
\toprule
ViT-B/16 & excess & $r$ & $z$, WordNet-30 & $z$ rotated & cert. & $z$, balanced & $z$ rotated & cert. \\
\midrule
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{The trained control separates the objectives by degree.} ViT-B/16 fine-tuned with leaf cross-entropy and with an added hierarchical term, 2 seeds: excess, depth $z$ intact and with clusters rotated, on both groupings.
}
\label{tab:q4-depth-d}
\end{table}
\begin{table}[H]
\centering
\scriptsize
...
\toprule
& \multicolumn{4}{c|}{L2-normalized} & \multicolumn{4}{c}{radial component removed} \\
model & $z$ aniso & $z$ Haar-hub & $z$ rotated & cert. & $z$ aniso & $z$ Haar-hub & $z$ rotated & cert. \\
\midrule
...
\par\vspace{2pt}
\endgroup
\caption{\textbf{The alignment is not the radial spread of feature norms.} The same clouds after removing the radial part of every offset and after full L2 normalization: the depth $z$ under both stars.
}
\label{tab:q4-depth-e}
\end{table}
