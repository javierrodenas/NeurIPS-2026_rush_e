...
def main_khrulkov():
    ...
    if S85 is not None: assert S85.loc["cifar100", "p_left_max"] > 0.05 and S.loc["cifar100", "p_left_max"] <= 0.05, "caption: CIFAR-100 not below the null at 0.05 under the supremum (it is under the record statistic)"
    sup = (lambda d: f" & ${S85.loc[d, 'excess_sup_mean']:+.4f}$ & {int(round(S85.loc[d, 'r_above_median']))}/200, {S85.loc[d, 'p_left_max']:.3f}") if S85 is not None else (lambda d: "")
