...
# ======================================================================================================================
# Q4: the hierarchy test — real backbones (both stars, both frames, K sweep), leaf-label ViTs, MERU, the trained control
# ======================================================================================================================
...
def q_power():
    ...
    if FINAL and ex("expR80_decision.csv") and ex("expR80_implanted_alignment.csv"):   # priority 1c (brief of 2026-09-21): in the appendix whatever the decision rule said; its outcome is stated
        ...
        T.panel("(c) Planted hub alignment on the real ImageNet clouds: from the decoupled cloud of the decoupling control, each cluster's principal axis rotated toward its hub direction by a fraction $s$ of the angle (hubs and within-cluster spectra unchanged); detection rate at $z\\le-2$ over 5 seeds per backbone and pooled.", "l" + "c" * len(SS),
    ...
    if FINAL and ex("expR81_deep_per_backbone_summary.csv"):   # closing pass (2026-09-22): power per backbone at its own spectrum and ratio
        ...
        T.panel("(e) Power per backbone for a deep hierarchy at the backbone's own noise level: the three-level synthetic hierarchy of (d) built with each backbone's real ImageNet spectrum and within/between ratio; hierarchy test at $K{=}30$ (10 star seeds) on the intact cloud and the decoupling control (10 seeds) on each.", "lcccccc",
    ...
    if FINAL and ex("expR83_flat_balanced_summary.csv"):   # ninth review (2026-09-22): the balanced frame's false alarms on the flat control, matched and mismatched hub assignment
        ...
        T.panel("(f) False alarms of the balanced frame: the flat control of (d), ViT-L's spectrum and ratio, 5 seeds, read on the balanced thirty-superclass frame intact and decoupled (10 seeds each), with the flat hubs assigned by the balanced frame itself or by the WordNet-30 frame.", "lcc|ccc",
                [r"flat control & intact fired & mean $z$ & decoupled fired & mean $z$ & range \\"], rows, size=r"\footnotesize", colsep="3pt")
        ctxt83 = f" (f) With matched hubs the balanced frame fires once decoupled in {int(S83.loc['balanced', 'decoupled_fired'])} of {int(S83.loc['balanced', 'n_decoupled'])} runs, against {fa79} of {10 * len(E79f)} on the frame; a flat cloud clustered by the other frame fires on it intact in {int(S83.loc['wn30', 'intact_fired'])} of {int(S83.loc['wn30', 'n_intact'])}, which is frame mismatch, not depth. % expR83_flat_balanced_summary.csv, expR79_synthetic_deep_poincare.csv"
    T.write((r"\textbf{The hierarchy test has full power on synthetic hierarchies at the leaf frame and none at the top-level frame used on real backbones.} " if FINAL else r"\textbf{The hierarchy test meets its bar on synthetic hierarchies at ImageNet's class count, raises no false alarm on real clouds under either star, and misses planted depth at the real within-cluster spread.} ") +
