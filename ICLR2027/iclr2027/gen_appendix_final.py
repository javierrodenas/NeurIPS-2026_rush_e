...
def q_census():
    ...
    if ex("expR45_convnet_rows.csv"):   # the two self-supervised ResNets join the census as rows (author's brief, 2026-09-24)
        ...
        for a in load("expR45_convnet_rows.csv"):
            rows.append(RNM.get(a["model"], a["model"]) + f" & ${float(a['excessA']):+.3f}$ & {int(round(float(a['rankA']) * 20))}/20 & -- " + "& -- & -- & -- " * 5 + r"\\")
...
def q_robust():
    ...
    T.write(r"\textbf{The excess is budget-stable, its size is partly a property of the null construction, and the genuine count survives resampling and estimator noise.} "
            ...
            contd=[r"(c) Excess under three constructions of the spectrum null: Gaussian and PC-permutation agree, the exact-sample-spectrum construction yields smaller excesses for low-effective-rank clouds on the small-$C$ sets (most visibly DINOv2 on CIFAR-100), while signs and the ImageNet readings are stable. "
                   ...
                   r"(e) Class-count control under the record (means over subset seeds: 5 for $C\le200$, 2 for $C{=}500$, 1 for $C{=}1000$); ``below'' = subset seeds whose real value lies below the null at uncorrected $p\le0.05$; NC adv = prototype-classifier advantage of the Poincar\'{e} metric in pp. % expR60_c_sweep_record.csv"])
...
def q_sample():
    ...
    T.panel("(a) The census cells at the sample level." if FINAL else "", "lcccccc|cccccc", [r"& \multicolumn{6}{c|}{CIFAR-100 (10 images/class, $n{=}1000$)} & \multicolumn{6}{c}{DTD (22 images/class, $n{=}1034$)} \\",
            r"model & supremum $\hat\delta$ & $\hat\delta_{99.9}$ & excess & exc./null & $r$/200 & $p$ & supremum $\hat\delta$ & $\hat\delta_{99.9}$ & excess & exc./null & $r$/200 & $p$ \\"], rows, mids=(4, 9))
...
# ======================================================================================================================
# Q4: the hierarchy test — real backbones (both stars, both groupings, K sweep), leaf-label ViTs, MERU, the trained control
# ======================================================================================================================
...
def q_depth():
    ...
    for m in M12:
        ...
        rows.append(NAME[m] + " & " + " & ".join(cs) + r" \\")
    DEC = {}
    if FINAL:   # two narrower panels: the stars with Gaussian hubs and the groupings, then the star with Haar-resampled hubs
        ...
        T.panel("(a) The 12 backbones under the stars with Gaussian hubs: depth = hub excess $e_{\\mathrm{hub}}$ of the real centroids minus that of a matched star (negative = more hierarchical above the grouping), with $z$ against the combined spread; the WordNet cut at $K{=}10$, 30 and 60 superclasses and the balanced grouping.", "lcc@{\\hspace{5pt}}cccc@{\\hspace{5pt}}c",
                [r" & \multicolumn{2}{c}{C100, $K{=}20$} & \multicolumn{4}{c}{ImageNet, matched star} & bal.\ grouping \\",
        ...
        if FINAL and ex("expR83_flat_balanced_summary.csv"):   # the balanced-frame counts of the flat control join this table as rows (author's brief, 2026-09-24)
            ...
            _L83 = {"balanced": "flat control, balanced grouping", "wn30": "flat control, grouping mismatch"}
        ...
        T.panel("(a$'$) The same backbones under the star whose hubs are a Haar resample of the real hubs (ImageNet, $K{=}30$)" + ("; last column: the rotation control, the real hubs kept and each cluster's offsets rotated by an independent Haar rotation (mean $z$ $\\pm$ s.d.\\ over 10 seeds; seeds certified at $z\\le-2$)." if DEC else "."), "lcccc" + ("cc" if DEC else ""),
                [r"model & star $e_{\mathrm{hub}}$ & depth & $z$ & $r_{\text{star}}$" + (r" & $z$ rotated & cert." if DEC else "") + r" \\"], rows_h, mids=(4, 9), colsep="4pt")
    else:
        T.panel("(a) The 12 backbones: depth = hub excess $e_{\\mathrm{hub}}$ of the real centroids minus that of a matched star (negative = more hierarchical above the grouping), with $z$ against the combined spread.", "lcc@{\\hspace{5pt}}cc@{\\hspace{5pt}}ccc@{\\hspace{5pt}}c",
                [r" & \multicolumn{2}{c}{C100, $K{=}20$, Gaussian hubs} & \multicolumn{2}{c}{IN, $K{=}30$, Gaussian hubs} & \multicolumn{3}{c}{IN, $K{=}30$, Haar-resampled hubs} & bal.\ grouping \\",
                 r"\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-8}\cmidrule(lr){9-9}",
                 r"model & iso.\ star & aniso.\ star & iso.\ star & aniso.\ star & star $e_{\mathrm{hub}}$ & depth ($z$) & $r_{\text{star}}$ & real $z$ \\"], rows, mids=(4, 9), colsep="1.8pt")
    def summ(ds, K, v):
    ...
    if H:
        ...
        htxt = f" With the hubs of the matched star drawn as a Haar resample of the real hubs (exact hub spectrum, 10 star seeds) the certified set is {', '.join(NAME[m] for m in ch)}, with $z$ from ${max(zh):+.2f}$ to ${min(zh):+.2f}$; $r_{{\\text{{star}}}}$ counts the star seeds whose hub excess lies at or below the real one, a rank whose resolution is one part in eleven and which does not separate the certified backbones from the others."
    btxt = ""
    if BAL:
        cb = [m for m in M12 if float(BAL[m]["real_z"]) <= -2]; btxt = f" Balanced grouping (complete-linkage WordNet cut with the smallest variance of cluster sizes, chosen before any depth run): certified {', '.join(NAME[m] for m in cb)}."
    # (b) leaf-label ViTs
    ltxt = ""
    ...
    if FINAL and ex("expR77_positive_control.csv"):
        ...
        T.panel("(d) A trained positive control: the census ViT-B/16 fine-tuned on the full ImageNet-1k training set with leaf cross-entropy alone and with an added hierarchical cross-entropy at the WordNet 30/6/2 cuts (weights 1/2/4), 2 seeds, identical batches within a seed; census excess under the centered Haar null, hierarchy test and cluster rotation on both groupings.", "lcccccccc",
                [r"ViT-B/16 & excess & $r$ & $z$, WordNet-30 & $z$ rotated & cert. & $z$, balanced & $z$ rotated & cert. \\"], rows, size=r"\footnotesize", colsep="3pt")
        _c1 = int(round(P77.loc["ce_seed1", "dec_frac_cert_wn30"] * 10)) if "ce_seed1" in P77.index else None
        ftxt = (r" (d) Five epochs, AdamW, lr $10^{-5}$ encoder and $10^{-3}$ heads, batch 256, seeds 0 and 1; centroids of the census subset; cluster rotation over 10 seeds (fraction certified in parentheses). The hierarchical model is certified on both groupings and keeps firing with clusters rotated in both seeds; the leaf-CE model and the frozen checkpoint are certified intact, the frozen checkpoint does not fire with clusters rotated on the grouping"
                + (f" and the leaf-CE model fires there in 0 of 10 rotated seeds for seed 0 and {_c1} of 10 for seed 1" if _c1 is not None else " and the leaf-CE model does not either") + ", and the frozen checkpoint fires in 7 of 10 rotated seeds on the balanced grouping. "
    ...
    if FINAL and ex("expR82_radial_control_summary.csv"):   # eighth review (1e): the radial control, once merged over the twelve backbones
        ...
        if S82.model.nunique() == 12 and len(S82) == 24 and bool((S82.dec_runs == 10).all()):   # every backbone, both transforms, the ten decoupling seeds
            ...
            T.panel("(e) Radial control: the hierarchy test on the same clouds after L2-normalizing every centroid and after removing the radial component of every offset (its projection onto the hub direction), under both matched stars, with each cluster rotated on each transformed cloud.", "lcccc|cccc",
                    [r"& \multicolumn{4}{c|}{L2-normalized} & \multicolumn{4}{c}{radial component removed} \\", r"model & $z$ aniso & $z$ Haar-hub & $z$ rotated & cert. & $z$ aniso & $z$ Haar-hub & $z$ rotated & cert. \\"], rows, mids=(4, 9), size=r"\footnotesize", colsep="3pt")
            c_ = S82[S82.model.isin(["i21k_s", "i21k_b", "i21k_l", "dinov2_l"])]
            rtxt = f" (e) The four certified backbones keep $z\\le-2$ under both stars once the radial component is removed ({int((c_[c_['transform'] == 'deradial'].z_aniso <= -2).sum())} of 4), and {int((c_[c_['transform'] == 'l2norm'].z_aniso <= -2).sum())} of 4 under full L2 normalization; the transformed clouds with clusters rotated fire in {int(round(c_[c_['transform'] == 'deradial'].dec_frac_certified.sum() * 10))} of 40 runs. % expR82_radial_control_summary.csv"
    T.write((r"\textbf{The hierarchy test certifies in 4 of 12 ImageNet backbones under both matched stars that clusters are oriented toward their hubs, not that the hubs of the grouping form a hierarchy, and no hub hierarchy is found in 9 of 12 backbones where the control with clusters rotated detects a planted one, and in the other 3 the test cannot see a planted hierarchy: none fires once cluster orientations are randomized, whereas a three-level hierarchy planted at the same noise level is detected and survives that randomization (Table~\ref{tab:q5-power}); imposing the geometry does not create the structure, and leaf-label supervision can produce it or not.} " if (FINAL and DEC) else r"\textbf{Hierarchy above the superclasses is certified in four ImageNet backbones under both matched stars, imposing the geometry does not create it, and leaf-label supervision can produce it or not.} ") +
            r"(a) Isotropic star: Gaussian clouds with each superclass's RMS spread (the earlier, withdrawn construction); anisotropic star: within each superclass a Haar sample with the cloud's own covariance, hubs Gaussian at the real hub radius; Haar-resampled hubs: the same clusters with the hubs drawn as a Haar resample of the real hubs. Hub-randomizing Haar null, 10 star seeds; groupings: CIFAR-100 coarse labels, ImageNet WordNet cut. The test is validated at $n{=}1000$ only (Table~\ref{tab:q5-power}); CIFAR-100 readings ($n{=}100$) are reported without certification."
...
# ======================================================================================================================
# Q5: power of the hierarchy test — synthetic hierarchies, planted trees on real clouds, both stars
# ======================================================================================================================
def q_power():
    ...
    T.panel("(a) Synthetic hierarchies at $d{=}768$: two- and three-level hierarchies (pooled) and pure stars, within/between noise ratios 0.1/0.3/0.6, isotropic and anisotropic clusters pooled, 5 seeds.", "llcccccc",
            [r"grouping & $n$ & $K$ & power @ ratio 0.1 & 0.3 & 0.6 & star $z\le-2$ & star $z\ge+2$ \\"], rows, mids=tuple(mids[:-1]), size=r"\footnotesize", colsep="4pt")
    bar_ = ""
    if f2.exists():
        ...
        bar_ = (f" Pre-set bar: power $\\ge0.8$ for two- and three-level hierarchies at noise ratios $\\le0.3$ for both $n$, and false alarms $\\le5$\\% in each direction. Leaf grouping: met at $n{{=}}1000$ (power {p03:.2f} at ratios $\\le0.3$ and {p06:.2f} at $0.6$; false alarms {100*fa1000[0]:.0f}\\% and {100*fa1000[1]:.0f}\\%), failed at $n{{=}}100$, where $K\\ge12$ (under ten points per cluster) gives {100*fa100:.0f}\\% false alarms at $z\\le-2$. Top-level grouping: power {top_pw:.2f}, false alarms {100*top_fa:.0f}\\% pooled.")
    # (b) planted
    ptxt = ""
    if ex("expR64b_wn30_summary.csv"):
        ...
        T.panel("(b) Planted two-level trees on the real ImageNet centroids: the hub arrangement replaced at strength $s$, every within-cluster offset kept; hits = planted-tree seeds with $z\\le-2$ at $s{=}0/0.5/1$, $\\bar z_1$ = mean $z$ at $s{=}1$, w/b = within/between spread.", "lcccccc|cc|ccc",
                [r" & \multicolumn{6}{c|}{WordNet-30 grouping, Gaussian-hub star} & \multicolumn{2}{c|}{Haar-hub star} & \multicolumn{3}{c}{balanced grouping (pre-specified)} \\",
        ...
        fctxt = (f" Balanced grouping: the $K{{=}}30$ agglomerative cut of the WordNet distance matrix ({fc['chosen']} linkage) with the smallest variance of cluster sizes, chosen and logged before any depth run; sizes {fc['chosen_sizes'][0]}--{fc['chosen_sizes'][-1]} classes against {min(fc['sizes']['average'])}--{max(fc['sizes']['average'])} for the grouping." if fc else "")
        htx = ""
        if Hs:
            ...
            htx = f" Under the Haar-hub star the same planted trees give {fa} of {n_} false alarms at $s{{=}}0$ and {hi} of {n1} detections at $s{{=}}1$ (power {hi/max(1,n1):.2f})."
        ptxt = (r" (b) Six super-hubs of five, hub $= s\cdot$super-hub $+ (1-s+0.4s)\cdot$own Gaussian draw, hub cloud rescaled to the real RMS radius; the hierarchy test of Table~\ref{tab:q4-depth} runs unchanged, 5 planted-tree seeds per $s$. hits: planted-tree seeds with $z\le-2$ (of 5); $s^*$: first $s$ with $\ge4$ of 5; within/between: RMS of the within-cluster offsets over the point-weighted RMS of the hub displacements"
                + f" ({ratio_lo:.1f} to {ratio_hi:.1f} on the real clouds, against 0.1--0.6 in the synthetic sweep); tight: the same planted tree after shrinking the offsets to within/between $=0.6$ (2 seeds). Power (fraction of runs with $z\\le-2$) by $s$: " + ", ".join(f"$s{{=}}{s:g}$: {v:.2f}" for s, v in pw.items()) + f"; false alarms at $s{{=}}0$: {fa0} of {n0}." + htx + fctxt)
    ctxt80 = ""; ctxt79 = ""
    if FINAL and ex("expR80_decision.csv") and ex("expR80_implanted_alignment.csv"):   # priority 1c (brief of 2026-09-21): in the appendix whatever the decision rule said; its outcome is stated
        ...
        T.panel("(c) Planted hub alignment on the real ImageNet clouds: from the cloud with its clusters rotated, each cluster's principal axis rotated toward its hub direction by a fraction $s$ of the angle (hubs and within-cluster spectra unchanged); detection rate at $z\\le-2$ over 5 seeds per backbone and pooled.", "l" + "c" * len(SS),
        ...
        ctxt80 = (f" (c) Planted alignment is detected in {h1} of {n1} runs at full strength ({100 * h1 / n1:.0f}\\%) and in {h0} of {n0} at zero; " + ("the pre-set bar of 0.8 power at full strength with false alarms at or below 0.05 is met" if met80 else "the pre-set bar of 0.8 power at full strength is not met")
    ...
    if FINAL and ex("expR79_synthetic_deep_poincare.csv"):   # priority 1b (brief of 2026-09-21): a deep hierarchy at the real noise level, and the WordNet Poincare embeddings
        ...
        T.panel("(d) A deep hierarchy at the real noise level: synthetic clouds with ViT-L's real ImageNet spectrum and a three-level planted hierarchy (nested 2/6/30 cuts of the grouping) at ViT-L's real within/between ratio, a flat two-level control (30 iid hubs, same spectrum and ratio), 5 seeds each, and the WordNet Poincar\\'e embeddings of Nickel and Kiela trained on the transitive closure of the tree over the 1000 ImageNet leaves, read with Euclidean distances on the ball coordinates.", "lccccccc",
                [r"cloud & seed & $d$ & excess & $r$ & depth $z$ & $z$ rotated & cert. \\"], rows, mids=(5, 10), size=r"\footnotesize", colsep="4pt")
        deep = E79[E79.cloud == "synthetic_deep_vitl_spectrum"]; flat = E79[E79.cloud == "synthetic_flat_vitl_spectrum"]
        ctxt79 = (f" (d) Census excess under the centered Haar null (rank in parentheses), hierarchy test with the matched anisotropic star at $K{{=}}30$ (10 star seeds), cluster rotation (mean $\\pm$ s.d.\\ over 10 seeds; fraction certified in parentheses). The deep hierarchy fires in {int((deep.z <= -2).sum())} of {len(deep)} seeds and the flat control in {int((flat.z <= -2).sum())} of {len(flat)}; "
                  f"the decoupling control fires in {int((deep.zdec_mean <= -2).sum())} of {len(deep)} deep seeds and reads deeper than the intact cloud" + (", from both sides: the rotation shrinks the star spread and deepens the excess below the star" if (ex("final_dec_obs.json") and json.load(open(RES / "final_dec_obs.json")).get("kind") == "both") else "") + r". The WordNet Poincar\'e embeddings \citep{nickel2017poincare}, read with Euclidean distances, do not fire: the hierarchy they carry lives in the curvature and is not visible to a Euclidean reading, so they are not a positive control. % expR79_synthetic_deep_poincare.csv, final_dec_obs.json")
    ctxt81 = ""
    if FINAL and ex("expR81_deep_per_backbone_summary.csv"):   # closing pass (2026-09-22): power per backbone at its own spectrum and ratio
        ...
        T.panel("(e) Power per backbone for a deep hierarchy at the backbone's own noise level: the three-level synthetic hierarchy of (d) built with each backbone's real ImageNet spectrum and within/between ratio; hierarchy test at $K{=}30$ (10 star seeds) on the intact cloud and the rotated one (10 seeds) on each.", "lcccccc",
                [r"model & within/between & seeds & power & mean $z$ & power rotated & mean $z$ rotated \\"], rows, mids=(4, 9), size=r"\footnotesize", colsep="4pt")
    ...
    if FINAL and ex("expR83_flat_balanced_summary.csv"):   # ninth review (2026-09-22): the balanced frame's false alarms on the flat control, matched and mismatched hub assignment
        ...
        LBL = {"balanced": "matched hubs (balanced grouping)", "wn30": "mismatched hubs (WordNet-30 grouping)"}
        ...
        T.panel("(f) False alarms of the balanced grouping: the flat control of (d), ViT-L's spectrum and ratio, 5 seeds, read on the balanced thirty-superclass grouping intact and with clusters rotated (10 seeds each), with the flat hubs assigned by the balanced grouping itself or by the WordNet-30 grouping.", "lcc|ccc",
                [r"flat control & intact fired & mean $z$ & fired rotated & mean $z$ & range \\"], rows, size=r"\footnotesize", colsep="3pt")
        ctxt83 = f" (f) With matched hubs the balanced grouping fires with clusters rotated in {int(S83.loc['balanced', 'decoupled_fired'])} of {int(S83.loc['balanced', 'n_decoupled'])} runs, against {fa79} of {10 * len(E79f)} on the grouping; a flat cloud clustered by the other grouping fires on it intact in {int(S83.loc['wn30', 'intact_fired'])} of {int(S83.loc['wn30', 'n_intact'])}, which is a grouping mismatch, not depth. % expR83_flat_balanced_summary.csv, expR79_synthetic_deep_poincare.csv"
    T.write((r"\textbf{The hierarchy test has full power on synthetic hierarchies at the leaf grouping and none at the top-level grouping used on real backbones.} " if FINAL else r"\textbf{The hierarchy test meets its bar on synthetic hierarchies at ImageNet's class count, raises no false alarm on real clouds under either star, and misses planted depth at the real within-cluster spread.} ") +
            r"(a) Power = fraction of hierarchy runs with $z\le-2$; star columns = false-alarm rates in each direction. Top-level grouping: the test receives the $K$ super-cluster labels, as the earlier isotropic test did on the real backbones; its hub null keeps every cluster of the grouping intact and only rearranges the $K$ hubs, so hierarchy below the grouping is invisible by construction. Leaf grouping: the test receives the finest cluster labels with the anisotropic matched star (configurations with more leaves than points are infeasible and omitted)." + bar_ + ptxt + ctxt80 + ctxt79 + ctxt81 + ctxt83
...
def q_corollary():
    ...
    T.newpart()
    T.panel("(d) The same correlations on the raw $\\hat\\delta_{99.9}$, the record excess and the hierarchy-test $z$ (Fisher-$z$ CI95; dm: family-demeaned $r$).", "llcc|cc|c",
    ...
    T.write(r"\textbf{The calibrated reading predicts the zero-cost gain within datasets, the hierarchy verdict predicts nothing, and cosine is the safe default.} "
            r"(a) Nearest-centroid (NC) and 5-way 5-shot (FS, 1000 paired episodes) accuracy under Euclidean and Poincar\'e metrics, regenerated from the audited reruns; FS advantage of cosine and of the radial map with Euclidean distances (the map without the metric changes nothing) over Euclidean, and of the Poincar\'e metric over cosine after L2 normalization. % table1_regenerated.csv, exp2_metric_controls.csv, exp2b_normalized_stack.csv",
            contd=[r"(b) Within-dataset correlations of the raw supremum reading; bootstrap CI95 from 10k resamples of the 10 models, pooled rows by cluster bootstrap over datasets; family control: both variables centered within family before correlating, and within-family correlations (ViT, DINOv2: 4 models each; CLIP has two, so only its sign is shown). "
                   ...
                   + r" (c) The gain per backbone and which metric collects it. % night/correlation_cis.csv, exp20_null_ztable.csv, exp2_metric_controls.csv",
                   r"(d) The calibrated reading: the raw 99.9th-percentile statistic, the record excess and the hierarchy-test $z$ of Table~\ref{tab:q4-depth} as predictors. (e) Validation picking retains the oracle advantage and the zero-validation objective rule modestly beats always-cosine. (f) The gate's value is avoiding the Poincar\'e projection on flat-label data; cosine is a safe NC default everywhere. (g) The headline $t=1/\sqrt2\approx0.71$ was fixed a priori and is conservative for FS. % expR68_corollary_excess.csv, exp24_val_metric_selection.csv, night/t_sweep.csv"])
...
def q_robust_final():
    ...
    T.write(r"\textbf{The excess is budget-stable, the genuine count survives resampling and estimator noise, the excess shrinks with the class count, and training moves it.} "
            ...
            contd=[r"(c) Per cell: $z_{\text{joint}} = \text{excess}/\sqrt{\sigma_{\text{null}}^2+\sigma_{\text{boot}}^2+\sigma_{\text{est}}^2}$ and the number of resamples in which the cell is genuine under BH over the 72 cells; $^{\circ}$: not genuine in the record; $^{\dagger}$: record-genuine but failing one of the two criteria ($z_{\text{joint}}\le-2$, genuine in $\ge27$ of 30); $^{\ddagger}$: not record-genuine but passing one." + jt
                   + r" (d) Class-count control under the record (means over subset seeds): the excess shrinks with the number of classes for coherent and random subsets alike, so magnitudes are not comparable across class counts and the verdict is what carries across datasets; ``below'' = subset seeds whose real value lies below the null at uncorrected $p\le0.05$; NC adv = prototype-classifier advantage of the Poincar\'{e} metric in pp. (e) Raw $\delta$ within architecture: fine-tuning on a task without class hierarchy raises it and it falls across transformer depth. % expR66_joint_sensitivity_summary.csv, expR60_c_sweep_record.csv, analysis4_finetuning.csv, e1_delta_by_layer.csv" + ctxt])
...
def main_khrulkov():
    ...
    if S85 is not None: assert all(S85.loc[d, "p_left_median"] < 0.05 for d in ("cifar100", "miniimagenet")) and all(S85.loc[d, "p_left_median"] > 0.05 for d in ("cifar10", "cub")) and S85.loc["cifar100", "p_left_max"] > 0.05, "caption: the median trial decides; CIFAR-100 below in the median trial but not in every trial"
    sup = (lambda d: f" & ${S85.loc[d, 'excess_sup_mean']:+.4f}$ & {int(round(S85.loc[d, 'r_above_median']))}/200, {S85.loc[d, 'p_left_min']:.3f}--{S85.loc[d, 'p_left_max']:.3f}") if S85 is not None else (lambda d: "")
    ...
    hdr = ((r" & & & & & \multicolumn{2}{c}{their statistic, the supremum} \\" "\n" r"\cmidrule(lr){6-7}" "\n"
            r"dataset & their $\delta_{\text{rel}}$ & ours, their estimator & excess & $r$/200, largest $p$ & excess & $r$/200, $p$ range \\") if S85 is not None else r"dataset & their $\delta_{\text{rel}}$ & ours, their estimator & excess & $r$/200, largest $p$ \\")
    cap = (r"\caption{\textbf{A published reading reproduced and calibrated.} ResNet-34 setting of \citet{Khrulkov_2020_CVPR}: their relative hyperbolicity, ours with their estimator, then the excess and the rank of 200 with the largest left-tail $p$ for the reading (99.9th percentile). For their own statistic, the supremum from a fixed base point, against the same replicates: the mean excess, the median rank over 10 trials and the range of $p$ across trials. CIFAR-100 is below the null in the median trial but not in every trial. % expR78_khrulkov_replication_summary.csv, expR85_khrulkov_sup_summary.csv" if S85 is not None
    ...
    L = ["% prov: expR78_khrulkov_replication_summary.csv" + (", expR85_khrulkov_sup_summary.csv" if S85 is not None else ""), r"\begin{table}[t]", r"\centering", r"\footnotesize", r"\setlength{\tabcolsep}{" + ("2.0pt" if S85 is not None else "5pt") + "}", r"\begin{tabular}{" + ("lcccccc" if S85 is not None else "lcccc") + "}", r"\toprule", hdr, r"\midrule"] + rows + [r"\bottomrule", r"\end{tabular}", cap, "}", r"\label{tab:khrulkov}", r"\end{table}"]
