...
def q_robust():
    ...
    T.write(r"\textbf{The excess is budget-stable, its size is partly a property of the null construction, and the genuine count survives resampling and estimator noise.} "
            r"(a) Excess against the quadruple budget for nine cells; drift = range of the record excess over budgets $\ge10^5$, divided by the excess's spread in the last column." + a5
...
def q_depth():
    ...
    if FINAL and ex("expR77_positive_control.csv"):
        ...
        LB = {"frozen": "frozen (census)", "ce_seed0": "leaf cross-entropy", "hier_seed0": "leaf + hierarchical cross-entropy"}
        rows = [f"{LB[m]} & ${P77.loc[m, 'excess']:+.4f}$ ({int(P77.loc[m, 'r_above'])}) & ${P77.loc[m, 'z_wn30']:+.2f}$ & ${P77.loc[m, 'zdec_mean_wn30']:+.2f}$ ({P77.loc[m, 'dec_frac_cert_wn30']:.1f}) & ${P77.loc[m, 'z_wn30bal']:+.2f}$ & ${P77.loc[m, 'zdec_mean_wn30bal']:+.2f}$ ({P77.loc[m, 'dec_frac_cert_wn30bal']:.1f}) \\\\" for m in ("frozen", "ce_seed0", "hier_seed0") if m in P77.index]
        T.panel("(d) A trained positive control: the census ViT-B/16 fine-tuned on the full ImageNet-1k training set with leaf cross-entropy alone and with an added hierarchical cross-entropy at the WordNet 30/6/2 cuts (weights 1/2/4), one seed, identical batches; census excess under the centered Haar null, depth test and decoupling control on both frames.", "lccccc",
                [r"ViT-B/16 & excess ($r$) & $z$, WordNet-30 & decoupled $z$ (cert.) & $z$, balanced & decoupled $z$ (cert.) \\"], rows, size=r"\footnotesize", colsep="4pt")
        ftxt = (r" (d) Five epochs, AdamW, lr $10^{-5}$ encoder and $10^{-3}$ heads, batch 256, seed 0; centroids of the census subset; decoupling over 10 seeds (fraction certified in parentheses). The hierarchical model is certified on both frames and keeps firing once decoupled; the leaf-CE model and the frozen checkpoint are certified intact but do not fire once decoupled on the frame of record, and the frozen checkpoint fires in 7 of 10 decoupled seeds on the balanced frame. "
    ...
    T.write((r"\textbf{The depth test certifies in four ImageNet backbones under both matched stars that clusters are oriented toward their hubs, not that the hubs form a hierarchy, and no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven: none fires once cluster orientations are randomized, whereas a three-level hierarchy implanted at the same noise level is detected and survives that randomization (Table~\ref{tab:q5-power}); imposing the geometry does not create the structure, and leaf-label supervision can produce it or not.} " if (FINAL and DEC) else r"\textbf{Hierarchy above the superclasses is certified in four ImageNet backbones under both matched stars, imposing the geometry does not create it, and leaf-label supervision can produce it or not.} ") +
...
def q_power():
    ...
    if FINAL and ex("expR81_deep_per_backbone_summary.csv"):   # closing pass (2026-09-22): power per backbone at its own spectrum and ratio
        ...
        cov = [NAME[m] for m in M12 if m in S81.index and S81.loc[m, "power"] >= 0.8]
        ctxt81 = f" (e) Power $\\ge0.8$ in {len(cov)} backbones ({', '.join(cov)}) and below in the other {12 - len(cov)}; DINOv2 with 20 seeds, the rest with 5. % expR81_deep_per_backbone_summary.csv"
