...
def q_depth():
    ...
    if ex("expR65_hier_finetune.csv"):
        F = {r["obj"]: r for r in load("expR65_hier_finetune.csv")}; LB = {"frozen":"frozen (census)","ce":"cross-entropy","hier":"cross-entropy + hierarchical CE (WordNet-30)"}
    ...
    T.write((r"\textbf{Hub--offset structure above the superclasses is certified in four ImageNet backbones under both matched stars and none of them fires once the offsets are decoupled from the hubs; imposing the geometry does not create it, and leaf-label supervision can produce it or not.} " if (FINAL and DEC) else r"\textbf{Hierarchy above the superclasses is certified in four ImageNet backbones under both matched stars, imposing the geometry does not create it, and leaf-label supervision can produce it or not.} ") +
            ...
            contd=[mtxt.strip() + ftxt + r" % expR63_meru_record.csv, expR71_meru_radii.csv, expR65_hier_finetune.csv"])
...
def conservation_check(verbose=True):
    ...
    order = [s for s in open(OLD / "INPUT_ORDER.txt").read().split() if s != "tab_b1_textnulls"]
    old_tokens = {}
    for s in order:
