...
def q_depth():
    ...
    T.write((r"\textbf{The depth test certifies in four ImageNet backbones under both matched stars that clusters are oriented toward their hubs, not that the hubs of the frame form a hierarchy, and no hub hierarchy is found in the nine backbones where the decoupled control detects an implanted one, and the test is blind in the other three: none fires once cluster orientations are randomized, whereas a three-level hierarchy implanted at the same noise level is detected and survives that randomization (Table~\ref{tab:q5-power}); imposing the geometry does not create the structure, and leaf-label supervision can produce it or not.} " if (FINAL and DEC) else r"\textbf{Hierarchy above the superclasses is certified in four ImageNet backbones under both matched stars, imposing the geometry does not create it, and leaf-label supervision can produce it or not.} ") +
