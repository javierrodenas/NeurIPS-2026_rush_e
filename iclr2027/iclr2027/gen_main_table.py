...
for m in ORDER:
    r = d20[(m,"imagenet")]
    exc_tr = st.mean(float(d20[(m,ds)]["excess"]) for ds in HIER_TR)
...
lines = [r"\begin{table}[t]", r"\centering", r"\scriptsize", r"\setlength{\tabcolsep}{2.6pt}",
         r"\begin{tabular}{l c c c c c c c c}", r"\toprule",
         r"model & $\delta_{\text{norm}}$ IN & excess IN ($z$) & excess transfer & $\xi$-excess IN & $\rho_{\text{WN}}$ & ARI$_{20}$ & best$-$R (pp) & metric \\",
         r"\midrule"]
for i,(n,dl,ex,z,tr,xi,rho,ari,best,met) in enumerate(rows):
    ...
    lines.append(f"{n} & ${dl:.3f}$ & {exs} & ${tr:+.3f}$ & ${xi:+.3f}$ & ${rho:+.2f}$ & ${ari:.2f}$ & {bests} & {met} \\\\")
lines += [r"\bottomrule", r"\end{tabular}",
  r"\caption{Calibrated census of the 12 vision backbones (supervised / self-supervised / contrastive). "
  r"$\delta_{\text{norm}}$: raw ImageNet reading; excess IN: excess over the spectrum-matched null with $z$ (bold: sign-positive, at null); excess transfer: mean over CIFAR-100, CIFAR-10, DTD; "
...
for r in rows: print(r[0], f"d={r[1]:.3f} exc={r[2]:+.3f} z={r[3]:+.1f} tr={r[4]:+.3f} xi={r[5]:+.3f} rho={r[6]:+.2f} ari={r[7]:.2f} best={r[8]} {r[9]}")
