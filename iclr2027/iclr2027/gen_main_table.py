...
ORDER = list(NAME)
src = "expR39b_census20_cache.csv"
if not (RES/src).exists():
    src = "expR39_census20.csv"; print(f"WARNING: expR39b not found; layout run from {src} (store-based ImageNet rows)")
...
for m in ORDER:
    ...
    rows.append((NAME[m], fam(m), float(r["delta"]),
                 float(r["excess"]), round(20*float(r["frac_null_above"])),
                 float(cen[(m, "cifar100")]["excess"]), float(cen[(m, "cifar10")]["excess"]),
                 float(d3[m]["spearman_wn"])))
lines = [r"\begin{table}[t]", r"\centering", r"\small", r"\setlength{\tabcolsep}{4.5pt}",
         r"\begin{tabular}{l l c c c c c}", r"\toprule",
         r"model & family & $\delta_{\text{norm}}$ IN & excess IN ($r$/20) & exc.\ C100 & exc.\ C10 & $\rho_{\text{WN}}$ \\",
         r"\midrule"]
for i, (n, f, dl, ex, rk, e100, e10, rho) in enumerate(rows):
    if i in (4, 9): lines.append(r"\midrule")
    exs = f"${ex:+.3f}$ ({rk}/20)"
    if ex > 0: exs = r"\textbf{" + exs + "}"
    c100 = f"${e100:+.3f}$"; c10 = f"${e10:+.3f}$"
    if e100 > 0: c100 = r"\textbf{" + c100 + "}"
    if e10 > 0: c10 = r"\textbf{" + c10 + "}"
    lines.append(f"{n} & {f} & ${dl:.3f}$ & {exs} & {c100} & {c10} & ${rho:+.2f}$ \\\\")
lines += [r"\bottomrule", r"\end{tabular}",
  ...
  r"excess: $\delta_{\text{norm}}$ minus the spectrum-matched null mean, with the percentile rank $r$/20 = how many of the 20 "
  r"null replicates lie above the real value (20/20 = below every replicate; bold: sign-positive, i.e.\ less tree-like than the null); "
...
for r in rows: print(r[0], f"{r[1]} d={r[2]:.3f} exc={r[3]:+.3f} rank={r[4]}/20 c100={r[5]:+.3f} c10={r[6]:+.3f} rho={r[7]:+.2f}")
