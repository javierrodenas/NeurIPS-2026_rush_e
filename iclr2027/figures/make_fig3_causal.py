...
stored ablation results: (a) random vs pretrained weights (e5_random_control.csv),
(b) non-hierarchical fine-tuning (analysis4_finetuning.csv), (c) delta across depth
...
rnd = {a["model"]: float(a["delta_normalized"]) for a in r if a["mode"]=="random"}
ms = ["i21k_b","dinov2_s","dinov2_b"]
pct = [100*(rnd[m]-pre[m])/pre[m] for m in ms]
b = ax[0].bar(range(3), pct, color=RED, width=0.6)
...
ax[0].set_ylabel(r"$\Delta\delta$: random vs trained (%)", fontsize=6.5); ax[0].set_ylim(0, max(pct)*1.25)
ax[0].set_title("(a) Random weights inflate $\\delta$", fontsize=7)
...
ax[1].bar(range(3), pct, color=RED, width=0.6)
for i,v in enumerate(pct): ax[1].text(i, v+3, f"+{v:.0f}%", ha="center", fontsize=6.5, fontweight="bold")
ax[1].set_xticks(range(3)); ax[1].set_xticklabels([NAME[m] for m in ms], fontsize=6.5)
ax[1].set_ylabel(r"$\Delta\delta$ vs pretrained (%)", fontsize=6.5); ax[1].set_ylim(0, max(pct)*1.25)
ax[1].set_title("(b) Non-hierarchical fine-tuning", fontsize=7)
# (c) depth
...
for m, col in [("dinov2_b", GREEN), ("clip_b_vision", BLUE)]:
    ...
    ax[2].plot(xs, ys, "-o", color=col, ms=3, lw=1.2, label=LEG[m])
    print(m, "layers", xs, "delta first->last", round(ys[0],3), "->", round(ys[-1],3), f"({100*(ys[-1]-ys[0])/ys[0]:+.0f}%)")
ax[2].set_xlabel("transformer layer", fontsize=6.5); ax[2].set_ylabel(r"$\delta$", fontsize=6.5)
ax[2].set_title("(c) $\\delta$ falls across depth", fontsize=7); ax[2].legend(fontsize=6, frameon=False)
