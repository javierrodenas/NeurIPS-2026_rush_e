#!/usr/bin/env python3
"""expR55 (review-response, A2): power of the matched-star depth test on synthetic clouds.
...
Output: expR55_depth_power.csv (one row per run); figures/fig_depth_power.pdf made by --figure.
"""
...
def make(level, K, ratio, aniso, n, seed):
    ...
    sup = rng.randn(K, d); lab1 = labels(rng, K, n)
    if level == "star":
        centers, grp = sup[lab1], lab1
    elif level == "hier2":
        sub = sup[:, None, :] + 0.5*rng.randn(K, 5, d); lab2 = rng.randint(0, 5, n)
        centers, grp = sub[lab1, lab2], lab1*5+lab2
    else:
        sub = sup[:, None, :] + 0.5*rng.randn(K, 3, d); sub2 = sub[:, :, None, :] + 0.25*rng.randn(K, 3, 3, d)
        lab2 = rng.randint(0, 3, n); lab3 = rng.randint(0, 3, n)
        centers, grp = sub2[lab1, lab2, lab3], (lab1*3+lab2)*3+lab3
    return (centers + ratio*noise(rng, n, d, aniso, grp)).astype(np.float32), lab1
...
def run(part, Ks):
    csv_path = OUT/f"expR55_depth_power.part_{part}.csv"
...
def merge():
    parts = sorted(glob.glob(str(OUT/"expR55_depth_power.part_*.csv")))
    df = pd.concat([pd.read_csv(p) for p in parts]).drop_duplicates(subset=["level","K","ratio","aniso","n","seed"])
    df.to_csv(OUT/"expR55_depth_power.csv", index=False)
...
def figure():
    ...
    df = pd.read_csv(OUT/"expR55_depth_power.csv"); h = df[df.level!="star"]; st = df[df.level=="star"]
