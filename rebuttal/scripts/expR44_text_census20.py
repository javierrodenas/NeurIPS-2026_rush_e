#!/usr/bin/env python3
"""expR44 (round-7 P7): the text census (paper template) re-run with 20 spectrum-null replicates.
...
each) per (model, template). Output: expR44_text_census20.csv."""
import os, sys, time
...
def specnull(C, rep):
    ...
    return (G*S)@Vt + mu
@torch.no_grad()
def encode(kind, hf, pool, prefix, texts, fp16, bs=16):
...
def main():
    ...
    csv_path = OUT/"expR44_text_census20.csv"
