...
# ---- S5.3: priority 1b (expR79): synthetic deep hierarchy with ViT-L's spectrum, and the WordNet Poincare embeddings, through the hierarchy test
if not IN79 and os.path.exists(R + 'expR79_synthetic_deep_poincare.csv') and pd.read_csv(R + 'expR79_synthetic_deep_poincare.csv').cloud.str.startswith('wordnet_poincare_d50').any():   # the last row of the run: partial results never enter
    ...
    tables.append("% prov: expR79_synthetic_deep_poincare.csv\n\\begin{table}[tbp]\n\\centering\n\\footnotesize\n\\setlength{\\tabcolsep}{4pt}\n\\begin{tabular}{lccccc}\n\\toprule\n"
                  ...
                  "\n\\bottomrule\n\\end{tabular}\n\\caption{\\textbf{Priority 1b: a deep hierarchy at the real noise level, and the WordNet Poincar\\'e embeddings.} Synthetic clouds with ViT-L's real ImageNet spectrum and a three-level planted hierarchy (nested 2/6/30 cuts of the frame) at ViT-L's real within/between ratio, and a flat two-level control, 5 seeds each; the WordNet Poincar\\'e embeddings of Nickel and Kiela (2017) trained on the transitive closure of the tree spanning the 1000 ImageNet leaves at $d{=}10$ and $50$. "
                  "Census excess under the centered Haar null (200 replicates, rank in parentheses), hierarchy test with the matched anisotropic star at $K{=}30$ (10 star seeds), and the decoupling control (mean $\\pm$ s.d.\\ over 10 seeds). % expR79_synthetic_deep_poincare.csv\n}\n\\label{tab:r1b-deep}\n\\end{table}\n")
...
# ---- S5.3: priority 1c (expR80): planted alignment; enters the rebuttal file only when the decision rule is NOT met (otherwise it is in the submission already)
if not IN80 and os.path.exists(R + 'expR80_decision.csv') and os.path.exists(R + 'expR80_implanted_alignment.csv'):
    ...
    if not met80:
        ...
        par = ("\\paragraph{Planted alignment on the real clouds.} From the decoupled cloud of each backbone, each cluster's principal axis was rotated toward its hub direction by a fraction $s$ of the angle, hubs and within-cluster spectra unchanged, and the test was read at every $s$. "
        ...
        tables.append("% prov: expR80_implanted_alignment.csv, expR80_decision.csv\n\\begin{table}[tbp]\n\\centering\n\\footnotesize\n\\setlength{\\tabcolsep}{6pt}\n\\begin{tabular}{l" + "c" * len(SS) + "}\n\\toprule\n"
                      ...
                      "\n\\bottomrule\n\\end{tabular}\n\\caption{\\textbf{Priority 1c: planted alignment on the real ImageNet clouds.} From the decoupled cloud of expR74 (real hubs kept, offsets Haar-rotated), each cluster's principal axis rotated toward its hub direction by a fraction $s$ of the angle, hubs and within-cluster spectra unchanged; detection rate at $z\\le-2$ under the matched anisotropic star at $K{=}30$, "
