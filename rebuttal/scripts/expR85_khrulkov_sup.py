...
left-tail p = (1 + K - r) / (K + 1). One exact delta_rel takes 10-17 s on 1500 points, so a trial costs about 40 min; run in shards.
...
def run(ds, trials):
    ...
    for t in trials:
        ...
        Xb = batch(ds, t); t0 = time.time(); dr = R78.delta_rel_theirs(Xb)
        nulls = np.array([R78.delta_rel_theirs(R75.null_haar_centered(Xb.astype(np.float32), r).astype(np.float64)) for r in range(N_REP)])
...
def merge():
    ...
    S = df.groupby("dataset").agg(n_trials=("trial", "count"), delta_rel_sup_mean=("delta_rel_sup", "mean"), null_mean=("null_mean", "mean"), excess_sup_mean=("excess_sup", "mean"), excess_sup_sd=("excess_sup", "std"), r_above_mean=("r_above", "mean"), p_left_max=("p_left", "max"), p_left_min=("p_left", "min")).reset_index()
