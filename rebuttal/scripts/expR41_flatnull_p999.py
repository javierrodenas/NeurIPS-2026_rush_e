#!/usr/bin/env python3
"""expR41 (round-4 follow-up): the hierarchy-flattening null B under the p99.9 statistic.
...
clustering. Output: expR41_flatnull_p999.csv."""
import os, sys, time
...
def main():
    csv_path = OUT/"expR41_flatnull_p999.csv"
    ...
    for ds, sup in [("imagenet", WN30), ("cifar100", SUP100)]:
        for m in MODELS:
            ...
            nA = [delta_norm(spec_sample(C, r, 300), n_seeds=3)[0] for r in range(20)]
            nB = [delta_norm(flatnull(C, sup, r), n_seeds=3)[0] for r in range(20)]
