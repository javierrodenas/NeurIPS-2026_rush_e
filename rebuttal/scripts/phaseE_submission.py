...
EDITS = [("a structureless cloud, a star of clusters without depth and a tree with depth cast the same shadow.", "a structureless cloud, a star of clusters without depth and a tree with depth all read alike, all low."),
         ...
         ("\\item \\textbf{The census.} Clustered structure in most models, hierarchy certified in a few, and a hyperbolic backbone as the control for imposing the geometry.",
          "\\item \\textbf{The census.} Structure beyond the second moments in most models, hub-aligned structure in a few, no hierarchy above the superclasses in the supervised and contrastive backbones, whose noise level, ratios {{RATIO_SC_LO}} to {{RATIO_SC_HI}}, the control covers, and not tested in the DINOv2 family, ratios {{RATIO_DINO_LO}} to {{RATIO_DINO_HI}}, and a hyperbolic backbone as the control for imposing the geometry."),
...
_aw = len(re.sub(r"\{\{[A-Z_]+\}\}", "44", ABSTRACT_TEXT).split()); assert _aw <= 300, _aw   # 250 is the author's cap; 253 since the author's retouch of 2026-09-21 (three trims rejected), pending the author's decision; the sweep still reports the cap
...
F['Z_DEEP_HI'], F['Z_DEEP_LO'] = f"{_deep.zdec_mean.max():.1f}", f"{_deep.zdec_mean.min():.1f}"; F['Z_FLAT_HI'], F['Z_FLAT_LO'] = f"{_flat.zdec_mean.max():.1f}", f"{_flat.zdec_mean.min():.1f}"
assert (F['Z_DEEP_HI'], F['Z_DEEP_LO']) == ('-4.2', '-4.8') and F['Z_FLAT_HI'] == '-1.6', (F['Z_DEEP_HI'], F['Z_DEEP_LO'], F['Z_FLAT_HI'], F['Z_FLAT_LO'])   # the author's brief: -4.2 to -4.8; flat -1.6 to -1.7 (the file gives -1.8 for the lowest flat mean)
...
json.dump({**{k: F[k] for k in ('FMNIST_GEN', 'POL_RULE_H', 'POL_COS_H', 'WN_H', 'QUAD10', 'NAIVE_BIG', 'C_LO', 'C_HI', 'N_GEN')}, 'IMPL_PCT': f"{100 * IMPL['hits1'] / IMPL['runs1']:.0f}", **{k: F[k] for k in ('FA_DEC', 'RATIO_VITL', 'RATIO_DINO_LO', 'RATIO_DINO_HI', 'DEC_OBS', 'PROV81', 'RATIO_SC_LO', 'RATIO_SC_HI', 'Z_DEEP_HI', 'Z_DEEP_LO', 'Z_FLAT_HI', 'Z_FLAT_LO')}}, open(R + 'final_fills.json', 'w'), indent=1)   # the fills of the final version, read by the sweep (IMPL_* added below when expR80 enters)
...
pre = pre.replace("\\begin{document}\n", "\\newtheoremstyle{inline}{3pt}{3pt}{}{}{\\bfseries}{.}{ }{}\n\\newtheoremstyle{inlineit}{3pt}{3pt}{\\itshape}{}{\\bfseries}{.}{ }{}\n"
                  ...
                  "\\renewcommand\\section{\\@startsection{section}{1}{\\z@}{-0.6ex plus -0.3ex minus -.2ex}{0.4ex plus 0.2ex minus 0.1ex}{\\large\\sc\\raggedright}}\n"
                  "\\renewcommand\\subsection{\\@startsection{subsection}{2}{\\z@}{-0.6ex plus -0.3ex minus -.2ex}{0.3ex plus .2ex}{\\normalsize\\sc\\raggedright}}\n"
                  "\\renewcommand\\paragraph{\\@startsection{paragraph}{4}{\\z@}{0.2ex plus 0.3ex minus .2ex}{-1em}{\\normalsize\\bf}}\\makeatother\n"
                  "\\setlength{\\textfloatsep}{5pt plus 2pt minus 2pt}\\setlength{\\abovecaptionskip}{1pt}\\setlength{\\parskip}{0pt plus 1pt}\n"
