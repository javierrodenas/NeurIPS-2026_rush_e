...
EDITS = [('Figure~\\ref{fig:concept} shows why a low reading is not enough: a structureless cloud, a star of clusters without depth and a tree with depth cast the same shadow.', 'Figure~\\ref{fig:concept} frames the question: an observer who sees only a shadow, the raw $\\delta$, asks which of three worlds cast it, a structureless cloud, a star of clusters or a tree. All three read alike.'),
         ...
         ("\\item \\textbf{The map of trees.} The island is an artifact of the cut; sharing is graded and supervision-dependent, and the self-supervised tree is angular.", "\\item \\textbf{The map of trees.} The island is an artifact of the cut; the tree topology is largely shared and the metric is not, and the self-supervised tree is angular.")]
def edit(s):
    for a, b in EDITS:
...
ABSTRACT_TEXT = ("Hyperbolic representation learning rests on a premise we call latent hyperbolicity: standard models are already tree-like because their class geometry scores a low Gromov $\\delta$, a measure of tree-likeness that is zero for a tree. "
    ...
    "The depth test certifies in {{N_IN}} of 12 ImageNet backbones that clusters are oriented toward their hubs, not that the hubs of the frame form a hierarchy: randomizing the orientations removes every verdict, whereas an implanted hierarchy, synthetic or trained in, survives it. "
    "No hub hierarchy is found in the nine backbones where the decoupled control detects an implanted one, and the test is blind in the other three. "
    ...
    "Across models the tree topology is largely shared and the metric is not: the naive comparison manufactures an island, triplet agreement reaches {{TRIP_BIG}} against a within-model ceiling of {{CEIL_TRIP_LO}}--{{CEIL_TRIP_HI}} once the clustering cut is controlled, and the self-supervised tree is angular. "
    "Read correctly, foundation models organize classes into clustered structure whose tree topology is largely shared, up to a gap beyond resampling noise, and whose metric is not; no hub hierarchy is found where the test has power; and their raw tree-likeness is not evidence for hyperbolic geometry.")
