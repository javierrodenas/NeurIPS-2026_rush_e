...
# ---- priority 1c (expR80, brief of 2026-09-20 evening): the planted-alignment control enters the submission only when the decision rule
...
EDITS = [('Figure~\\ref{fig:concept} shows why a low reading is not enough: a structureless cloud, a star of clusters without depth and a tree with depth cast the same shadow.', 'Figure~\\ref{fig:concept} frames the question: an observer who sees only a shadow, the raw $\\delta$, asks which of three worlds cast it, a structureless cloud, a star of clusters or a tree. All three read alike.'),
         ...
         ("\\item \\textbf{The census.} Clustered structure in most models, hierarchy certified in a few, and a hyperbolic backbone as the control for imposing the geometry.",
          '\\item \\textbf{The census.} Structure beyond a random cloud in most models, clusters oriented toward their hubs in a few, no hub hierarchy in 9 of 12 backbones where a planted one is detected once cluster orientations are randomized, a blind test in the other 3, and a nominally hyperbolic backbone as the control for imposing the geometry.'),
         ...
         ('On image features, where the premise is read, the calibrated reading is indistinguishable from a random cloud of the same shape in most cells and small elsewhere; the 4 values reported by \\citet{Khrulkov_2020_CVPR} are reproduced and, calibrated, 2 of them are indistinguishable from a random cloud. On class centroids there is structure that a random cloud does not have, in 44 of 72 cells and 30 of 36 on the 3 datasets with many classes (ImageNet, CIFAR-100 and DTD), but a star of clusters already produces it. The depth test certifies in 4 of 12 ImageNet backbones that clusters are oriented toward the superclass centers, which we call hubs, not that the hubs form a hierarchy within the grouping of classes into superclasses: randomizing the orientations removes every verdict, whereas a planted hierarchy, synthetic or trained in, survives it; no hub hierarchy is found in 9 of 12 backbones where a planted one is detected once cluster orientations are randomized, and the test is blind in the other 3. A backbone trained in hyperbolic space shows the same structure as its Euclidean twin and lives in the near-flat regime. Across models, the trees agree on which classes group together well above chance, though less than two readings of the same model, and not on distances; the self-supervised models organize classes by direction rather than by distance, which a comparison by distance misses.', '(i) \\emph{Where the premise is read,} on image features, the calibrated reading is indistinguishable from a random cloud of the same shape in most model--dataset cells and small elsewhere. The 4 values reported by \\citet{Khrulkov_2020_CVPR} are reproduced and, calibrated, 2 of them are indistinguishable from a random cloud. (ii) \\emph{Within each model,} class centroids carry structure that a random cloud does not have, in 44 of 72 cells and 30 of 36 on the 3 datasets with many classes (ImageNet, CIFAR-100 and DTD), but a star of clusters already produces it. The hierarchy test finds more in 4 of 12 ImageNet backbones: the orientation of each cluster relative to its superclass center, which we call its hub, not a hierarchy among the hubs. Randomizing the orientations removes every verdict, whereas a planted hierarchy, synthetic or trained in, survives. In the 9 of 12 backbones where a planted hierarchy is detected, none as strong is found; in the other 3 the test is blind. A backbone trained in hyperbolic space shows the same structure as its Euclidean twin and stays nearly flat. (iii) \\emph{Across models,} the trees agree on which classes group together well above chance, though less than two resampled versions of the same model, and not on distances; the self-supervised models organize classes by direction rather than by distance, which a comparison by distance misses.'),
         ('\\item \\textbf{The census.} Structure beyond a random cloud in most models, clusters oriented toward their hubs in a few, no hub hierarchy in 9 of 12 backbones where a planted one is detected once cluster orientations are randomized, a blind test in the other 3, and a nominally hyperbolic backbone as the control for imposing the geometry.', '\\item \\textbf{The census.} The premise calibrated where it is read, including a published reading reproduced; structure beyond a random cloud in most models; clusters oriented toward their hubs in a few; no hierarchy among the hubs where the test can see one; and a hyperbolic-trained backbone as the control for imposing the geometry.'),
         ...
         ('and they are never calibrated: a value is called low on its own, without asking what a structureless cloud of the same dimension and spectrum would score', 'and they are read against the ideal values of a tree and of a non-hyperbolic space, never against what a structureless cloud of the same dimension and spectrum, its spread along each direction, would score'),
...
,
         ...
         # plain-language pass (author's brief, 2026-09-25): S1 and the abstract keep no coined term but "excess"; the
         # hierarchy test is "the second test" here, hubs are "superclass centers", and "blind" is said in words
         ('adds a \\emph{hierarchy test} whose', 'adds a \\emph{second test} whose'),
         ('The hierarchy test finds more in 4 of 12 ImageNet backbones: the orientation of each cluster relative to its superclass center, which we call its hub, not a hierarchy among the hubs. Randomizing the orientations removes every verdict, whereas a planted hierarchy, synthetic or trained in, survives. In the 9 of 12 backbones where a planted hierarchy is detected, none as strong is found; in the other 3 the test is blind.',
          'The second test finds more in 4 of 12 ImageNet backbones: how each cluster is oriented relative to its superclass center, not a hierarchy among those centers. Rotating each cluster at random removes every verdict, whereas a hierarchy we plant ourselves, synthetic or trained into a model, survives. In the 9 of 12 backbones where a planted hierarchy is detected, none as strong is found; in the other 3 the test cannot see a planted hierarchy.'),
         ...
         ('2 of them are indistinguishable from a random cloud (Figure~\\ref{fig:premise}).',
          'only MiniImageNet stays below the random cloud. CIFAR-10 and CUB-200 are indistinguishable from a random cloud, and CIFAR-100 falls below it only before correcting for testing four datasets at once.'),
         ...
         # plain-language pass at first use (author's brief, 2026-09-25): the null cloud, the 200 replicates and what the second test measures
         ('expresses every reading as an \\emph{excess} over a null cloud matched in dimension and covariance spectrum, ranks that excess against matched replicates',
...
_dra = _dr.z   # the sentence says 'under both stars', so the range runs over both (author's correction, 2026-09-25)
F['Z_RAD_HI'], F['Z_RAD_LO'] = f"{_dra.max():.2f}", f"{_dra.min():.2f}"
...
_cov = [m for m in _ord if _s81.loc[m, 'dec_power'] >= 0.8]; _unc = [m for m in _ord if _s81.loc[m, 'dec_power'] < 0.8]; assert len(_cov) == 9 and len(_unc) == 3, (_cov, _unc)
F['N_COV'] = str(len(_cov))   # the abstract counts the backbones where a planted hierarchy is detected (author's brief, 2026-09-25)   # ninth review (2026-09-22): one criterion, the decoupled power, the test that decides: 'the nine' and 'the other three'
def _names(ms):
    out = []; k = 0
    while k < len(ms):
...
# ---- BH consistency for the published values (author's brief, 2026-09-25): the paper's definition of genuine, applied across the 4 datasets
_s78bh = pd.read_csv(R + 'expR78_khrulkov_replication_summary.csv').set_index('dataset')
_p78 = _s78bh['p_left_max'].sort_values(); _m78 = len(_p78); _k78 = 0
for _i, (_d, _v) in enumerate(_p78.items(), 1):
    if float(_v) <= 0.05 * _i / _m78: _k78 = _i
_bh78 = list(_p78.index[:_k78]); assert _bh78 == ['miniimagenet'], _bh78   # only MiniImageNet survives BH over the four
F['KH_C100_P'] = f"{float(_s78bh.loc['cifar100', 'p_left_max']):.3f}"; assert F['KH_C100_P'] == '0.030', F['KH_C100_P']
json.dump({"p_left_max": _s78bh['p_left_max'].round(4).to_dict(), "bh_genuine": _bh78, "alpha": 0.05, "rule": "Benjamini-Hochberg over the 4 published datasets"}, open(R + 'final_khrulkov_bh.json', 'w'), indent=1)
...
F['FA_MIS'] = str(_mis)
F['BAL_SENT'] = f"A flat cloud clustered under the WordNet grouping fires under the balanced grouping in {_mis} of 50 runs with clusters rotated. A real cloud read under a grouping that is not its own is therefore expected to fire. The frozen ViT-B firing in {F['PC_FROZEN_BAL']} of 10 there is consistent with that mismatch and is not evidence of hub structure."   # the author's frame-mismatch reading (tenth review), split at 45 words (consolidated pass)
...
INTRO = {
 "tab_q08_robust": r"\paragraph{What this table answers.} Two choices could have made the reading look structured: how many quadruples we sample and which images stand behind each centroid. Each is varied in turn and the same cells are read again. The verdicts move only where the excess was already at the edge of the null.",
...
HOWTO = ""   # the reading page and the glossary go (author's brief, 2026-09-24): each caption defines what it uses
...
app = app.replace("with their result files indexed in Table~\\ref{tab:provenance}.", "and the README of the supplementary material maps each table and figure to its script and its result file.", 1)
head = app[:app.index("\\FloatBarrier")].replace("\\section{Appendix: one table per question}", "\\section{Additional results}\n\\renewcommand{\\arraystretch}{0.92}") + HOWTO
...
REPL_ALL = [("Power on synthetic and implanted hierarchies", "Power on synthetic and planted hierarchies"),
            ("Is the reading robust? Quadruple budget, null construction, resampling and class count", "Is the reading robust? Quadruple budget and resampling"),   # appendix reduction (2026-09-24); the class count left the title with its figure (2026-09-26)
...
# ---- ninth review (2026-09-22): B.7 points to Table 8(d) instead of the superseded inconclusive control; the planted two-level tree curves (formerly Figure 4b) become an appendix figure
...
_figI = ("\\begin{figure}[H]\n\\centering\n\\includegraphics[width=0.5\\linewidth]{figures/fig_implant_final.pdf}\n\\caption{\\textbf{A planted two-level tree on the real ImageNet clouds is missed at the real within-cluster spread and detected once the spread is shrunk.} "
         "Detection rate at $z\\le-2$ against planted-tree strength $s$ with the real within-cluster spread (solid) and shrunk (dashed): at full strength " + F['IMPL_REAL_S1'] + " against " + F['IMPL_SHRUNK_S1'] + ", and " + F['IMPL_REAL_S0'] + " at $s=0$ with the real spread. % expR64b_wn30.csv\n}\n\\label{fig:implant}\n\\end{figure}\n")
...
# the per-cell census carries the label of the former main-text table too (author's brief, 2026-09-24): one census table
...
DETAIL = {   # appendix reduction (2026-09-24): the paragraphs of the former Appendix C sit in the question they belong to
  'tab_q01_census': '\\paragraph{The structure depends on the class set more than on the model.} The smallest backbone is genuine on one dataset only, and the flat datasets have smaller and less consistent excesses; FMNIST is genuine in {{FMNIST_GEN}} of 12 backbones. Table~\\ref{tab:q1-census-b} gives the details. % expR52_census_haar_p999_200.csv, expR45_convnet_rows.csv\n\n\\paragraph{Neural collapse is the flat limit, not what the census sees.} Neural collapse predicts that supervised class means converge to a simplex frame \\citep{papyan2020prevalence}, a star with no hierarchy. The census does not certify that picture: class means cluster around superclass hubs, which the superclass recovery of Section~\\ref{sec:content} shows. % expR52_census_haar_p999_200.csv, analysis4_finetuning.csv, e1_delta_by_layer.csv',
  'tab_q04_depth': '\\paragraph{The certified set under other groupings.} Only ViT-B and ViT-L are certified under every grouping. {{BAL_SENT}} % expR64b_wn30bal_summary.csv, expR77_positive_control.csv, expR83_flat_balanced_summary.csv',
  'tab_q09_corollary': '\\paragraph{The zero-cost metric.} In few-shot learning a fixed-radius Euclidean encoder does at least as well as hyperbolic prototypes \\citep{moreira2024hyperbolic} (Table~\\ref{tab:q9-corollary}). % exp2b_normalized_stack.csv, exp24_val_metric_selection.csv',
}
for _k, _v in DETAIL.items():
    for _fk, _fv in F.items(): DETAIL[_k] = DETAIL[_k].replace('{{' + _fk + '}}', _fv)
    assert '{{' not in DETAIL[_k], DETAIL[_k][:80]
for _k, _v in DETAIL.items():
    _a = '\n\\input{appendix_tables/' + FINAL_SRC.get(_k, _k) + '}'
    assert bytab[_k].count(_a) == 1, _k
    bytab[_k] = bytab[_k].replace(_a, '\n' + _v + '\n' + _a, 1)
labels = {}
for stem in KEEP:
...
NEWCAP = {
 ...
 "tab_q14_panel": r"\textbf{The models and the extraction.} Every backbone, language model and text embedder of the census and of the controls, with its parameter count; the note below the table says how features are extracted and pooled.",
 ...
 "tab_q04_depth": r"\textbf{The hierarchy test certifies that the orientation of clusters relative to their hubs is not random in a few backbones, and finds no hierarchy among the hubs.} Panels: the 12 backbones under both matched stars, the leaf-label ViTs, a hyperbolic backbone with its Euclidean twin, the trained control and the radial control.",
 "tab_q05_power": r"\textbf{The test detects a planted hierarchy where the cloud is quiet enough, and its false alarms are measured.} Panels: planted two-level trees, planted hub alignment, a deep hierarchy at the real noise level, the power per backbone and the false alarms of the balanced grouping.",
 ...
 "tab_q13_local": r"\textbf{Cross-model agreement survives permutation calibration.} Columns: the raw measure, the null mean, the calibration threshold, the calibrated value, the null-centred value and the number of model pairs that survive, for mutual-kNN agreement and for centroid CKA.",
 "tab_q09_corollary": r"\textbf{Both readings predict the gain of a non-Euclidean metric, and the hierarchy verdict predicts none.} Panels: accuracies and zero-cost gains per cell, the correlations under the raw reading and under the calibrated one, and the best zero-cost metric per backbone.",
}
...
TITLE = {"(a) The 12 backbones under the stars with Gaussian hubs": "(a) The 12 backbones under the star with Gaussian hubs",
         ...
         "(c) Planted hub alignment on the real ImageNet clouds": "(c) Planted hub alignment on the real ImageNet clouds",
         "(d) A deep hierarchy at the real noise level": "(d) A deep hierarchy at the real noise level",
         "(f) False alarms of the balanced grouping": "(f) False alarms of the balanced grouping",
...
PANELCAP = {
 ...
 ("tab_q08_robust", "b"): r"\textbf{The genuine count survives resampling of the images behind each centroid.} Per cell: images per class, null replicates, the reference excess, the mean and spread over 30 bootstrap resamples, and the fraction of resamples whose excess stays negative.",
 ...
 ("tab_q01_census", "a"): r"\textbf{Structure beyond the second moments is the rule at the class level.} Per cell: the excess over the matched null, the number $r$ of replicates above it and the left-tail $p$; for Barlow Twins and BYOL, $r$ of 20.",
 ...
 ("tab_q02_text", "a"): r"\textbf{In text the verdict follows the recipe and the scale.} Per model: dimension, the reading, the excess, the excess as a fraction of the null, the rank, the left-tail $p$, $k$/4 constructions calling it genuine, and the cosine reading beside it.",
 ...
 ("tab_q04_depth", "a"): r"\textbf{How clusters are oriented relative to their hubs is not random in 4 of 12 backbones under the star with Gaussian hubs.} Per backbone: depth and $z$ on CIFAR-100 and ImageNet, 3 WordNet cuts and the balanced grouping.",
 ("tab_q04_depth", "a$'$"): r"\textbf{The same 4 backbones are certified under the star whose hubs are a Haar resample of the real ones.} Per backbone: the star's excess, the depth $z$, the star spread and the same test with clusters rotated, over 10 seeds.",
 ("tab_q04_depth", "b"): r"\textbf{Supervision on leaf labels can produce the alignment but does not guarantee it.} The 2 ViT-B/16 trained on ImageNet-1k leaf labels, the excess and its rank on ImageNet and CIFAR-100, and the depth $z$ under both stars (WordNet agreement: Table~\ref{tab:q7-wordnet}).",
 ("tab_q04_depth", "c"): r"\textbf{Training in hyperbolic space shows no detected hub structure.} MERU and its Euclidean twin at 3 sizes, on images and on prompts: the excess, the rank $r$ of 200, the reading in each model's own metric and the depth $z$.",
 ("tab_q04_depth", "d"): r"\textbf{The trained control separates the objectives by degree.} ViT-B/16 fine-tuned with leaf cross-entropy and with an added hierarchical term, 2 seeds: excess, depth $z$ intact and with clusters rotated, on both groupings.",
 ...
 ("tab_q05_power", "c"): r"\textbf{Planted hub alignment reproduces the verdict in the supervised ViTs and CLIP-B.} From the cloud with clusters rotated, each cluster's principal axis turned toward its hub: the detection rate per backbone and seed.",
 ("tab_q05_power", "d"): r"\textbf{A deep hierarchy at the real noise level is detected, and a flat control is not.} Synthetic clouds with ViT-L's spectrum and ratio: the depth $z$ intact and with clusters rotated, for the hierarchy and for the flat control.",
 ("tab_q05_power", "e"): r"\textbf{The power follows the backbone.} The same deep hierarchy built with each backbone's spectrum and ratio: the intact power and the power with clusters rotated per backbone, and the within-between ratio it sits at.",
 ("tab_q05_power", "f"): r"\textbf{The balanced grouping raises no false alarm.} The flat control of the deep experiment read on the balanced grouping: the firing counts intact and with clusters rotated.",
 ...
 ("tab_q06_treemap", "b"): r"\textbf{The self-supervised models sit apart on distances, not on topology.} The three agreement measures for DINOv2 against the supervised and contrastive models. Each cell: DINOv2 against the other models / the other models among themselves.",
 ...
 ("tab_q07_wordnet", "a"): r"\textbf{Agreement with the human taxonomy follows supervision and recipe.} Per backbone on ImageNet: the correlation between inter-centroid and WordNet distances, the same under shuffled labels, a Gaussian cloud and single images, and the raw reading of WordNet against random groups.",
 ...
 ("tab_q09_corollary", "a"): r"\textbf{The zero-cost gains are modest and limited to prototype tasks.} The 10 backbones with cached per-dataset features, DINO-B and SigLIP-B excluded. Per cell: nearest-centroid and few-shot accuracy under the Euclidean (Eucl.) and the Poincar\'{e} (Poinc.) metric, then the advantage of each zero-cost metric in percentage points over the Euclidean metric.",
 ...
 ("tab_q09_corollary", "c"): r"\textbf{Cosine collects the gain for the self-supervised models.} Per backbone: the best zero-cost advantage over the Euclidean metric, averaged over the hierarchical datasets, and which metric collects it.",
 ("tab_q09_corollary", "d"): r"\textbf{The calibrated reading predicts the gain as well as the raw one, and the hierarchy verdict predicts none.} Per dataset and task: the same correlations for the raw statistic, the excess and the hierarchy-test $z$; dm is the same correlation with both variables centred within family.",
}
for _k, _v in PANELCAP.items(): assert len(_v.split()) <= (50 if "$k$/5:" in _v or "$k$/4" in _v or "(Eucl.)" in _v or "dm is" in _v else 40), (_k, len(_v.split()))   # 50 words when the caption spends some of them defining a symbol or an abbreviation
...
_NOTE14 = ("\\par\\vspace{3pt}\n{\\scriptsize Extraction: vision backbones use the encoder's default pooled embedding (timm, CLS or global pool per architecture); "
           "causal language models the last-token state of ``a photo of a \\{class\\}'', one prompt at a time, since batching alters GPT-2's hidden states; "
           "text embedders their recommended pooling, CLS for BGE, mean for GTE and E5, last token for GTE-Qwen2; the CLIP and SigLIP text towers their own text encoders.}\n")
_p14 = FD + "tab_q14_panel.tex"; _t14 = open(_p14).read()   # the extraction notes the caption promises, under the table (author's brief, 2026-09-25)
assert _t14.count("\\end{tabular}") == 1 and "Extraction:" not in _t14
open(_p14, "w").write(_t14.replace("\\end{tabular}", "\\end{tabular}" + _NOTE14, 1))
# every symbol is defined in the caption of the table that uses it (author's brief, 2026-09-24: the appendix glossary is gone)
SYMDEF = [("^{\\circ}", " $^{\\circ}$: not genuine at the corrected threshold."), ("$r$/200", " $r$/200: replicates above the real value."),
          ("w/b", " w/b: the within-cluster spread over the spread between hubs."), ("$s^{*}$", " $s^{*}$: the smallest strength at which the test fires."),
          ("$\\bar z_1$", " $\\bar z_1$: the mean $z$ at full strength.")]
_ndef = 0
for _fn in sorted(os.listdir(FD)):
    if not _fn.startswith("tab_q"): continue
    _src = open(FD + _fn).read(); _out = []
    for _blk in re.split(r"(?=\\begin\{table\})", _src):
        _m = re.search(r"(\\caption\{)(.*?)(\n\})", _blk, re.S)
        if _m:
            _body = _blk[:_m.start()] + _blk[_m.end():]; _cap = _m.group(2); _add = ""
            for _sym, _txt in SYMDEF:
                if _sym in _body and _sym not in _cap: _add += _txt; _ndef += 1
            if _add: _blk = _blk[:_m.start()] + _m.group(1) + _cap + _add + _m.group(3) + _blk[_m.end():]
        _out.append(_blk)
...
_pan = bytab["tab_q14_panel"]; _pre = "\\FloatBarrier\n\\subsection{The model panel and extraction}\n\\label{app:panel}\n"
assert _pan.startswith(_pre), _pan[:120]
_impl = "\\section{Implementation}\n\\label{app:panel}\n" + _pan[len(_pre):]
appendix = _impl + head + "".join(bytab[s] for s in order if s != "tab_q14_panel")   # the provenance table is now the README of the supplementary material
for _k, _v in F.items(): appendix = appendix.replace("{{" + _k + "}}", _v)   # the C.8 pointer carries a fill (author's brief, 2026-09-25)
...
appendix = appendix.replace("\\section{Implementation}", "\\renewcommand{\\topfraction}{0.95}\\renewcommand{\\bottomfraction}{0.95}\\renewcommand{\\textfraction}{0.03}\\renewcommand{\\floatpagefraction}{0.85}\\setcounter{topnumber}{4}\\setcounter{bottomnumber}{4}\\setcounter{totalnumber}{6}\\renewcommand{\\arraystretch}{0.92}\n"
                            "% final version: the tables float ([tbp], one float per panel) within their subsection (a \\FloatBarrier closes each one)\n\\section{Implementation}", 1)
assert "\\input{appendix_tables/final/tab_q08_robust_final}" in appendix and appendix.count("\\FloatBarrier") >= 11
...
pre = pre.replace("\\begin{document}\n", "\\newtheoremstyle{inline}{3pt}{3pt}{}{}{\\bfseries}{.}{ }{}\n\\newtheoremstyle{inlineit}{3pt}{3pt}{\\itshape}{}{\\bfseries}{.}{ }{}\n"
                  ...
                  ""   # textfloatsep, abovecaptionskip and parskip: the template's own (compliance pass, 2026-09-24)
...
BOXPRE = r"""
% ---- boxed statements: the colors of the figure palette (author's brief, 2026-09-25); definitions in the supervised-ViT blue,
% Proposition 1 in the self-supervised teal, both over a pale wash of their own hue. Thin left rule, no frame, 2pt padding, same font.
\usepackage[most]{tcolorbox}
\definecolor{stmtdefrule}{HTML}{3B528B}    % palette.py FAMILY_COLORS["supervised"]
\definecolor{stmtdefback}{HTML}{E9EDF5}
\definecolor{stmtproprule}{HTML}{21918C}   % palette.py FAMILY_COLORS["ssl"]
\definecolor{stmtpropback}{HTML}{E2F2F1}
...
\newtcolorbox{defbox}{statementbox=stmtdefrule, colback=stmtdefback}
\newtcolorbox{propbox}{statementbox=stmtproprule, colback=stmtpropback}
"""
def boxed(t):
    ...
    t = t[:i5] + t[j5:]
    # full-read fixes (2026-09-24): the pointer paragraph is gone; the S5.4 tree-map paragraph carries the local-agreement sentence
