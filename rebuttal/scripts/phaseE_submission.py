...
ABSTRACT_TEXT = ("Hyperbolic representation learning rests on a premise we call latent hyperbolicity: standard models are already tree-like because their class geometry scores a low Gromov $\\delta$, a measure of tree-likeness that is zero for a tree. "
    ...
    "Read correctly, foundation models organize classes into clustered structure that is moderately shared and does not converge to one common tree; no hub hierarchy is found where the test has power; and their raw tree-likeness is not evidence for hyperbolic geometry.")
...
F['FA_MIS'] = str(_mis)
F['BAL_SENT'] = f"On the balanced frame the frozen ViT-B fires once decoupled in {F['PC_FROZEN_BAL']} of 10 seeds. That frame's matched flat control does not fire once decoupled, so this is not a false alarm: ViT-B shows hub structure under that grouping that the frame of record does not resolve; and a cloud organized under one frame fires under the other even once decoupled, so frames are not interchangeable."   # the author's sentence of 2026-09-22 (afternoon) with its two balanced-frame counts moved to 'What the test can see' (each number once, evening brief)
json.dump({"fa_bal": _fab, "fa_record": int(F['FA_DEC']), "high": bool(_high), "rule": "high if fa_bal >= 2 * fa_record", "sentence": F['BAL_SENT'], "author_sentence": "adapted: counts moved to S5.3(c), evening brief of 2026-09-22", "mismatch_construction_fired": _mis, "mismatch_intact_fired": int(_s83.loc['wn30', 'intact_fired'])}, open(R + 'final_bal_frame.json', 'w'), indent=1)
DEC_OBS_KIND = "open"
