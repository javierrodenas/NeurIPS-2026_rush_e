...
EDITS = [('Figure~\\ref{fig:concept} shows why a low reading is not enough: a structureless cloud, a star of clusters without depth and a tree with depth cast the same shadow.', 'Figure~\\ref{fig:concept} frames the question: an observer who sees only a shadow, the raw $\\delta$, asks which of three worlds cast it, a structureless cloud, a star of clusters or a tree. All three read alike.'),
         ...
         ("\\item \\textbf{The census.} Clustered structure in most models, hierarchy certified in a few, and a hyperbolic backbone as the control for imposing the geometry.",
          '\\item \\textbf{The census.} Structure beyond a random cloud in most models, clusters oriented toward their hubs in a few, no hub hierarchy in the five backbones where the control detects an implanted one, a blind test in the other seven, and a hyperbolic backbone as the control for imposing the geometry.'),
...
ABSTRACT_TEXT = ("Hyperbolic representation learning rests on a premise we call latent hyperbolicity: standard models are already tree-like because their class geometry scores a low Gromov $\\delta$, a measure of tree-likeness that is zero for a tree. "
    ...
    "The depth test certifies in {{N_IN}} of 12 ImageNet backbones that clusters are oriented toward their hubs, not that the hubs form a hierarchy: randomizing the orientations removes every verdict, whereas an implanted hierarchy, synthetic or trained in, survives it. "
    "No hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven. "
    ...
    "Read correctly, foundation models organize classes into clustered structure that is moderately shared and does not converge to one common tree; no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven; and their raw tree-likeness is not evidence for hyperbolic geometry.")
...
_cov = [m for m in _ord if _s81.loc[m, 'power'] >= 0.8]; _unc = [m for m in _ord if _s81.loc[m, 'power'] < 0.8]; assert len(_cov) == 5 and len(_unc) == 7, (_cov, _unc)   # 'the five' and 'the other seven'
def _names(ms):
    out = []; k = 0
    while k < len(ms):
        pre = _NM[ms[k]].rsplit('-', 1)[0]; grp = [ms[k]]
        while k + len(grp) < len(ms) and _NM[ms[k + len(grp)]].rsplit('-', 1)[0] == pre and pre in ('ViT', 'DINOv2'): grp.append(ms[k + len(grp)])
...
F['P81_COVERED'] = ', '.join(_NM[m] for m in _cov[:-1]) + ' and ' + _NM[_cov[-1]]; F['P81_UNCOVERED'] = _names(_unc)   # the author's lists: the covered ones spelled out, the uncovered ones compressed by family
F['P81_COV_LO'], F['P81_COV_HI'] = f"{_s81.loc[_cov, 'power'].min():.2f}", f"{_s81.loc[_cov, 'power'].max():.2f}"; F['P81_UNC_LO'], F['P81_UNC_HI'] = f"{_s81.loc[_unc, 'power'].min():.2f}", f"{_s81.loc[_unc, 'power'].max():.2f}"
assert F['P81_COVERED'] == 'ViT-L, DINOv2-L, DINOv2-G, CLIP-L and SigLIP-B' and F['P81_UNCOVERED'] == 'ViT-T/S/B, DINO-B, DINOv2-S/B and CLIP-B', (F['P81_COVERED'], F['P81_UNCOVERED'])
assert all(_s81.loc[m, 'power'] == 0.0 for m in ('i21k_t', 'i21k_s', 'i21k_b')) and all(_s81.loc[m, 'power'] >= 0.8 for m in ('dinov2_l', 'dinov2_g')), "'three supervised ViTs miss the implant, DINOv2-L and G detect it'"
_v3 = _r64[['i21k_t', 'i21k_s', 'i21k_b']]; F['RATIO_VIT3_LO'], F['RATIO_VIT3_HI'] = f"{_v3.min():.1f}", f"{_v3.max():.1f}"; assert f"{_r64['dinov2_l']:.1f}" == f"{_r64['dinov2_g']:.1f}"; F['RATIO_DINOLG'] = f"{_r64['dinov2_l']:.1f}"
