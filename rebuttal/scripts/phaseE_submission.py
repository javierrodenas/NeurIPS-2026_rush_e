...
EDITS = [('Figure~\\ref{fig:concept} shows why a low reading is not enough: a structureless cloud, a star of clusters without depth and a tree with depth cast the same shadow.', 'Figure~\\ref{fig:concept} frames the question: an observer who sees only a shadow, the raw $\\delta$, asks which of three worlds cast it, a structureless cloud, a star of clusters or a tree. All three read alike.'),
         ...
         ("\\item \\textbf{The census.} Clustered structure in most models, hierarchy certified in a few, and a hyperbolic backbone as the control for imposing the geometry.",
          '\\item \\textbf{The census.} Structure beyond a random cloud in most models, clusters oriented toward their hubs in a few, no hub hierarchy in 9 of 12 backbones where an planted one is detected once cluster orientations are randomized, a blind test in the other 3, and a nominally hyperbolic backbone as the control for imposing the geometry.'),
...
DETAIL = {   # appendix reduction (2026-09-24): the paragraphs of the former Appendix C sit in the question they belong to
  ...
  'tab_q04_depth': '\\paragraph{The certified set under other frames.} Only ViT-B and ViT-L are certified under every frame. {{BAL_SENT}} % expR64b_wn30bal_summary.csv, expR77_positive_control.csv, expR83_flat_balanced_summary.csv',
...
NEWCAP = {
 ...
 "tab_q05_power": r"\textbf{The test detects a planted hierarchy where the cloud is quiet enough, and its false alarms are measured.} Panels: planted two-level trees, planted hub alignment, a deep hierarchy at the real noise level, the power per backbone and the false alarms of the balanced frame.",
...
PANELCAP = {
 ...
 ("tab_q05_power", "c"): r"\textbf{Planted hub alignment reproduces the verdict in the supervised ViTs and CLIP-B.} From the decoupled cloud, each cluster's principal axis rotated toward its hub: the detection rate per backbone and seed.",
 ...
 ("tab_q09_corollary", "a"): r"\textbf{The zero-cost gains are modest and limited to prototype tasks.} Per cell: nearest-centroid and few-shot accuracy under the Euclidean (Eucl.) and the Poincar\'{e} (Poinc.) readout, then the advantage of each zero-cost metric in percentage points over the Euclidean readout.",
