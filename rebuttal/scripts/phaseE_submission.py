...
if os.path.exists(R + 'expR85_khrulkov_sup_summary.csv'):
    ...
    F['SUP_CLAUSE'] = "Under their own statistic, the supremum, calibrated against the same replicates, CIFAR-100 joins CIFAR-10 and CUB-200 among the indistinguishable, and only MiniImageNet falls below the random cloud. "; F['PROV85'] = ", expR85_khrulkov_sup_summary.csv"   # author's sentence (2026-09-23, 17:20)
    assert set(_s85.index[_s85.p_left_max > 0.05]) == {'cifar10', 'cifar100', 'cub'} and float(_s85.loc['miniimagenet', 'p_left_max']) <= 0.05, "'CIFAR-100 joins CIFAR-10 and CUB-200 among the indistinguishable, and only MiniImageNet falls below' (largest p over the 10 trials)"
    assert int((_s85.p_left_max > 0.05).sum()) == 3, "S1: '3 under their own statistic'"
    json.dump({"n_trials": 10, "excess_sup_mean": _s85.excess_sup_mean.round(4).to_dict(), "p_left_max": _s85.p_left_max.round(3).to_dict(), "all_negative": bool((_s85.excess_sup_mean < 0).all()), "n_indistinguishable_sup": int((_s85.p_left_max > 0.05).sum()), "r_above_median": _s85.r_above_median.round(1).to_dict()}, open(R + 'final_khrulkov_sup.json', 'w'), indent=1)
else:
