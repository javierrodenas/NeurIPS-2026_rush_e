...
def final_checks():
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "What it certifies is alignment.": (5, 2), "What the test can see.": (16, 4), "Controlling the cut leaves a moderate gap.": (12, 4), "The certified set depends on the frame.": (2, 2)}   # the ceilings per measure beside the cross-model values (thesis brief, 2026-09-22 evening)   # cleanup of 2026-09-22: S5.3 in seven paragraphs, each number once; (c) carries the counts of the test's power and biases
    SENT_EXEMPT = {"What the test can see.", "The count survives resampling.", "Neural collapse is the flat limit, not what the census sees.", "Size and evidence.", "Controlling the cut leaves a moderate gap.", "What a genuine excess means.", "Three readouts are compared on the intact representation.", "The raw reading cannot select a curvature.", "Models share neighborhoods, not metrics.", "Both readings predict the gain, and the depth verdict predicts none."}   # the last two at two sentences for the page budget under the 361-word abstract (consolidated pass)   # cleanup of 2026-09-22: (c) by design, the two S5.2 paragraphs cut to two sentences by the author   # the author's count sentences carry more than one number
    import pandas as pd
    ...
    for name, text in blocks:
        ...
        for par in pars:
            ...
            if vb: continue
            nverb_sents += w
            w_ = [len(s.split()) for s in sents if not s.startswith(("Read correctly, foundation models", "The test misses an implanted two-level tree", "Implanted alignment is detected in", "The decoupled test is therefore biased", "No hierarchy above the superclasses is found in the supervised", "The alignment survives removing the radial component", "The control detects an implanted hierarchy in", "The decoupled control detects an implanted hierarchy in", "The decoupled control detects the same hierarchy", "That frame's matched flat control does not fire once decoupled", "A flat cloud clustered under the frame of record fires under the balanced frame", "It certifies alignment above its frame only", "Figure REF compares the trees of the twelve models", "ViT-B fine-tuned with a hierarchical cross-entropy"))]   # the author's verbatim sentences are exempt from the 35-word rule   # the author's thesis sentence is verbatim and exempt from the 35-word rule
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              and f"The decoupled control detects the same hierarchy, built at each backbone's own spectrum and ratio, in 9 of 12 backbones (decoupled power {S81.loc[cov, 'dec_power'].min():.2f}--{S81.loc[cov, 'dec_power'].max():.2f}) and not in the other 3 (power {S81.loc[unc, 'dec_power'].min():.2f}--{S81.loc[unc, 'dec_power'].max():.2f})." in bf
              ...
              and f"The 3 blind backbones sit at ratios {rb.min():.1f} to {rb.max():.1f}, inside the {rc.min():.1f} to {rc.max():.1f} spanned by the 9 covered backbones, so the power follows the backbone, not its ratio or family." in bf and rc.min() <= rb.min() and rb.max() <= rc.max()
              and "(e) Decoupled power" in pw and all(int(S81.loc[m, "n_seeds"]) == 20 for m in ("dinov2_s", "dinov2_b", "dinov2_l", "dinov2_g")) and f"Under the decoupled control it is {S81.loc[cov, 'dec_power'].min():.2f} or more in 9 of 12 backbones and {S81.loc[unc, 'dec_power'].max():.2f} or less in the other 3, at" in bf)
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 10(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 4; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 9(d)",
        ...
        and f"A flat control with the same spectrum and ratio as the deep one fires in {nf} of {len(flat)} intact and in {fa_dec} of 50 decoupled runs." in bf and f"at {fa_dec} of 50 false alarms on flat clouds. The verdict of no hub hierarchy therefore holds in those 9 only." in bf
        and bal_ok
