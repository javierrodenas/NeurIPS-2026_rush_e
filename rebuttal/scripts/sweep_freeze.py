...
def final_checks():
    ...
    bal_ok = (S83 is not None and bool((S83.n_decoupled == 50).all()) and bool(bal) and int(bal["fa_bal"]) == int(S83.loc["balanced", "decoupled_fired"]) and bal["sentence"] in bf and bal.get("author_sentence") is True
              and bal["sentence"] == f"That frame's matched flat control fires once decoupled in {int(S83.loc['balanced', 'decoupled_fired'])} of 50 runs, so the {int(round(P77.loc['frozen', 'dec_frac_cert_wn30bal'] * 10))} of 10 is not a false alarm: ViT-B shows hub structure under that grouping that the frame of record does not resolve; and a cloud organized under one frame fires under the other even once decoupled ({int(S83.loc['wn30', 'decoupled_fired'])} of 50), so frames are not interchangeable."
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 9(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 3; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 8(d)",
        ...
        and "\\label{fig:implant}" in app_f and "Figure~\\ref{fig:implant} the detection rates" in bf and "Table~\\ref{tab:q5-power} gives the power per backbone." in bf
