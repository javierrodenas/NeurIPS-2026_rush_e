...
def final_checks():
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "What it certifies is alignment.": (5, 2), "What the test can see.": (4, 2), "A trained hierarchy survives, and the decoupled test leans toward firing.": (3, 1), "Controlled, the trees share their topology, not their metric.": (3, 2), "The certified set depends on the frame.": (2, 2), "Training in hyperbolic space leaves the clustering unchanged.": (2, 2), "Agreement with WordNet follows supervision and recipe.": (3, 3)}   # MERU: '95 per cent … within 0.29' (final accuracy pass, 2026-09-23)   # the ceilings per measure beside the cross-model values (thesis brief, 2026-09-22 evening)   # cleanup of 2026-09-22: S5.3 in seven paragraphs, each number once; (c) carries the counts of the test's power and biases
    ...
    chk("final (5th review): confounds stated as a reference level in S1 and S3.2; Proposition 1(b) for fixed n; Khrulkov's rule calibrated with the supremum; S4 210 quadruples; S5.2 flat datasets and 30 of 36; S5.3 WordNet levels and the decoupling control; S6 hierarchy verdict predicts no hyperbolic gain and the calibration paragraph; Figure 2(b) same dataset; no 'OLMo-7B was not extracted'; power table with two-decimal z; Table 14 and S B.12 'Both readings'; Table 6 with 8 causal LMs; AI statement in three items",
        "Proposition~\\ref{prop:bound} makes the dimension confound precise" in bf and "with $n$ fixed" in bf and "with the supremum it was calibrated for" in bf and "210 quadruples" in bf and "FMNIST is genuine in" in TF and "levels" in bf and "decoupling control" in bf
        and "and the hierarchy verdict predicts none. Calibration tells what a low $\\delta$ means, not which readout to use." in bf and "\\paragraph{The raw reading cannot select a curvature.}" in bf and seg(bf, "\\section{Implications for Hyperbolic Representation Learning}", "\\section{Conclusion and Limitations}").count("\\paragraph{") == 1 and "\\textbf{The same reading, opposite verdicts.}" in TF and "OLMo-7B was not extracted" not in bf
    ...
    chk("final (priority 1c as a limitation): the author's sentence with the percentage and the family statement re-derived from expR80_implanted_alignment.csv, Table 10c per backbone and pooled with the rule's outcome, no 'measured power' certification wording, no dotted curve unless the rule was met",
        ...
        and "Planted hub alignment reproduces the verdict in the supervised ViTs and CLIP-B." in pw and "pooled (12 backbones" in pw and "with measured power:" not in bf and "certifies hub alignment with measured power" not in bf
        and bool(f4.get("implanted_alignment_curve", False)) == (d80["rule_power_ge_0_8_fa_le_0_05"] == "True"), f"pct {pct} fam {fam_ok}")
    LEADS53 = ["The hierarchy test certifies that clusters are oriented toward their hubs in 4 of 12 backbones: ViT-S, ViT-B, ViT-L and DINOv2-L.", "What it certifies is alignment.", "What the test can see.", "A trained hierarchy survives, and the decoupled test leans toward firing.", "No hub hierarchy is found where the test has power.", "The certified set depends on the frame.", "Leaf labels can produce the alignment but do not guarantee it.", "Training in hyperbolic space leaves the clustering unchanged."]   # cleanup of 2026-09-22; MERU lead-in since the twelfth review
    ...
    rad_ok = (dr82.model.nunique() == 4 and bool((dr82.z <= -2).all()) and set(l282[(l282.star == "aniso") & (l282.z <= -2)].model) == {"i21k_b", "i21k_l"}
              ...
              and "The alignment is not the radial spread of feature norms, which a control removes without changing the verdicts; full L2 normalization keeps it in 2 of the 4 backbones." in TF and "a control that removes it is in progress" not in bf
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              ...
              and all(int(S81.loc[m, "n_seeds"]) == 20 for m in ("dinov2_s", "dinov2_b", "dinov2_l", "dinov2_g")) and f"Under the decoupled control it is {S81.loc[cov, 'dec_power'].min():.2f} or more in 9 of 12 backbones and {S81.loc[unc, 'dec_power'].max():.2f} or less in the other 3. Its false alarms" in TF)
    ...
    pc_ok = (P77.loc["hier_seed0", "z_wn30"] <= -2 and P77.loc["hier_seed0", "z_wn30bal"] <= -2 and P77.loc["ce_seed0", "z_wn30"] <= -2 and P77.loc["frozen", "z_wn30"] <= -2 and not V77["criterion_met"]
             ...
             and "A pre-set criterion expecting no intact certification of the leaf and frozen models was not met, because they are aligned." in TF and "The pre-set criterion asked" not in bf and "The trained control separates the objectives by degree." in dep and f"Its trained control separates the objectives by degree, not absolutely: the leaf-only fine-tune also fires once decoupled, in {sum(int(round(P77.loc[f'ce_seed{s_}', 'dec_frac_cert_wn30'] * 10)) for s_ in (0, 1))} of 20 runs." in TF and "(seed 1)" not in bf and "2 seeds:" in dep
    ...
    bal_ok = (S83 is not None and bool((S83.n_decoupled == 50).all()) and bool(bal) and int(bal["fa_bal"]) == int(S83.loc["balanced", "decoupled_fired"]) and bal["sentence"] in TF and bal.get("author_sentence")
              ...
              and f"On the balanced frame the matched flat control fires in {int(S83.loc['balanced', 'decoupled_fired'])} of 50 decoupled runs." in TF and int(S83.loc["balanced", "decoupled_fired"]) == 0
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 10(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 4; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 9(d)",
        p81_ok and bf.count("no hub hierarchy is found in 9 of 12 backbones where the decoupled control detects an planted one, and the test is blind in the other 3") == 0 and "\\paragraph{No hub hierarchy is found where the test has power.}" in bf
        ...
        and f"A flat control with the same spectrum and ratio as the deep one fires in {nf} of {len(flat)} intact and in {fa_dec} of 50 decoupled runs." in bf and f"Its false alarms on flat clouds are {fa_dec} of 50, and the verdict of no hub hierarchy therefore holds in those 9 only." in TF
        and bal_ok
        and "measures structure above the clusters of the frame only" in bf and "measures hierarchy above" not in bf and "\\paragraph{Leaf labels can produce the alignment but do not guarantee it.}" in bf and "\\paragraph{Training in hyperbolic space leaves the clustering unchanged.}" in bf and "detected depth" not in bf and "a hyperbolic-trained backbone as the control for imposing the geometry" in bf
    ...
    s53d = s53 + nocom(seg(TF, "\\section{Detail from the main text}", "\\renewcommand{\\topfraction}"))   # page-9 recovery of 2026-09-24: the detail of S5.3 moved to Appendix~\ref{app:detail}; each number still appears once between the two
    ...
    chk("final (cleanup, 2026-09-22): S3.3 duplicates out ('Size and evidence', 'What a genuine excess means'); Definition 8 (decoupling control) after the matched star with the author's wording and the power/false-alarm sentence; S4 lead-in and S5.2 sentence gone, the two S5.2 paragraphs at two sentences; S3.2 supremum sentence gone (citation on Definition 1); S5.3 in the seven paragraphs in order with each count once and the bias sentence once; the Khrulkov table in S5.1 with its cells from expR78, cited by the paragraph",
        ...
        and "The power of the test is the fraction of planted hierarchies it detects, and its false-alarm rate is the fraction of flat controls that fire. Both are measured on real clouds in Section~\\ref{sec:f-depth}." in s3_ and "the false-alarm rate and power are measured on real clouds" not in bf
        ...
        and kt_ok and "\\input{tab_khrulkov_final}" not in bf and "\\input{tab_khrulkov_final}" in TF[TF.index("\\appendix"):] and "Figure~\\ref{fig:premise}b shows the 4 datasets and Table~\\ref{tab:khrulkov} gives the rows" in bf and bf.index("\\label{fig:premise}") < bf.index("\\paragraph{A published reading is reproduced and calibrated.}") < bf.index("\\subsection{Structure beyond the second moments at the class level}"),
    ...
    chk("final (10th review): the balanced frame read as frame mismatch (author's sentence, counts from expR83/expR77) and the matched count kept in (c); the alignment stated as relational, not radial, reproduced by the planted principal-axis alignment in the supervised ViTs and CLIP-B and not in DINOv2-L, form unresolved (S5.3 and (iii)); the decoupling's separation and 'no hub hierarchy' = none among the frame's hubs (S5.3 and (iii)); 'hubs of the frame' in the abstract, S1 and the captions; the within-model ceiling from expR84 beside 0.77/0.76 with the keep branch (ceiling >= 0.9), Table 11(c); limitation (vi) on analysis choices, scope renumbered; Table 14(b) restored",
        bal_ok and "The planted principal-axis alignment reproduces it in the supervised ViTs and CLIP-B but not in DINOv2-L, and its geometric form is not resolved here." in bf and fam_ok
        ...
        and "Its exact form is not resolved here." in TF
    ...
    chk("final (consolidated pass): the thesis is the abstract's last sentence and S7's; S1 (rewrite of 2026-09-23) glosses the hub at first use, the frame gloss sits at its first use in S3.4 (Definition 6), P5 has its three labelled parts, P1/P3/P4 as briefed; S1 and contribution 3 mirror the abstract's sharing sentence; S5.4 opens by citing Figure 5 with the author's sentence; Figure 5 caption and the appendix matrices; Definition 8 and the frame-mismatch reading split at 45 words",
        ...
        and (TEX/"figures/fig_treemap_matrices_final.pdf").exists() and "mean_ari_other" in v5 and "ceiling_band" in v5 and "The frozen ViT-B firing in" in TF and "A verdict that survives the decoupling is carried by the arrangement of the hubs; one that does not" in bf,
    ...
    chk("final (full read, 2026-09-24): 'planted' everywhere, no 'implanted' left in the text, the appendix titles, the tables or the figure captions (file names and labels keep their spelling); the abstract adds the hierarchy test; S4 gives each class set its count in parentheses; the depth equation reads on into the 'where' clause; the statement boxes are unbreakable; the Ethics statement drops the metric-selection heuristic; Figure 3(b) names the supremum in the caption",
        ...
        and "two of four are indistinguishable from it, on their statistic, the supremum; filled and hollow dots follow the reading, which gives the same verdict." in bf
    ...
    chk("final (main text without tables, 2026-09-24): no table environment or table input left in the main text; the census and the Khrulkov tables inputted in the appendix with every column; Figure 3 in S5.1 with the author's caption, its 24 sample-level cells from expR62 and the four published datasets from expR78/expR85 (10 of 24 genuine; CIFAR-10 and CUB-200 indistinguishable); S5.1 and the S1 mirror point at it; Figure 2(a) retitled",
        ...
        and all(x in TF[TF.index("\\appendix"):] for x in ("\\input{tab_census_final}", "\\input{tab_khrulkov_final}"))
        and "\\includegraphics[width=\\linewidth]{figures/fig_premise_final.pdf}" in bf and (TEX/"figures/fig_premise_final.pdf").exists()
        and "\\caption{\\textbf{The premise where it is read.} (a) Per-image features: raw reading and a random cloud of the same shape; filled dots are genuine. (b) The published values, their reproduction and the random cloud: two of four are indistinguishable from it, on their statistic, the supremum; filled and hollow dots follow the reading, which gives the same verdict." in bf
        and _prem["sample_genuine"] == int(_sl.genuine_bh.sum()) == 10 and _prem["indistinguishable"] == sorted(S78.index[S78.p_left_max > 0.05]) == ["cifar10", "cub"]
        ...
        and "Figure~\\ref{fig:premise}a shows the 24 cells and Table~\\ref{tab:q3-sample} gives the reading per cell." in bf
        and "2 of them are indistinguishable from a random cloud (Figure~\\ref{fig:premise})." in bf,
        f"sample-level genuine {int(_sl.genuine_bh.sum())}/24; indistinguishable {_prem['indistinguishable']}")
    ...
    chk("final (12th review, item 5): Table 1 carries the excess and rank of the published statistic itself, the supremum, calibrated on the same 200 replicates over 10 trials per dataset (cells from expR85_khrulkov_sup_summary.csv), and S5.1 says so; while the run is not merged, the fallback clause of the brief",
        (all(f"${S85.loc[d, 'excess_sup_mean']:+.4f}$ & {int(round(S85.loc[d, 'r_above_median']))}/200, {S85.loc[d, 'p_left_min']:.3f}--{S85.loc[d, 'p_left_max']:.3f}" in kt85 for d in S85.index) and bool((S85.n_trials == 10).all()) and "(sup.)" in kt85 and "expR85_khrulkov_sup_summary.csv" in kt85
         and "fall below it. Under their own statistic, the supremum, CIFAR-10 and CUB-200 stay indistinguishable from a random cloud and MiniImageNet stays below it. The verdict for CIFAR-100 changes from trial to trial, as the statistic confound predicts. Figure~\\ref{fig:premise}b shows the 4 datasets and Table~\\ref{tab:khrulkov} gives the rows." in bf
         ...
         and "under their own statistic" not in s1_ and "2 of them are indistinguishable from a random cloud (Figure~\\ref{fig:premise}). (ii)" in s1_
        ...
        else ("The calibration reads the percentile statistic $\\hat\\delta_{99.9}$, whereas their statistic is the supremum. Figure~\\ref{fig:premise}b shows the 4 datasets and Table~\\ref{tab:khrulkov} gives the rows." in bf and "(sup.)" not in kt85),
    ...
    chk("final (readability 2): Definition 3 opens with the plain sentence; S3.2 confounds paragraph in one sentence; S3.3 'Two cells of the same dataset can have the same reading and opposite verdicts'; S3.4 opens with the author's two sentences before Definition 6; the three verbatim S2/S3 sentences split by recorded edits; no sentence over 30 words in S3-S7 outside definitions, citation lists and the thesis",
        ...
        and "Two cells of the same dataset can have the same reading and opposite verdicts (Figure~\\ref{fig:samereading})." in bf and bf.index("\\paragraph{Why a second test.} A genuine excess can come from clusters alone, so a second test asks whether the clusters are themselves arranged hierarchically. It needs three constructions.") < bf.index("\\begin{definition}[hub null and hub excess]")
    ...
    chk("final: the census table is the centered Haar record with the short caption (tab_census_final from expR75, twelve rows, same layout as v1's tab_census), inputted in the appendix since the main text carries no table (2026-09-24)", "\\input{tab_census_final}" not in bf and "\\input{tab_census_final}" in TF[TF.index("\\appendix"):] and "expR75_census_centered_haar.csv" in tcf and tcf.count("\\\\") >= 13 and tcf.split("\\midrule")[0] == open(TEX/"tab_census.tex").read().split("\\midrule")[0])
    # ---- statements
