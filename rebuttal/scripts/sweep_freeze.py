...
def final_checks():
    ...
    if (R/"expR84_tree_ceiling_summary.csv").exists():   # thesis brief (2026-09-22 evening): the ceilings per measure, their ranges, and the cross-model values of expR58
        ...
        HEADLINE |= {f"{_c58h.triplet_agree_big_vs_block:.2f}", f"{_c58h.triplet_agree_within_block:.2f}", f"{_c58h.coph_corr_big_vs_block:.2f} and {_c58h.coph_corr_within_block:.2f}", f"{_c58h.ari_cut_big_vs_block:.2f} and {_c58h.ari_cut_within_block:.2f}"}
    ...
    LEADS53 = ["The depth test certifies that clusters are oriented toward their hubs in four backbones.", "What it certifies is alignment.", "What the test can see.", "No hub hierarchy is found where the test has power.", "The certified set depends on the frame.", "Leaf labels can produce the alignment but do not guarantee it.", "MERU's objective does not create hub structure."]   # cleanup of 2026-09-22; MERU lead-in since the twelfth review
    ...
    rad_ok = (dr82.model.nunique() == 4 and bool((dr82.z <= -2).all()) and set(l282[(l282.star == "aniso") & (l282.z <= -2)].model) == {"i21k_b", "i21k_l"}
              ...
              and "The alignment is not the radial spread of feature norms, which a control removes without changing the verdicts; full L2 normalization keeps it in two of the four backbones." in bf and "a control that removes it is in progress" not in bf
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              and f"The decoupled control detects the same hierarchy, built at each backbone's own spectrum and ratio, in nine backbones (decoupled power {S81.loc[cov, 'dec_power'].min():.2f}--{S81.loc[cov, 'dec_power'].max():.2f}) and not in the other three (power {S81.loc[unc, 'dec_power'].min():.2f}--{S81.loc[unc, 'dec_power'].max():.2f})." in bf
              ...
              and f"The three blind backbones sit at ratios {rb.min():.1f} to {rb.max():.1f}, inside the {rc.min():.1f} to {rc.max():.1f} of the nine covered, so the power follows the backbone, not its ratio or family." in bf and rc.min() <= rb.min() and rb.max() <= rc.max()
              and "(e) Decoupled power" in pw and all(int(S81.loc[m, "n_seeds"]) == 20 for m in ("dinov2_s", "dinov2_b", "dinov2_l", "dinov2_g")) and f"Under the decoupled control it is {S81.loc[cov, 'dec_power'].min():.2f} or more in nine backbones and {S81.loc[unc, 'dec_power'].max():.2f} or less in the other three, at" in bf)
    ...
    pc_ok = (P77.loc["hier_seed0", "z_wn30"] <= -2 and P77.loc["hier_seed0", "z_wn30bal"] <= -2 and P77.loc["ce_seed0", "z_wn30"] <= -2 and P77.loc["frozen", "z_wn30"] <= -2 and not V77["criterion_met"]
             and f"ViT-B fine-tuned with a hierarchical cross-entropy keeps firing once decoupled in {sum(int(round(P77.loc[f'hier_seed{s_}', 'dec_frac_cert_wn30'] * 10)) for s_ in (0, 1))} of 20 runs over two seeds, mean $z$ ${P77.loc[['hier_seed0', 'hier_seed1'], 'zdec_mean_wn30'].mean():.2f}$, the leaf-only fine-tune in {sum(int(round(P77.loc[f'ce_seed{s_}', 'dec_frac_cert_wn30'] * 10)) for s_ in (0, 1))} of 20, mean $z$ ${P77.loc[['ce_seed0', 'ce_seed1'], 'zdec_mean_wn30'].mean():.2f}$, and the frozen checkpoint in none, on the frame of record." in bf and int(round(P77.loc['frozen', 'dec_frac_cert_wn30'] * 10)) == 0 and "Fine-tuning itself adds some decoupled signal and the hierarchical objective adds more, so the trained control separates the objectives by degree." in bf and not any(v["criterion_met"] for v in _j.load(open(R/"expR77_positive_control_verdict.json"))["verdict"])
             and "The pre-set criterion asked the leaf-CE and frozen models not to be certified intact, which they are, by alignment, so the discriminating comparison is the decoupled one." in bf and "(d) A trained positive control" in dep and f"Its trained control separates the objectives by degree, not absolutely: the leaf-only fine-tune also fires once decoupled, in {sum(int(round(P77.loc[f'ce_seed{s_}', 'dec_frac_cert_wn30'] * 10)) for s_ in (0, 1))} of 20 runs." in bf and "(seed 1)" not in bf and "two seeds, identical batches" in dep
    ...
    chk("final (7th review): headline scoped in the abstract (twice), S1, contribution 2, S5.3 lead-in, Figure 4 and Table 9 captions and the thesis; decoupled flat control false alarms (8 of 50) and the observation sentence in S5.3 from the files; limitation (iii) with ViT-L's ratio 1.9 and the DINOv2 range from expR64b; Figure 1 caption 'read alike, all low'; no unproved extension in the proof of Proposition 1(b)",
        "The control covers the supervised" not in bf and "whose noise level, ratios" not in bf and p81_ok and pc_ok and bf.count("no hub hierarchy is found in the nine backbones where the decoupled control detects an implanted one, and the test is blind in the other three") == 1 and "Where a planted hierarchy is detected, in nine backbones, none as strong as the planted one is found; in the other three the test cannot tell." in abs_now and "noise level at which the test was validated" not in bf and "\\paragraph{No hub hierarchy is found where the test has power.}" in bf and " in the backbones whose noise level the control covers" not in TF and f"the synthetic hierarchy at $z$ ${deep.zdec_mean.max():.2f}$ to ${deep.zdec_mean.min():.2f}$ stands well clear of that bias." in bf and f"none of the four fires, and ViT-L reads ${float(next(r for r in load('expR74_decoupling_summary.csv') if r['model'] == 'i21k_l')['dec_z_mean']):.2f}$, within the ${flat.zdec_mean.max():.2f}$ to ${flat.zdec_mean.min():.2f}$ of the flat control: without its orientations the real cloud reads like a flat one." in bf and "lie at the edge of the covered range" not in bf and rad_ok and "On class centroids there is structure that a random cloud does not have, in 44 of 72 cells and 30 of 36" in bf and bf.index("The noise level of a cloud is its within-cluster spread relative to the distance between its hubs.") < bf.index("at this noise level but detects") and "unregime" not in TF and bf.count("four values reported by") == 1 and "two of them are indistinguishable from a random cloud" in bf and int((S78.p_left_max > 0.05).sum()) == 2 and not any(w_ in bf[bf.index("\\textbf{The answer has three parts.}"):bf.index("\\textbf{The answer has three parts.}") + 260].lower() for w_ in ("genuine", "certified", "record", "matched star", "null")) and "The decoupled test is therefore biased toward firing, so the four real backbones not firing once decoupled is conservative evidence" in bf and "its decoupled form is biased toward firing" in bf and "where a deep synthetic one is detected" not in bf
    ...
    bal_ok = (S83 is not None and bool((S83.n_decoupled == 50).all()) and bool(bal) and int(bal["fa_bal"]) == int(S83.loc["balanced", "decoupled_fired"]) and bal["sentence"] in bf and bal.get("author_sentence")
              and bal["sentence"] == f"A flat cloud clustered under the frame of record fires under the balanced frame in {int(S83.loc['wn30', 'decoupled_fired'])} of 50 decoupled runs, so a real cloud read under a frame that is not its own is expected to fire. The frozen ViT-B firing in {int(round(P77.loc['frozen', 'dec_frac_cert_wn30bal'] * 10))} of 10 there is consistent with that mismatch and is not evidence of hub structure."
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 10(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 4; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 9(d)",
        p81_ok and bf.count("no hub hierarchy is found in the nine backbones where the decoupled control detects an implanted one, and the test is blind in the other three") == 1 and "\\paragraph{No hub hierarchy is found where the test has power.}" in bf
        and "Where a planted hierarchy is detected, in nine backbones, none as strong as the planted one is found; in the other three the test cannot tell." in abs_now and "five backbones" not in TF and "other seven" not in TF and "in the five only" not in TF and "in the five and" not in TF
        and f"A flat control with the same spectrum and ratio as the deep one fires in {nf} of {len(flat)} intact and in {fa_dec} of 50 decoupled runs." in bf and f"at {fa_dec} of 50 false alarms on flat clouds. The verdict of no hub hierarchy therefore holds in the nine only." in bf
        and bal_ok
        ...
        and f"against {c58.triplet_agree_big_vs_block:.2f} for DINOv2 against the block and {c58.triplet_agree_within_block:.2f} within it." in bf and "About a third of the within-block agreement is missing under the admissible configurations as a whole." in bf
    ...
    chk("final (cleanup, 2026-09-22): S3.3 duplicates out ('Size and evidence', 'What a genuine excess means'); Definition 8 (decoupling control) after the matched star with the author's wording and the power/false-alarm sentence; S4 lead-in and S5.2 sentence gone, the two S5.2 paragraphs at two sentences; S3.2 supremum sentence gone (citation on Definition 1); S5.3 in the seven paragraphs in order with each count once and the bias sentence once; the Khrulkov table in S5.1 with its cells from expR78, cited by the paragraph",
        ...
        and "The power of the test is the fraction of implanted hierarchies it detects and its false-alarm rate the fraction of flat controls that fire; both are measured on real clouds in Section~\\ref{sec:f-depth}." in s3_ and "the false-alarm rate and power are measured on real clouds" not in bf
        and "Every reading is calibrated against two hundred replicates" not in bf and "\\paragraph{Calibration budget and frames.} Each cell is read against two hundred Haar replicates" in bf and bf.count("two hundred") == 1
        ...
        and kt_ok and "\\input{tab_khrulkov_final}" in bf and "Table~\\ref{tab:khrulkov} gives the four rows." in bf and bf.index("\\paragraph{A published reading is reproduced and calibrated.}") < bf.index("\\input{tab_khrulkov_final}") < bf.index("\\subsection{Structure beyond the second moments at the class level}"),
    ...
    chk("final (10th review): the balanced frame read as frame mismatch (author's sentence, counts from expR83/expR77) and the matched count kept in (c); the alignment stated as relational, not radial, reproduced by the principal-axis implant in the supervised ViTs and CLIP-B and not in DINOv2-L, form unresolved (S5.3 and (iii)); the decoupling's separation and 'no hub hierarchy' = none among the frame's hubs (S5.3 and (iii)); 'hubs of the frame' in the abstract, S1 and the captions; the within-model ceiling from expR84 beside 0.77/0.76 with the keep branch (ceiling >= 0.9), Table 11(c); limitation (vi) on analysis choices, scope renumbered; Table 14(b) restored",
        ...
        and "The alignment is relational, needing both the real hubs and the orientation of each cluster relative to them: randomizing either removes it, 0 of 60 with random hubs and 0 of 4 with random orientations." in bf
        and "Its exact form is not resolved here." in bf
        and "The decoupling separates structure among the hubs of the frame from structure carried by cluster orientations. A hierarchy below the frame expressed in how clusters open would be removed with the orientations and counted as alignment, so no hub hierarchy means no hierarchy among the frame's hubs." in bf
    ...
    chk("final (thesis after expR84): the new thesis twice, the abstract's sharing sentence and its S1 mirror with 0.77 and the triplet ceiling range from the files, contribution 3 mirrored, S5.4 with the three ceilings (range and mean) beside the cross-model values and the reading (topology within most of the ceiling, metric at about half)",
        ...
        and f"The cophenetic correlation reaches {b84.coph_corr_mean.min():.2f} to {b84.coph_corr_mean.max():.2f} within a model, mean {S84.loc['ALL', 'coph_corr_mean']:.2f}, against {c58.coph_corr_big_vs_block:.2f} and {c58.coph_corr_within_block:.2f} across models." in bf
        and f"The ARI at the cut reaches {b84.ari_cut_mean.min():.2f} to {b84.ari_cut_mean.max():.2f}, mean {S84.loc['ALL', 'ari_cut_mean']:.2f}, against {c58.ari_cut_big_vs_block:.2f} and {c58.ari_cut_within_block:.2f}." in bf
