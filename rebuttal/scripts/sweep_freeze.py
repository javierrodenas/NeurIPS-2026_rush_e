...
def final_checks():
    ...
    THESIS = "Read correctly, foundation models organize classes into clusters, agree on how those clusters nest almost as much as noise allows but not on the distances between them, show no hierarchy among the superclasses where the test can see one, and their raw tree-likeness is not evidence for hyperbolic geometry."   # thesis after expR84 (2026-09-22, evening): topology largely shared, metric not
    ...
    if "hier_seed1" in _p77h.index: HEADLINE |= {f"{int(round(_p77h.loc['ce_seed1', 'dec_frac_cert_wn30'] * 10))} of 10", f"{int(round(_p77h.loc['ce_seed0', 'dec_frac_cert_wn30'] * 10))} of 10 and {int(round(_p77h.loc['ce_seed1', 'dec_frac_cert_wn30'] * 10))} of 10", f"{_p77h.loc['hier_seed0', 'zdec_mean_wn30']:.2f} and {_p77h.loc['hier_seed1', 'zdec_mean_wn30']:.2f}", f"{_p77h.loc['ce_seed0', 'zdec_mean_wn30']:.2f} and {_p77h.loc['ce_seed1', 'zdec_mean_wn30']:.2f}"}   # seed 1 of the trained control (2026-09-23)
    ...
    chk("final (author's decisions, 2026-09-20): the record is the centered Haar null (44 of 72; five constructions in the census table with the uncentered census as a column; Table 2, Figure 3 and the joint sensitivity from it); the decoupling result rewrites the depth claim with the author's wording (abstract, S1, contribution 2, thesis, S5.3, MERU, Figure 4 caption); 'certified' only for the decoupling-tested structure",
        ...
        and "certifies clustering" not in TF and "validated regime" not in TF and "validated range" not in TF and "The census does not certify that frame" in bf and "The alignment is relational" in bf and "organize classes into clusters, agree on how those clusters nest almost as much as noise allows" in bf
    ...
    LEADS53 = ["The depth test certifies that clusters are oriented toward their hubs in four backbones.", "What it certifies is alignment.", "What the test can see.", "No hub hierarchy is found where the test has power.", "The certified set depends on the frame.", "Leaf labels can produce the alignment but do not guarantee it.", "Imposing the geometry does not create hub structure."]   # cleanup of 2026-09-22
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              ...
              and "The power is the decoupled control's: it detects an implanted hierarchy in ViT-S/B/L, DINOv2-B/L/G, CLIP-B/L and SigLIP-B, and the test is blind in ViT-T, DINO-B and DINOv2-S." in bf
    ...
    pc_ok = (P77.loc["hier_seed0", "z_wn30"] <= -2 and P77.loc["hier_seed0", "z_wn30bal"] <= -2 and P77.loc["ce_seed0", "z_wn30"] <= -2 and P77.loc["frozen", "z_wn30"] <= -2 and not V77["criterion_met"]
             and f"ViT-B fine-tuned with a hierarchical cross-entropy at three WordNet cuts keeps firing once decoupled in {int(round(P77.loc['hier_seed0', 'dec_frac_cert_wn30'] * 10))} of 10 runs in both seeds, $z$ ${P77.loc['hier_seed0', 'zdec_mean_wn30']:.2f}$ and ${P77.loc['hier_seed1', 'zdec_mean_wn30']:.2f}$. The leaf-only fine-tune fires in {int(round(P77.loc['ce_seed0', 'dec_frac_cert_wn30'] * 10))} of 10 in one seed and {int(round(P77.loc['ce_seed1', 'dec_frac_cert_wn30'] * 10))} of 10 in the other, $z$ ${P77.loc['ce_seed0', 'zdec_mean_wn30']:.2f}$ and ${P77.loc['ce_seed1', 'zdec_mean_wn30']:.2f}$, and the frozen checkpoint in {int(round(P77.loc['frozen', 'dec_frac_cert_wn30'] * 10))} of 10, on the frame of record." in bf and int(round(P77.loc['hier_seed1', 'dec_frac_cert_wn30'] * 10)) == 10 and "Fine-tuning itself adds some decoupled signal in one seed and the hierarchical objective adds more, so the trained control separates the objectives by degree." in bf and not any(v["criterion_met"] for v in _j.load(open(R/"expR77_positive_control_verdict.json"))["verdict"]) and f"frozen ViT-B firing in {int(round(P77.loc['frozen', 'dec_frac_cert_wn30bal'] * 10))} of 10 there is consistent with that mismatch" in bf
             and "The pre-set criterion asked the leaf-CE and frozen models not to be certified intact, which they are, by alignment, so the discriminating comparison is the decoupled one." in bf and "(d) A trained positive control" in dep and "Its trained control separates the objectives by degree, not absolutely: in one of two seeds the leaf-only fine-tune also fires once decoupled." in bf and "(seed 1)" not in bf and "two seeds, identical batches" in dep
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 10(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 4; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 9(d)",
        ...
        and "Where a planted hierarchy is detected, in nine backbones, none is found in the real model; in the other three the test cannot tell." in abs_now and "five backbones" not in TF and "other seven" not in TF and "in the five only" not in TF and "in the five and" not in TF
        and f"A flat control with the same spectrum and ratio as the deep one fires in {nf} of {len(flat)} intact and in {fa_dec} of 50 decoupled runs." in bf and f"The verdict of no hub hierarchy therefore holds in the nine only, at {fa_dec} of 50 false alarms on flat clouds." in bf
        and bal_ok
    ...
    once = {k: len(_re.findall(r"(?<!\d)" + _re.escape(k) + r"(?!\d)", s53)) for k in ("4 of 5", "8 of 50", "0 of 50", "38 of 50", "7 of 10", "10 of 10", "0 of 10", "0 of 60", "0 of 4", "0 of 5")}
    ...
    chk("final (cleanup, 2026-09-22): S3.3 duplicates out ('Size and evidence', 'What a genuine excess means'); Definition 8 (decoupling control) after the matched star with the author's wording and the power/false-alarm sentence; S4 lead-in and S5.2 sentence gone, the two S5.2 paragraphs at two sentences; S3.2 supremum sentence gone (citation on Definition 1); S5.3 in the seven paragraphs in order with each count once and the bias sentence once; the Khrulkov table in S5.1 with its cells from expR78, cited by the paragraph",
        ...
        and all(v == (2 if k in ("0 of 10",) else 1) for k, v in once.items()) and len(_re.findall(r"(?<!\d)6 of 10(?!\d)", s53)) == 1 and s53.count("biased toward firing") == 1 and len(_re.findall(r"\\paragraph\{", s53)) == 7 and all(("\\paragraph{" + l + "}") in s53 for l in LEADS53)
    ...
    chk("final (10th review): the balanced frame read as frame mismatch (author's sentence, counts from expR83/expR77) and the matched count kept in (c); the alignment stated as relational, not radial, reproduced by the principal-axis implant in the supervised ViTs and CLIP-B and not in DINOv2-L, form unresolved (S5.3 and (iii)); the decoupling's separation and 'no hub hierarchy' = none among the frame's hubs (S5.3 and (iii)); 'hubs of the frame' in the abstract, S1 and the captions; the within-model ceiling from expR84 beside 0.77/0.76 with the keep branch (ceiling >= 0.9), Table 11(c); limitation (vi) on analysis choices, scope renumbered; Table 14(b) restored",
        bal_ok and "The implanted principal-axis alignment reproduces the verdict in the supervised ViTs and CLIP-B but not in DINOv2-L, and its exact geometric form is not resolved here." in bf and fam_ok
        ...
        and "The alignment is thus relational, not radial, and reproduced by a principal-axis implant in the supervised ViTs and CLIP-B but not in DINOv2-L; its exact geometric form is not resolved here." in bf
        ...
        and "It certifies alignment above its frame only: a hierarchy below the frame expressed in how clusters open would be removed with the orientations and counted as alignment, so no hub hierarchy means none among the frame's hubs." in bf
        ...
        and "(vi)~The analysis rests on many choices, frames, stars and null variants among them, and a single pre-specified analysis is future work." in bf and "(vii)~The census" in bf and "(viii)~The objective--geometry" in bf and "(ix)~Downstream" in bf
    ...
    chk("final (thesis after expR84): the new thesis twice, the abstract's sharing sentence and its S1 mirror with 0.77 and the triplet ceiling range from the files, contribution 3 mirrored, S5.4 with the three ceilings (range and mean) beside the cross-model values and the reading (topology within most of the ceiling, metric at about half)",
        ...
        and "The tree topology is therefore shared to within most of the ceiling and the metric agrees at about half of it." in bf and "the island shrinks to a gap beyond resampling noise" in bf
