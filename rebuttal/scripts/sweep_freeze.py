...
def final_checks():
    ...
    chk("final: the recorded edits are exactly the briefs' (shadow x2, geometric face, intent of the supremum, the bridge; 4th/5th reviews: abstract, S1 confounds and counts, S2 citations) and none of the old phrases survives",
        len(ED) == 17 and all((a not in bf) or (a in b) for a, b in ED) and any("clusters oriented toward their hubs in a few" in b for _, b in ED) and any("no hub hierarchy is found in 9 of 12 backbones where an implanted one is detected once cluster orientations are randomized" in b for _, b in ED) and "49 of 72" not in bf and all((fillb(b) in bf) for _, b in ED if b) and sum(1 for a, _ in ED if "shadow" in a) == 2 and any("geometric face" in a for a, _ in ED) and any("intent of the supremum" in a for a, _ in ED) and any("next question" in a for a, _ in ED)
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "What it certifies is alignment.": (5, 2), "What the test can see.": (18, 4), "Controlling the cut leaves a moderate gap.": (12, 4), "The certified set depends on the frame.": (2, 2)}   # the ceilings per measure beside the cross-model values (thesis brief, 2026-09-22 evening)   # cleanup of 2026-09-22: S5.3 in seven paragraphs, each number once; (c) carries the counts of the test's power and biases
    SENT_EXEMPT = {"What the test can see.", "The count survives resampling.", "Neural collapse is the flat limit, not what the census sees.", "Size and evidence.", "Controlling the cut leaves a moderate gap.", "What a genuine excess means.", "Three readouts are compared on the intact representation.", "The raw reading cannot select a curvature.", "Models share neighborhoods, not metrics.", "Both readings predict the gain, and the depth verdict predicts none.", "A raw value cannot be called low on its own.", "Why a second test.", "A star of clusters already passes the census."}   # the last two at two sentences for the page budget under the 361-word abstract (consolidated pass)   # cleanup of 2026-09-22: (c) by design, the two S5.2 paragraphs cut to two sentences by the author   # the author's count sentences carry more than one number
    import pandas as pd
    ...
    for name, text in blocks:
        ...
        for par in pars:
            ...
            sents_nolead = sent_split(_re.sub(r"\\paragraph\{[^}]*\}", "", cp)); over30 = [] if name == "Introduction" else [(len(s_.split()), s_) for k_, s_ in enumerate(sents_nolead) if len(s_.split()) > 30 and not s_.startswith("Read correctly, foundation models") and not (len(iscl) == len(sents_nolead) and iscl[k_])]   # pages 3-9: S2's page-3 part onward
            ...
            counts_ = set(m.group(1) for m in _re.finditer(r"(" + GROUP + r"(?: of (?:the )?" + GROUP + r")?)\s+(?:the )?(?:[A-Z][\w-]*\s+)?" + NOUNS, cp)) | set(m.group(1) for m in _re.finditer(r"\bother (\d+)\b", cp)) | set(m.group(1) for m in _re.finditer(r"\bthose (\d+) only\b", cp)) | set(m.group(1) for m in _re.finditer(r"\bnone of the (\d+) backbones\b", cp))   # counts of models, cells, seeds and runs are numerals with their noun (author's rule, 2026-09-23): not result numbers
            if results:
    ...
    voc = {"excess": "\\begin{definition}[excess]", "genuine": "\\begin{definition}[genuine]", "reading of record": "\\begin{definition}[reading of record]", "matched star": "\\begin{definition}[matched star]", "hub null": "\\begin{definition}[hub null", "Haar null": "\\begin{definition}[Haar null]", "four-point defect": "\\begin{definition}[four-point defect]", "decoupling control": "\\begin{definition}[decoupling control]"}
    ...
    chk("final: the appendix ARI-matrix figure states the naive and the selected DINOv2-vs-block agreement read from the figure's data, and Figure 5 carries the author's caption (consolidated pass)", bool(v5) and f"where the DINOv2 block agrees with the rest at {v5['naive_dinov2_vs_block']:.2f}" in TF[TF.index("\\appendix"):] and f"where it agrees at {v5['selected_dinov2_vs_block']:.2f}" in TF[TF.index("\\appendix"):] and "\\label{fig:treemapmat}" in TF[TF.index("\\appendix"):] and "\\textbf{The island is the cut; tree topology is shared almost up to noise; the self-supervised structure is angular.} (a) Mean agreement of each model's tree with the other 11, naive (hollow) and corrected (filled). (b) Triplet agreement under the corrected comparison against the within-model ceiling (band). (c) Sibling-triplet agreement under cosine and Euclidean distance." in bf)
    ...
    chk("final: xi, ORC/interventions table, appendix figures, null-variant panel and the class-count sweep are gone from the final and nothing refers to them",
        all(g not in inputs for g in gone) and all(("\\ref{" + l + "}") not in TF for l in gone_lab) and app_f.count("\\includegraphics") == 2 and "figures/fig_implant_final.pdf" in app_f and "figures/fig_treemap_matrices_final.pdf" in app_f and "Ollivier" not in app_f
    ...
    for s in kept:
        ...
        if s in ("tab_q01_census",): ok = set(nb) <= set(na) and len(nb) < len(na)      # one panel dropped by the brief (null-variant panel, uncited supremum table)
    ...
    chk("final (5th review): confounds stated as a reference level in S1 and S3.2; Proposition 1(b) for fixed n; Khrulkov's rule calibrated with the supremum; S4 210 quadruples; S5.2 flat datasets and 30 of 36; S5.3 WordNet levels and the decoupling control; S6 depth verdict predicts no hyperbolic gain and the calibration paragraph; Figure 2(b) same dataset; no 'OLMo-7B was not extracted'; power table with two-decimal z; Table 14 and S B.12 'Both readings'; Table 6 with 8 causal LMs; AI statement in three items",
        "Proposition~\\ref{prop:bound} makes the dimension confound precise" in bf and "with $n$ fixed" in bf and "calibrated with the supremum" in bf and "210 quadruples" in bf and "FMNIST is genuine in" in bf and "levels" in bf and "decoupling control" in bf
    ...
    chk("final (author's decisions, 2026-09-20): the record is the centered Haar null (44 of 72; five constructions in the census table with the uncentered census as a column; Table 2, Figure 3 and the joint sensitivity from it); the decoupling result rewrites the depth claim with the author's wording (abstract, S1, contribution 2, thesis, S5.3, MERU, Figure 4 caption); 'certified' only for the decoupling-tested structure",
        ...
        and "Given an implanted two-level tree at this noise level, the test misses it" in bf and bf.count("That detection survives orientation randomization, whereas none of the 4 real verdicts does.") == 1 and "whereas an implanted hierarchy, synthetic or trained in, survives it; no hub hierarchy is found in 9 of 12 backbones where an implanted one is detected once cluster orientations are randomized, and the test is blind in the other 3." in bf and "given a three-level hierarchy with ViT-L's spectrum and noise, it detects it in" in bf and "no hierarchy above the superclasses is certified" not in bf and "clusters oriented toward their hubs in a few, no hub hierarchy in 9 of 12 backbones where an implanted one is detected once cluster orientations are randomized, a blind test in the other 3, and a nominally hyperbolic backbone" in bf
        and "\\textbf{The depth test certifies in 4 of 12 ImageNet backbones that clusters are oriented toward their hubs, not that the hubs of the frame form a hierarchy; no hub hierarchy is found in 9 of 12 backbones where the decoupled control detects an implanted one, and the test is blind in the other 3.}" in bf and "hub-aligned" not in bf and bf.count("clusters are oriented toward their hubs") >= 2 and "A model--dataset cell is genuine when" in bf and "certified against its matched star" in bf and "no hub hierarchy is found in 9 of 12 backbones where the decoupled control detects an implanted one, and the test is blind in the other 3" in bf and "where a deep synthetic one is detected" not in bf and "left open" not in bf and "no power at this noise level" not in bf
        ...
        and "certifies clustering" not in TF and "validated regime" not in TF and "validated range" not in TF and "The census does not certify that frame" in bf and "The alignment is relational" in bf and "organize classes into clusters, agree on how those clusters nest well above chance" in bf
    ...
    chk("final (priority 1c as a limitation): the author's sentence with the percentage and the family statement re-derived from expR80_implanted_alignment.csv, Table 10c per backbone and pooled with the rule's outcome, no 'measured power' certification wording, no dotted curve unless the rule was met",
        fam_ok and f"({pct}\\%)" in pw and "Implanted alignment is detected in" not in bf
    ...
    LEADS53 = ["The depth test certifies that clusters are oriented toward their hubs in 4 of 12 backbones.", "What it certifies is alignment.", "What the test can see.", "No hub hierarchy is found where the test has power.", "The certified set depends on the frame.", "Leaf labels can produce the alignment but do not guarantee it.", "MERU's objective does not create hub structure."]   # cleanup of 2026-09-22; MERU lead-in since the twelfth review
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 10d, and no 'left open' or 'no power' wording anywhere",
        ...
        and f"A flat control with the same spectrum and ratio as the deep one fires in {nf} of {len(flat)} intact and in {int(round(flat.dec_frac_cert.sum() * 10))} of 50 decoupled runs." in bf and "The decoupling control fires on every deep seed and reads deeper than the intact cloud" in bf and "read with Euclidean distances, do not fire (Tables" in bf
        and f"it detects it in {nd} of {len(deep)} seeds." in bf and "(d) A deep hierarchy at the real noise level" in pw and f"The deep hierarchy fires in {nd} of {len(deep)} seeds and the flat control in {nf} of {len(flat)}" in pw
        and "(iii)~The depth test's power for a three-level hierarchy follows the backbone rather than its within/between ratio or family" in bf and "left open" not in bf and "no power at this noise level" not in bf and "without power" not in bf and "\\paragraph{The two-level implant is missed" not in bf and "Figure~\\ref{fig:implant} and Table~\\ref{tab:q5-power} give the detection rates and the power." in bf,
    ...
    chk("final (priority 2): the S5.1 published-reading paragraph after the lead-in, its claims re-derived from expR78_khrulkov_replication_summary.csv (all four within 0.03, excess negative everywhere, p <= 0.05 on CIFAR-100 and MiniImageNet only), the new lead-in, limitation (vi), and Table 8(b) with the four published and reproduced values",
        ...
        and "reproduces the raw $\\delta_{\\text{rel}}$ that \\citet{Khrulkov_2020_CVPR} report for ResNet-34 on 4 datasets within 0.03" in bf and "Calibrated, CIFAR-10 and CUB-200 are indistinguishable from a random cloud, and CIFAR-100 and MiniImageNet fall below it." in bf and set(S78.index[S78.p_left_max > 0.05]) == {"cifar10", "cub"} and "2 datasets, one image budget and one published setting" in bf
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              and "The same hierarchy was then built at each backbone's own spectrum and ratio." in bf and f"The decoupled control detects it in 9 of 12 backbones (decoupled power {S81.loc[cov, 'dec_power'].min():.2f}--{S81.loc[cov, 'dec_power'].max():.2f}) and not in the other 3 (power {S81.loc[unc, 'dec_power'].min():.2f}--{S81.loc[unc, 'dec_power'].max():.2f})." in bf
              and "The power is the decoupled control's: it detects an implanted hierarchy in ViT-S/B/L, DINOv2-B/L/G, CLIP-B/L and SigLIP-B, and the test is blind in ViT-T, DINO-B and DINOv2-S. Where the implant is detected, none as strong as the planted one is found." in bf
              and f"The 3 blind backbones sit at ratios {rb.min():.1f} to {rb.max():.1f}, inside the {rc.min():.1f} to {rc.max():.1f} spanned by the 9 covered backbones. The power therefore follows the backbone, not its ratio or family." in bf and rc.min() <= rb.min() and rb.max() <= rc.max()
              and "(e) Decoupled power" in pw and all(int(S81.loc[m, "n_seeds"]) == 20 for m in ("dinov2_s", "dinov2_b", "dinov2_l", "dinov2_g")) and f"Under the decoupled control it is {S81.loc[cov, 'dec_power'].min():.2f} or more in 9 of 12 backbones and {S81.loc[unc, 'dec_power'].max():.2f} or less in the other 3. Its false alarms" in bf)
    ...
    pc_ok = (P77.loc["hier_seed0", "z_wn30"] <= -2 and P77.loc["hier_seed0", "z_wn30bal"] <= -2 and P77.loc["ce_seed0", "z_wn30"] <= -2 and P77.loc["frozen", "z_wn30"] <= -2 and not V77["criterion_met"]
             and f"ViT-B fine-tuned with a hierarchical cross-entropy keeps firing once decoupled in {sum(int(round(P77.loc[f'hier_seed{s_}', 'dec_frac_cert_wn30'] * 10)) for s_ in (0, 1))} of 20 runs over 2 seeds, mean $z$ ${P77.loc[['hier_seed0', 'hier_seed1'], 'zdec_mean_wn30'].mean():.2f}$, on the frame of record. The leaf-only fine-tune fires in {sum(int(round(P77.loc[f'ce_seed{s_}', 'dec_frac_cert_wn30'] * 10)) for s_ in (0, 1))} of 20, mean $z$ ${P77.loc[['ce_seed0', 'ce_seed1'], 'zdec_mean_wn30'].mean():.2f}$, and the frozen checkpoint in none." in bf and int(round(P77.loc['frozen', 'dec_frac_cert_wn30'] * 10)) == 0 and "Fine-tuning itself adds some decoupled signal and the hierarchical objective adds more, so the trained control separates the objectives by degree." in bf and not any(v["criterion_met"] for v in _j.load(open(R/"expR77_positive_control_verdict.json"))["verdict"])
    ...
    chk("final (7th review): headline scoped in the abstract (twice), S1, contribution 2, S5.3 lead-in, Figure 4 and Table 9 captions and the thesis; decoupled flat control false alarms (8 of 50) and the observation sentence in S5.3 from the files; limitation (iii) with ViT-L's ratio 1.9 and the DINOv2 range from expR64b; Figure 1 caption 'read alike, all low'; no unproved extension in the proof of Proposition 1(b)",
        "The control covers the supervised" not in bf and "whose noise level, ratios" not in bf and p81_ok and pc_ok and bf.count("no hub hierarchy is found in 9 of 12 backbones where the decoupled control detects an implanted one, and the test is blind in the other 3") == 1 and "Where a planted hierarchy is detected, in 9 of 12 backbones, none as strong as the planted one is found; in the other 3 the test cannot tell." in abs_now and "noise level at which the test was validated" not in bf and "\\paragraph{No hub hierarchy is found where the test has power.}" in bf and " in the backbones whose noise level the control covers" not in TF and f"The synthetic hierarchy at $z$ ${deep.zdec_mean.max():.2f}$ to ${deep.zdec_mean.min():.2f}$ stands well clear of that bias." in bf and "none of the 4 backbones fires" in bf and f"ViT-L then reads ${float(next(r for r in load('expR74_decoupling_summary.csv') if r['model'] == 'i21k_l')['dec_z_mean']):.2f}$, within the ${flat.zdec_mean.max():.2f}$ to ${flat.zdec_mean.min():.2f}$ of the flat control: without its orientations the real cloud reads like a flat one." in bf and "lie at the edge of the covered range" not in bf and rad_ok and "On class centroids there is structure that a random cloud does not have, in 44 of 72 cells and 30 of 36" in bf and bf.index("The noise level of a cloud is its within-cluster spread relative to the distance between its hubs.") < bf.index("at this noise level, the test misses it") and "unregime" not in TF and bf.count("4 values reported by") == 1 and "2 of them are indistinguishable from a random cloud" in bf and int((S78.p_left_max > 0.05).sum()) == 2 and not any(w_ in bf[bf.index("\\textbf{The answer has three parts.}"):bf.index("\\textbf{The answer has three parts.}") + 260].lower() for w_ in ("genuine", "certified", "record", "matched star", "null")) and "The decoupled test is therefore biased toward firing, so the 4 real backbones not firing once decoupled is conservative evidence" in bf and "its decoupled form is biased toward firing" in bf and "where a deep synthetic one is detected" not in bf
        and f"in {fa_dec} of 50 decoupled runs." in bf and obs_ok
        ...
        and ("no hub hierarchy is found in 9 of 12 backbones where the decoupled control detects an implanted one, and the test is blind in the other 3") in dep, f"fa_dec {fa_dec} obs {dobs['kind']}")
    ...
    bal_ok = (S83 is not None and bool((S83.n_decoupled == 50).all()) and bool(bal) and int(bal["fa_bal"]) == int(S83.loc["balanced", "decoupled_fired"]) and bal["sentence"] in bf and bal.get("author_sentence")
              and bal["sentence"] == f"A flat cloud clustered under the frame of record fires under the balanced frame in {int(S83.loc['wn30', 'decoupled_fired'])} of 50 decoupled runs. A real cloud read under a frame that is not its own is therefore expected to fire. The frozen ViT-B firing in {int(round(P77.loc['frozen', 'dec_frac_cert_wn30bal'] * 10))} of 10 there is consistent with that mismatch and is not evidence of hub structure."
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 10(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 4; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 9(d)",
        p81_ok and bf.count("no hub hierarchy is found in 9 of 12 backbones where the decoupled control detects an implanted one, and the test is blind in the other 3") == 1 and "\\paragraph{No hub hierarchy is found where the test has power.}" in bf
        and "Where a planted hierarchy is detected, in 9 of 12 backbones, none as strong as the planted one is found; in the other 3 the test cannot tell." in abs_now and "five backbones" not in TF and "other seven" not in TF and "in the five only" not in TF and "in the five and" not in TF
        and f"A flat control with the same spectrum and ratio as the deep one fires in {nf} of {len(flat)} intact and in {fa_dec} of 50 decoupled runs." in bf and f"Its false alarms on flat clouds are {fa_dec} of 50, and the verdict of no hub hierarchy therefore holds in those 9 only." in bf
        and bal_ok
        and "measures structure above the clusters of the frame only" in bf and "measures hierarchy above" not in bf and "\\paragraph{Leaf labels can produce the alignment but do not guarantee it.}" in bf and "\\paragraph{MERU's objective does not create hub structure.}" in bf and "detected depth" not in bf and "a nominally hyperbolic backbone as the control for imposing the geometry" in bf
        and f"{c58.triplet_agree_big_vs_block:.2f} for DINOv2 against the block, {c58.triplet_agree_within_block:.2f} within it." in bf and "About a third of the within-block agreement is missing under the admissible configurations as a whole." in bf
        and "the Gaussian band, the sampled supremum rather than the census percentile" in bf and "It assigns structureless clouds a curvature from" in bf and "the sampled supremum from the released control file" in (FD/"tab_q10_calibration.tex").read_text()
    ...
    s53 = nocom(seg(bf, "\\subsection{Depth above the labelled clusters}", "\\subsection{Whose tree}")); s3_ = seg(bf, "\\section{Methodology}", "\\section{Experimental Setup}")
    once = {k: len(_re.findall(r"(?<!\d)" + _re.escape(k) + r"(?!\d)", s53)) for k in ("4 of 5", "8 of 50", "0 of 50", "38 of 50", "7 of 10", "20 of 20", "6 of 20", "0 of 60", "0 of 4", "0 of 5")}
    ...
    kt_ok = bool(kt) and all(f"{NMD[d]} & {S78.loc[d].theirs:.2f} & {S78.loc[d].ours_raw_mean:.3f} & ${S78.loc[d].excess_mean:+.4f}$ & {int(round(S78.loc[d].r_above_mean))}/200, {S78.loc[d].p_left_max:.3f} \\\\" in kt for d in S78.index) and "\\label{tab:khrulkov}" in kt and "expR78_khrulkov_replication_summary.csv" in kt and kt.count("\\\\") == 5
    chk("final (cleanup, 2026-09-22): S3.3 duplicates out ('Size and evidence', 'What a genuine excess means'); Definition 8 (decoupling control) after the matched star with the author's wording and the power/false-alarm sentence; S4 lead-in and S5.2 sentence gone, the two S5.2 paragraphs at two sentences; S3.2 supremum sentence gone (citation on Definition 1); S5.3 in the seven paragraphs in order with each count once and the bias sentence once; the Khrulkov table in S5.1 with its cells from expR78, cited by the paragraph",
        "\\paragraph{Size and evidence.} The excess is a size; the evidence is the rank of the reading among its replicates and the left-tail $p$ that the rank gives," in bf and "The rank of the reading is the evidence" not in bf and "\\paragraph{What a genuine excess means.} A negative genuine excess, more than chance would give, certifies structure beyond the second moments; clustering is its most plausible reading" in bf and "A negative genuine excess means structure" not in bf
        and s3_.count("\\begin{definition}[decoupling control]") == 1 and s3_.index("\\begin{definition}[matched star]") < s3_.index("\\begin{definition}[decoupling control]") < s3_.index("\\paragraph{The depth test compares the real cloud with its matched star.}")
        and "The decoupling control keeps the hubs of the frame and rotates each cluster's offsets by an independent Haar rotation. A verdict that survives the decoupling is carried by the arrangement of the hubs; one that does not is carried by the orientation of the clusters relative to their hubs, which we call alignment." in s3_
        and "The power of the test is the fraction of implanted hierarchies it detects, and its false-alarm rate is the fraction of flat controls that fire. Both are measured on real clouds in Section~\\ref{sec:f-depth}." in s3_ and "the false-alarm rate and power are measured on real clouds" not in bf
        and "Every reading is calibrated against two hundred replicates" not in bf and "\\paragraph{Calibration budget and frames.} Each cell is read against 200 Haar replicates" in bf and bf.count("200 Haar replicates") == 1
        ...
        and all(v == 1 for v in once.values()) and s53.count("biased toward firing") == 1 and len(_re.findall(r"\\paragraph\{", s53)) == 7 and all(("\\paragraph{" + l + "}") in s53 for l in LEADS53)
        and kt_ok and "\\input{tab_khrulkov_final}" in bf and "Table~\\ref{tab:khrulkov} gives the 4 rows" in bf and bf.index("\\paragraph{A published reading is reproduced and calibrated.}") < bf.index("\\input{tab_khrulkov_final}") < bf.index("\\subsection{Structure beyond the second moments at the class level}"),
    ...
    chk("final (10th review): the balanced frame read as frame mismatch (author's sentence, counts from expR83/expR77) and the matched count kept in (c); the alignment stated as relational, not radial, reproduced by the principal-axis implant in the supervised ViTs and CLIP-B and not in DINOv2-L, form unresolved (S5.3 and (iii)); the decoupling's separation and 'no hub hierarchy' = none among the frame's hubs (S5.3 and (iii)); 'hubs of the frame' in the abstract, S1 and the captions; the within-model ceiling from expR84 beside 0.77/0.76 with the keep branch (ceiling >= 0.9), Table 11(c); limitation (vi) on analysis choices, scope renumbered; Table 14(b) restored",
        bal_ok and "The implanted principal-axis alignment reproduces it in the supervised ViTs and CLIP-B but not in DINOv2-L, and its geometric form is not resolved here." in bf and fam_ok
        and "The alignment is relational: 0 of 60 runs fire with random hubs and 0 of 4 with random orientations, so it needs both the real hubs and the real orientations." in bf
        and "Its exact form is not resolved here." in bf
        and "The decoupling separates structure among the hubs of the frame from structure carried by cluster orientations. A hierarchy below the frame expressed in how clusters open would be removed with the orientations and counted as alignment. No hub hierarchy therefore means no hierarchy among the frame's hubs." in bf
        and "and it certifies alignment above its frame only." in bf
        and "not a hierarchy among those centers" in abs_now and "not that the hubs of the frame form a hierarchy" in bf and TF.count("not that the hubs form a hierarchy") == TF.count("not that the hubs form a hierarchy within the grouping") and "not that the hubs form a hierarchy" not in dep and "not that the hubs of the frame form a hierarchy" in dep
        and S84 is not None and len(S84) == 13 and bool(tc) and tc.get("branch") == "keep" and float(S84.loc["ALL", "triplet_agree_mean"]) >= 0.9 and f"Two resamples of the same model agree on {S84.drop('ALL').triplet_agree_mean.min():.2f} to {S84.drop('ALL').triplet_agree_mean.max():.2f} of the triplets across backbones, mean {S84.loc['ALL', 'triplet_agree_mean']:.2f}. Across models: {c58.triplet_agree_big_vs_block:.2f} for DINOv2 against the block, {c58.triplet_agree_within_block:.2f} within it." in bf
    ...
    chk("final (thesis after expR84): the new thesis twice, the abstract's sharing sentence and its S1 mirror with 0.77 and the triplet ceiling range from the files, contribution 3 mirrored, S5.4 with the three ceilings (range and mean) beside the cross-model values and the reading (topology within most of the ceiling, metric at about half)",
        ...
        and "Across models, the usual comparison makes the self-supervised models look like outliers, an artifact of how trees are cut into groups; compared properly, the trees agree on which classes group together well above chance, though short of what two readings of the same model reach, but not on their distances; and the self-supervised models organize classes by direction rather than by distance." in abs_now and bf.count("well above chance, though short of what two readings of the same model reach") == 4 and "but not on their distances" in abs_now
        and "\\item \\textbf{The map of trees.} The island is an artifact of the cut; the trees agree on which classes group together well above chance, though short of what two readings of the same model reach, but not on the distances between them, and the self-supervised tree is angular." in bf and "sharing is graded and supervision-dependent" not in bf and "largely shared" not in bf
        and f"The cophenetic correlation reaches {b84.coph_corr_mean.min():.2f} to {b84.coph_corr_mean.max():.2f} within a model, mean {S84.loc['ALL', 'coph_corr_mean']:.2f}. Across models: {c58.coph_corr_big_vs_block:.2f} against the block, {c58.coph_corr_within_block:.2f} within it." in bf
        and f"The ARI at the cut reaches {b84.ari_cut_mean.min():.2f} to {b84.ari_cut_mean.max():.2f} within a model, mean {S84.loc['ALL', 'ari_cut_mean']:.2f}. Across models: {c58.ari_cut_big_vs_block:.2f} against the block, {c58.ari_cut_within_block:.2f} within it." in bf
        and "The tree topology is therefore shared well above chance, though short of what two readings of the same model reach, and the metric agrees at about half of the ceiling." in bf and "the island shrinks to a gap beyond resampling noise" in bf
    ...
    chk("final (consolidated pass): the thesis is the abstract's last sentence and S7's; S1 glosses hubs and the frame at first use and says 'once cluster orientations are randomized' instead of 'decoupled control'; S1 and contribution 3 mirror the abstract's sharing sentence; S5.4 opens by citing Figure 5 with the author's sentence; Figure 5 caption and the appendix matrices; Definition 8 and the frame-mismatch reading split at 45 words",
        abs_now.endswith(THESIS) and bf.count(THESIS) == 2 and "the superclass centers, which we call hubs" in s1_ and "the grouping of classes into superclasses, which we call the frame" in s1_ and "decoupled control" not in s1_ and s1_.count("once cluster orientations are randomized") == 2
        and s1_.find("hubs") == s1_.index("which we call hubs") + 14 and s1_.find("the frame") == s1_.index("which we call the frame") + 14
        and "Figure~\\ref{fig:treemap} compares the trees of the 12 models in three ways. Panel (a) compares them as usually done, (b) under the criterion of Section~\\ref{sec:f-treemap} against the within-model ceiling, and (c) by whether classes are grouped by direction or by distance." in bf and bf.index("\\paragraph{The naive map manufactures an island.} Figure~\\ref{fig:treemap} compares") > bf.index("\\label{fig:treemap}")
        and (TEX/"figures/fig_treemap_matrices_final.pdf").exists() and "mean_ari_other" in v5 and "ceiling_band" in v5 and "The frozen ViT-B firing in" in bf and "A verdict that survives the decoupling is carried by the arrangement of the hubs; one that does not" in bf,
    ...
    chk("final (12th review): 'none as strong as the planted one is found' in the abstract and S5.3(d); 'well above chance, though short of what two readings of the same model reach' in the abstract, S1 (twice) and S5.4; the thesis with 'well above chance, though not as much as two readings of one model' twice; MERU lead-in; limitation (vii) on the diameter normalization with scope renumbered (viii)-(x); every continued caption of Table 3 names its panel",
        abs_now.endswith(THESIS) and bf.count(THESIS) == 2 and "well above chance, though not as much as two readings of one model" in THESIS and "none as strong as the planted one is found" in abs_now and bf.count("none as strong as the planted one is found") == 2 and "none is found in the real model" not in bf
        and "\\paragraph{MERU's objective does not create hub structure.}" in bf and "Imposing the geometry does not create" not in bf
        and "(vii)~The reading divides by the diameter, so heavier tails in real clouds would lower $\\delta_{\\text{norm}}$ without any clustering. The cosine census mitigates this, and a percentile normalization is future work." in bf
        and len(rob_c) == 4 and all(rob_c[k].strip().startswith(p_) for k, p_ in enumerate(("(b) ", "(c) ", "(d) ", "(e) "))),
    ...
    DESCRIPTIVE = ["Two cells of the same dataset", "two readings of the same model", "two readings of one model", "three constructions", "two constructions", "two fifths", "two null standard deviations"]   # descriptive quantities stay in words (2026-09-23)
    for d_ in DESCRIPTIVE: main_txt = main_txt.replace(d_, " "); capt = capt.replace(d_, " ")
    spelled = [m.group(0) for m in _re.finditer(WORDS, main_txt + " " + capt)]   # case-sensitive: a count that opens a sentence is spelled out and capitalized (exception of 2026-09-23), so 'Four controls' and 'Two self-supervised ResNets' do not match
    ...
    chk("final (numeral rule): counts of models, cells, seeds and runs are numerals with their denominator in the abstract, the main text and every caption ('9 of 12 backbones', 'the other 3'); no spelled-out count before a count noun; words kept for descriptive quantities",
        not spelled and not digit_start and "\\paragraph{Four controls test the alternatives.} Two self-supervised ResNets" in bf and "in 9 of 12 backbones" in abs_now and "in the other 3 the test cannot tell" in abs_now and "two readings of the same model" in abs_now and "a three-level hierarchy" in bf and "12 backbones, 6 class sets and 15 text models" in bf, f"spelled {spelled[:8]} digit-start {digit_start[:4]}")
    ...
    chk("final (12th review, item 5): Table 1 carries the excess and rank of the published statistic itself, the supremum, calibrated on the same 200 replicates over 10 trials per dataset (cells from expR85_khrulkov_sup_summary.csv), and S5.1 says so; while the run is not merged, the fallback clause of the brief",
        (all(f"${S85.loc[d, 'excess_sup_mean']:+.4f}$ & {int(round(S85.loc[d, 'r_above_mean']))}/200, {S85.loc[d, 'p_left_max']:.3f}" in kt85 for d in S85.index) and bool((S85.n_trials == 10).all()) and "(sup.)" in kt85 and "expR85_khrulkov_sup_summary.csv" in kt85
         and "with their own statistic, the supremum, calibrated on the same replicates beside the record statistic" in bf) if S85 is not None
        else ("the calibration reads the percentile statistic $\\hat\\delta_{99.9}$, whereas their statistic is the supremum" in bf and "(sup.)" not in kt85),
    ...
    chk("final (readability 2): Definition 3 opens with the plain sentence; S3.2 confounds paragraph in one sentence; S3.3 'Two cells of the same dataset can have the same reading and opposite verdicts'; S3.4 opens with the author's two sentences before Definition 6; the three verbatim S2/S3 sentences split by recorded edits; no sentence over 30 words in S3-S7 outside definitions, citation lists and the thesis",
        ...
        and "Two cells of the same dataset can have the same reading and opposite verdicts (Figure~\\ref{fig:overview})." in bf and bf.index("\\paragraph{Why a second test.} A genuine excess can come from clusters alone, so a second test asks whether the clusters are themselves arranged hierarchically. It needs three constructions.") < bf.index("\\begin{definition}[hub null and excess B]")
