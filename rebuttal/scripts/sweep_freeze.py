...
def final_checks():
    ...
    LONGN_ = " in the supervised and contrastive backbones, whose noise level, ratios 1.3 to 2.0, the control covers, and not tested in the DINOv2 family, ratios 3.0 to 3.9"; SHORT_ = " in the supervised and contrastive backbones; the DINOv2 family lies outside the control's noise level"
    ...
    chk("final (7th review): headline scoped in the abstract (twice), S1, contribution 2, S5.3 lead-in, Figure 4 and Table 8 captions and the thesis; decoupled flat control false alarms (8 of 50) and the observation sentence in S5.3 from the files; limitation (iii) with ViT-L's ratio 1.9 and the DINOv2 range from expR64b; Figure 1 caption 'read alike, all low'; no unproved extension in the proof of Proposition 1(b)",
        bf.count("hierarchy above the superclasses is found" + LONGN_) == 3 and bf.count("no hierarchy above the superclasses" + LONGN_) == 1 and bf.count("no hierarchy above the superclasses is found" + SHORT_) == 3 and "\\paragraph{No hierarchy above the superclasses is found in the supervised and contrastive backbones.}" in bf and " in the backbones whose noise level the control covers" not in TF and f"the synthetic hierarchy at $z$ ${deep.zdec_mean.max():.1f}$ to ${deep.zdec_mean.min():.1f}$ stands well clear of that bias, the flat control at ${flat.zdec_mean.max():.1f}$ to ${flat.zdec_mean.min():.1f}$." in bf and "The decoupled test is therefore biased toward firing, so the four real backbones not firing once decoupled is conservative evidence" in bf and "which makes the real verdicts under decoupling conservative" in bf and "where a deep synthetic one is detected" not in bf
        ...
        and "in the supervised and contrastive backbones, whose noise level the control covers, and not tested in the DINOv2 family" in dep, f"fa_dec {fa_dec} obs {dobs['kind']}")
