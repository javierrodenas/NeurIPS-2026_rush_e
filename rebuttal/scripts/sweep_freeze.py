...
def final_checks():
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "What it certifies is alignment.": (5, 2), "What the test can see.": (12, 2)}   # cleanup of 2026-09-22: S5.3 in seven paragraphs, each number once; (c) carries the counts of the test's power and biases
    ...
    bal_ok = (S83 is not None and bool((S83.n_decoupled == 50).all()) and bool(bal) and int(bal["fa_bal"]) == int(S83.loc["balanced", "decoupled_fired"]) and bal["sentence"] in bf and bal.get("author_sentence")
              and bal["sentence"] == f"On the balanced frame the frozen ViT-B fires once decoupled in {int(round(P77.loc['frozen', 'dec_frac_cert_wn30bal'] * 10))} of 10 seeds. That frame's matched flat control does not fire once decoupled, so this is not a false alarm: ViT-B shows hub structure under that grouping that the frame of record does not resolve; and a cloud organized under one frame fires under the other even once decoupled, so frames are not interchangeable."
              and f"On the balanced frame the matched flat control fires in {int(S83.loc['balanced', 'decoupled_fired'])} of 50 decoupled runs and a flat cloud clustered by the frame of record in {int(S83.loc['wn30', 'decoupled_fired'])} of 50." in bf and int(S83.loc["balanced", "decoupled_fired"]) == 0
