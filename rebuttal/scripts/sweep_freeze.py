...
def final_checks():
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 10d, and no 'left open' or 'no power' wording anywhere",
        ...
        and "The power follows the backbone, not its noise level or family." not in bf and "The power follows the backbone, not the noise level of its cloud." in app_f and "left open" not in bf and "no power at this noise level" not in bf and "without power" not in bf and "\\paragraph{The two-level implant is missed" not in bf and "Figures~\\ref{fig:implant} and \\ref{fig:power} give the detection rates and the power." in bf,
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              ...
              and f"The 3 that cannot see a planted hierarchy, {_names_unc}, sit at noise levels {rb.min():.1f} to {rb.max():.1f}, inside the {rc.min():.1f} to {rc.max():.1f} spanned by the 9 covered backbones." in bf and rc.min() <= rb.min() and rb.max() <= rc.max()
    ...
    chk("final (12th review, item 5): Table 1 carries the excess and rank of the published statistic itself, the supremum, calibrated on the same 200 replicates over 10 trials per dataset (cells from expR85_khrulkov_sup_summary.csv), and S5.1 says so; while the run is not merged, the fallback clause of the brief",
        (all(f"${S85.loc[d, 'excess_sup_mean']:+.4f}$ & {int(round(S85.loc[d, 'r_above_median']))}/200, {S85.loc[d, 'p_left_min']:.3f}--{S85.loc[d, 'p_left_max']:.3f}" in kt85 for d in S85.index) and bool((S85.n_trials == 10).all()) and "(sup.)" not in kt85 and "\\multicolumn{2}{c}{supremum, fixed base point}" in kt85 and "expR85_khrulkov_sup_summary.csv" in kt85
         and "only before correction ($p=0.030$). Under their own statistic, the supremum from a fixed base point, CIFAR-10 and CUB-200 stay indistinguishable from a random cloud and MiniImageNet stays below it. The verdict for CIFAR-100 changes from trial to trial, as the statistic confound predicts. Figure~\\ref{fig:premise}b and Table~\\ref{tab:khrulkov} give the 4 datasets." in bf
         ...
         and "For their own statistic, the supremum from a fixed base point, against the same replicates: the mean excess, the median rank over 10 trials and the range of $p$ across trials. CIFAR-100 is below the null in the median trial but not in every trial." in kt85 and "$r$/200, $p$ range \\\\" in kt85) if S85 is not None
    ...
    chk("final (4th review): depth table with two-decimal z everywhere and the K = 10/30/60 sweep with the balanced frame; DBpedia supremum columns under the Haar null; no OLMo-7B row; no expR32 bootstrap row",
        ...
        and "OLMo-7B" not in txt_ and "OLMo-7B" not in pan_ and "centroid bootstrap: excess" not in rob and "expR32" not in rob and "expR73" in rob and "how many quadruples we sample and which images stand behind each centroid. Each is varied in turn and the same cells are read again." in app_f and "A class-count control shows" not in app_f and sorted(set(int(r_["C"]) for r_ in load("expR60_c_sweep_record.csv"))) == [10, 20, 50, 100, 200, 500, 1000] and rob.count("DINOv2-L & ") >= 2)
