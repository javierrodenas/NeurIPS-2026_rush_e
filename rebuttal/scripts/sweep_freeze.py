...
def final_checks():
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "What it certifies is alignment.": (5, 2), "What the test can see.": (12, 2), "Controlling the cut leaves a moderate gap.": (3, 2), "The certified set depends on the frame.": (2, 2)}   # cleanup of 2026-09-22: S5.3 in seven paragraphs, each number once; (c) carries the counts of the test's power and biases
    SENT_EXEMPT = {"What the test can see.", "The count survives resampling.", "Neural collapse is the flat limit, not what the census sees.", "Size and evidence."}   # cleanup of 2026-09-22: (c) by design, the two S5.2 paragraphs cut to two sentences by the author   # the author's count sentences carry more than one number
    import pandas as pd
    ...
    chk("final (9th review): nine covered / three blind from the decoupled power of Table 9(e); the long form in S1 and the Figure 4 caption, the short form as the thesis twice, the abstract sentence, no five/seven left; decoupled false alarms next to the split in S5.3 and (iii); the balanced-frame sentence from expR83 (matched hubs) with the recorded rule; vocabulary (structure above the clusters, alignment, hub structure, nominally hyperbolic); triplet 0.77 vs 0.76 from expR58; Gaussian band named as the sampled supremum in S6 and Table 3; Moreira 2024 cited with a verified entry; proof end tied to the statistic confound; no experiment IDs in any caption; Figure 4b = decoupled-power bars; implant curves as an appendix figure; B.7 points to Table 8(d)",
        ...
        and f"the sibling-triplet agreement of DINOv2 with the block is {c58.triplet_agree_big_vs_block:.2f} against {c58.triplet_agree_within_block:.2f} within the block." in bf and "About a third of the within-block agreement is missing under the admissible configurations as a whole." in bf
    ...
    chk("final (10th review): the balanced frame read as frame mismatch (author's sentence, counts from expR83/expR77) and the matched count kept in (c); the alignment stated as relational, not radial, reproduced by the principal-axis implant in the supervised ViTs and CLIP-B and not in DINOv2-L, form unresolved (S5.3 and (iii)); the decoupling's separation and 'no hub hierarchy' = none among the frame's hubs (S5.3 and (iii)); 'hubs of the frame' in the abstract, S1 and the captions; the within-model ceiling from expR84 beside 0.77/0.76 with the keep branch (ceiling >= 0.9), Table q6(c); limitation (vi) on analysis choices, scope renumbered; Table 14(b) restored",
        bal_ok and "The implanted principal-axis alignment reproduces the verdict in the supervised ViTs and CLIP-B but not in DINOv2-L, and the exact geometric form of the alignment is not resolved here." in bf and fam_ok
        ...
        and S84 is not None and len(S84) == 13 and bool(tc) and tc.get("branch") == "keep" and float(S84.loc["ALL", "triplet_agree_mean"]) >= 0.9 and f"Both sit below the within-model ceiling of {S84.loc['ALL', 'triplet_agree_mean']:.2f}, the agreement two resamples of the same model reach, {S84.loc['ALL', 'triplet_agree_min']:.2f} for the lowest backbone." in bf
        and c58.triplet_agree_big_vs_block < S84.loc["ALL", "triplet_agree_min"] and "(c) Within-model ceiling" in tm6 and f"mean over backbones & {S84.loc['ALL', 'ari_cut_mean']:.2f}" in tm6 and "moderately shared" in bf and THESIS in bf
