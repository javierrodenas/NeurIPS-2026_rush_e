...
def final_checks():
    ...
    chk("final (12th review, item 5): Table 1 carries the excess and rank of the published statistic itself, the supremum, calibrated on the same 200 replicates over 10 trials per dataset (cells from expR85_khrulkov_sup_summary.csv), and S5.1 says so; while the run is not merged, the fallback clause of the brief",
        (all(f"${S85.loc[d, 'excess_sup_mean']:+.4f}$ & {int(round(S85.loc[d, 'r_above_median']))}/200, {S85.loc[d, 'p_left_max']:.3f}" in kt85 for d in S85.index) and bool((S85.n_trials == 10).all()) and "(sup.)" in kt85 and "expR85_khrulkov_sup_summary.csv" in kt85
         and "fall below it. Under their own statistic, the supremum, calibrated against the same replicates, CIFAR-100 joins CIFAR-10 and CUB-200 among the indistinguishable, and only MiniImageNet falls below the random cloud. Table~\\ref{tab:khrulkov} gives the 4 rows." in bf and set(S85.index[S85.p_left_max > 0.05]) == {"cifar10", "cifar100", "cub"} and float(S85.loc["miniimagenet", "p_left_max"]) <= 0.05
         and "2 of them are indistinguishable from a random cloud, 3 under their own statistic" in s1_ and int((S85.p_left_max > 0.05).sum()) == 3
         and "For the supremum the excess is the mean and the rank the median over the 10 trials, and $p$ the largest. Under the supremum, CIFAR-100 is not below the null at 0.05." in kt85 and float(S85.loc["cifar100", "p_left_max"]) > 0.05 and float(S78.loc["cifar100"].p_left_max) <= 0.05) if S85 is not None
