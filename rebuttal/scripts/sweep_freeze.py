...
def final_checks():
    ...
    for name, text in blocks:
        ...
        for par in pars:
            ...
            if vb: continue
            nverb_sents += w
            w_ = [len(s.split()) for s in sents if not s.startswith(("Read correctly, foundation models", "The test misses an implanted two-level tree", "Implanted alignment is detected in"))]   # the author's verbatim sentences are exempt from the 35-word rule   # the author's thesis sentence is verbatim and exempt from the 35-word rule
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 9d, and no 'left open' or 'no power' wording anywhere",
        ...
        and bf.index("\\paragraph{No hierarchy above the superclasses is found in the backbones whose noise level the control covers.}") < bf.index("\\paragraph{A deep hierarchy at the real noise level is detected.}") < bf.index("\\paragraph{The test never fires on randomized hubs.}")
    ...
    chk("final (7th review): headline scoped in the abstract (twice), S1, contribution 2, S5.3 lead-in, Figure 4 and Table 8 captions and the thesis; decoupled flat control false alarms (8 of 50) and the observation sentence in S5.3 from the files; limitation (iii) with ViT-L's ratio 1.9 and the DINOv2 range from expR64b; Figure 1 caption 'read alike, all low'; no unproved extension in the proof of Proposition 1(b)",
        bf.count("no hierarchy above the superclasses is found" + " in the backbones whose noise level the control covers") >= 5 and ("\\paragraph{No hierarchy above the superclasses is found" + " in the backbones whose noise level the control covers" + ".}") in bf and ("no hierarchy above the superclasses" + " in the backbones whose noise level the control covers" + ", and a hyperbolic backbone") in bf and "where a deep synthetic one is detected" not in bf
        ...
        and "in the backbones whose noise level the control covers" in dep, f"fa_dec {fa_dec} obs {dobs['kind']}")
