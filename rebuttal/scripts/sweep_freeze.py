...
def final_checks():
    ...
    chk("final: the recorded edits are exactly the briefs' (shadow x2, geometric face, intent of the supremum, the bridge; 4th/5th reviews: abstract, S1 confounds and counts, S2 citations) and none of the old phrases survives",
        len(ED) == 28 and all((a not in bf) or (a in b) for a, b in ED) and any("clusters oriented toward their hubs in a few" in b for _, b in ED) and any("In the 9 of 12 backbones where an implanted hierarchy is detected, none as strong is found; in the other 3 the test is blind." in b for _, b in ED) and "49 of 72" not in bf and all((fillb(b) in bf) for _, b in ED if b and not any(a2 in b for a2, _ in ED if a2)) and sum(1 for a, _ in ED if "shadow" in a) == 2 and any("geometric face" in a for a, _ in ED) and any("intent of the supremum" in a for a, _ in ED) and any("next question" in a for a, _ in ED)
