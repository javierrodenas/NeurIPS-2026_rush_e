...
def final_checks():
    ...
    chk("final: abstract is the recorded text (250 words or fewer, cell defined in its own sentence, the deep synthetic hierarchy detected and no hierarchy above the superclasses found), 15 text models", abs_now == norm(abs_expected) and len(abs_now.split()) <= 250 and "A cell is one model on one dataset." in bf and "a three-level hierarchy implanted at the real noise level is detected and survives orientation randomization, whereas none of the four real verdicts does, so no hierarchy above the superclasses is found in the supervised and contrastive backbones, not in the DINOv2 family, which lies beyond the noise level at which the test was validated" in abs_now and "15 text models" in bf and "16 text models" not in bf and "sixteen" not in bf and "two OLMo" not in bf and "no hierarchy above the superclasses is found" in abs_now and "30 of 36" not in abs_now and "two of which survive" not in abs_now)
    ...
    THESIS = "Read correctly, foundation models organize classes into clustered structure, hub-aligned in a few backbones, that is moderately shared and does not converge to one common tree; no hierarchy above the superclasses is found in the supervised and contrastive backbones, and the DINOv2 family lies beyond the noise level at which the test was validated; their raw tree-likeness is not evidence for hyperbolic geometry."   # brief of 2026-09-21 (priorities 1b and 2 integrated)
    ...
    chk("final (author's decisions, 2026-09-20): the record is the centered Haar null (44 of 72; five constructions in the census table with the uncentered census as a column; Table 1, Figure 3 and the joint sensitivity from it); the decoupling result rewrites the depth claim with the author's wording (abstract, S1, contribution 2, thesis, S5.3, MERU, Figure 4 caption); 'certified' only for the decoupling-tested structure",
        ...
        and bf.count("The test misses an implanted two-level tree at this noise level but detects a three-level hierarchy with ViT-L's spectrum and noise in 4 of 5 seeds, and that detection survives orientation randomization, whereas none of the four real verdicts does") == 1 and bf.count("a three-level hierarchy implanted at the real noise level is detected and survives orientation randomization, whereas none of the four real verdicts does, so no hierarchy above the superclasses is found") == 2 and bf.count("so no hierarchy above the superclasses is found in the supervised and contrastive backbones, not in the DINOv2 family, which lies beyond the noise level at which the test was validated") == 2 and "a three-level hierarchy implanted at the real noise level is detected and survives orientation randomization, whereas none of the four real verdicts does, so no hierarchy above the superclasses is found" in bf and "no hierarchy above the superclasses is certified" not in bf and "hub-aligned structure in a few, no hierarchy above the superclasses in the supervised and contrastive backbones, not in the DINOv2 family, which lies beyond the noise level at which the test was validated" in bf
        and "\\textbf{The alignment of each cluster with its hub is certified in 4 of 12 ImageNet backbones; no hierarchy above the superclasses is found in the supervised and contrastive backbones, not in the DINOv2 family, which lies beyond the noise level at which the test was validated.}" in bf and "certified against its matched star" in bf and "no hierarchy above the superclasses is found in the supervised and contrastive backbones, not in the DINOv2 family, which lies beyond the noise level at which the test was validated" in bf and "where a deep synthetic one is detected" not in bf and "left open" not in bf and "no power at this noise level" not in bf
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 9d, and no 'left open' or 'no power' wording anywhere",
        ...
        and bf.index("\\paragraph{No hierarchy above the superclasses is found in the supervised and contrastive backbones.}") < bf.index("\\paragraph{A deep hierarchy at the real noise level is detected.}") < bf.index("\\paragraph{The test never fires on randomized hubs.}")
        ...
        and "(iii)~The depth test's power for a three-level hierarchy was measured at ViT-L's spectrum and within/between ratio (1.9) with five seeds, not at the DINOv2 ratios (" in bf and "left open" not in bf and "no power at this noise level" not in bf and "without power" not in bf and "\\paragraph{The two-level implant is missed at ImageNet's noise level.}" in bf,
    ...
    chk("final (priority 2): the S5.1 published-reading paragraph after the lead-in, its claims re-derived from expR78_khrulkov_replication_summary.csv (all four within 0.03, excess negative everywhere, p <= 0.05 on CIFAR-100 and MiniImageNet only), the new lead-in, limitation (vi), and Table 3(b) with the four published and reproduced values",
        ...
        and "reproduces the raw $\\delta_{\\text{rel}}$ that \\citet{Khrulkov_2020_CVPR} report for ResNet-34 on CIFAR-10, CIFAR-100, CUB and MiniImageNet within 0.03" in bf and "only CIFAR-100 and MiniImageNet fall below the null in every batch at the 0.05 level" in bf and "two datasets, one image budget and one published setting" in bf
    ...
    LONGN_ = " in the supervised and contrastive backbones, whose noise level, ratios 1.3 to 2.0, the control covers, and not tested in the DINOv2 family, ratios 3.0 to 3.9"; SHORT_ = " in the supervised and contrastive backbones, not in the DINOv2 family, which lies beyond the noise level at which the test was validated"
    ...
    fa_dec = int(round(flat.dec_frac_cert.sum() * 10)); r64 = pd.read_csv(R/"expR64b_wn30_summary.csv").set_index("model").ratio_real; dr = r64[[m for m in r64.index if m.startswith("dinov2")]]
    ...
    chk("final (7th review): headline scoped in the abstract (twice), S1, contribution 2, S5.3 lead-in, Figure 4 and Table 8 captions and the thesis; decoupled flat control false alarms (8 of 50) and the observation sentence in S5.3 from the files; limitation (iii) with ViT-L's ratio 1.9 and the DINOv2 range from expR64b; Figure 1 caption 'read alike, all low'; no unproved extension in the proof of Proposition 1(b)",
        bf.count("hierarchy above the superclasses is found" + LONGN_) == 1 and bf.count("no hierarchy above the superclasses" + LONGN_) == 0 and bf.count("no hierarchy above the superclasses is found" + SHORT_) == 3 and bf.count("no hierarchy above the superclasses" + SHORT_ + ", and a hyperbolic backbone") == 1 and bf.count("no hierarchy above the superclasses is found in the supervised and contrastive backbones, and the DINOv2 family lies beyond") == 2 and "\\paragraph{No hierarchy above the superclasses is found in the supervised and contrastive backbones.}" in bf and " in the backbones whose noise level the control covers" not in TF and f"the synthetic hierarchy at $z$ ${deep.zdec_mean.max():.2f}$ to ${deep.zdec_mean.min():.2f}$ stands well clear of that bias, the flat control at ${flat.zdec_mean.max():.2f}$ to ${flat.zdec_mean.min():.2f}$." in bf and f"none of the four fires, and ViT-L reads ${float(next(r for r in load('expR74_decoupling_summary.csv') if r['model'] == 'i21k_l')['dec_z_mean']):.2f}$, within the ${flat.zdec_mean.max():.2f}$ to ${flat.zdec_mean.min():.2f}$ of the flat control: without its orientations the real cloud reads like a flat one." in bf and f"DINO-B ({r64['dinov1_b']:.1f}) and ViT-B ({r64['i21k_b']:.1f}) lie at the edge of the covered range." in bf and bf.index("The noise level of a cloud is its within-cluster spread relative to the distance between its hubs.") < bf.index("at this noise level but detects") and "unregime" not in TF and bf.count("The four values reported by") == 2 and "two are indistinguishable from a random cloud" in abs_now and int((S78.p_left_max > 0.05).sum()) == 2 and not any(w_ in bf[bf.index("\\textbf{The answer has three parts.}"):bf.index("\\textbf{The answer has three parts.}") + 260].lower() for w_ in ("genuine", "certified", "record", "frame", "matched star")) and "The decoupled test is therefore biased toward firing, so the four real backbones not firing once decoupled is conservative evidence" in bf and "which makes the real verdicts under decoupling conservative" in bf and "where a deep synthetic one is detected" not in bf
        and f"and once decoupled it fires in {fa_dec} of 50 runs" in bf and obs_ok and f"within/between ratio ({r64['i21k_l']:.1f}) with five seeds, not at the DINOv2 ratios ({dr.min():.1f} to {dr.max():.1f})" in bf
        and "all read alike, all low" in bf and "all read the same" not in bf and "effective dimension $(\\operatorname{tr}" not in TF and "the same argument runs with $d$ replaced" not in TF and "A maximum over a growing sample of quadruples is non-decreasing in the sample size." in TF
        and ("no hierarchy above the superclasses is found" + SHORT_) in dep, f"fa_dec {fa_dec} obs {dobs['kind']}")
