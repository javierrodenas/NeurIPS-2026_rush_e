...
def final_checks():
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "What it certifies is alignment.": (5, 2), "What the test can see.": (12, 2), "Controlling the cut leaves a moderate gap.": (12, 4), "The certified set depends on the frame.": (2, 2)}   # the ceilings per measure beside the cross-model values (thesis brief, 2026-09-22 evening)   # cleanup of 2026-09-22: S5.3 in seven paragraphs, each number once; (c) carries the counts of the test's power and biases
    ...
    pc_ok = (P77.loc["hier_seed0", "z_wn30"] <= -2 and P77.loc["hier_seed0", "z_wn30bal"] <= -2 and P77.loc["ce_seed0", "z_wn30"] <= -2 and P77.loc["frozen", "z_wn30"] <= -2 and not V77["criterion_met"]
             and f"ViT-B fine-tuned with a hierarchical cross-entropy at three WordNet cuts keeps firing once decoupled, {int(round(P77.loc['hier_seed0', 'dec_frac_cert_wn30'] * 10))} of 10, whereas the leaf-CE and frozen models do not, {int(round(P77.loc['ce_seed0', 'dec_frac_cert_wn30'] * 10))} of 10, on the frame of record." in bf and f"frozen ViT-B firing in {int(round(P77.loc['frozen', 'dec_frac_cert_wn30bal'] * 10))} of 10 there is consistent with that mismatch" in bf
             and "The pre-set criterion asked the leaf-CE and frozen models not to be certified intact, which they are, by alignment, so the discriminating comparison is the decoupled one." in bf and "(d) A trained positive control" in dep and "trained positive control, one seed, discriminates the objectives only once decoupled" in bf
