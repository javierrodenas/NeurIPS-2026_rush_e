...
def final_checks():
    ...
    chk("final: abstract is the recorded text (310 words or fewer, the author's cap of 2026-09-22 evening, cell defined in its own sentence, the deep synthetic hierarchy detected and no hierarchy above the superclasses found), 15 text models", abs_now == norm(abs_expected) and len(_re.sub(r"\$[^$]*\$", "", abs_now).replace("--", "").split()) <= 310 and "model--dataset cells" in abs_now and "No hub hierarchy is found in the nine backbones where the decoupled control detects an implanted one, and the test is blind in the other three." in abs_now and "whereas an implanted hierarchy, synthetic or trained in, survives it" in abs_now and "15 text models" in bf and "16 text models" not in bf and "sixteen" not in bf and "two OLMo" not in bf and "no hub hierarchy is found where the test has power" in abs_now and "30 of 36" not in abs_now and "two of which survive" not in abs_now)
    ...
    THESIS = "Read correctly, foundation models organize classes into clustered structure whose tree topology is largely shared, up to a gap beyond resampling noise, and whose metric is not; no hub hierarchy is found where the test has power; and their raw tree-likeness is not evidence for hyperbolic geometry."   # thesis after expR84 (2026-09-22, evening): topology largely shared, metric not
    ...
    chk("final (author's decisions, 2026-09-20): the record is the centered Haar null (44 of 72; five constructions in the census table with the uncentered census as a column; Table 1, Figure 3 and the joint sensitivity from it); the decoupling result rewrites the depth claim with the author's wording (abstract, S1, contribution 2, thesis, S5.3, MERU, Figure 4 caption); 'certified' only for the decoupling-tested structure",
        ...
        and "certifies clustering" not in TF and "validated regime" not in TF and "validated range" not in TF and "The census does not certify that frame" in bf and "The alignment is relational" in bf and "clustered structure whose tree topology is largely shared, up to a gap beyond resampling noise, and whose metric is not" in bf
    ...
    chk("final (10th review): the balanced frame read as frame mismatch (author's sentence, counts from expR83/expR77) and the matched count kept in (c); the alignment stated as relational, not radial, reproduced by the principal-axis implant in the supervised ViTs and CLIP-B and not in DINOv2-L, form unresolved (S5.3 and (iii)); the decoupling's separation and 'no hub hierarchy' = none among the frame's hubs (S5.3 and (iii)); 'hubs of the frame' in the abstract, S1 and the captions; the within-model ceiling from expR84 beside 0.77/0.76 with the keep branch (ceiling >= 0.9), Table q6(c); limitation (vi) on analysis choices, scope renumbered; Table 14(b) restored",
        ...
        and "not that the hubs of the frame form a hierarchy" in abs_now and bf.count("not that the hubs of the frame form a hierarchy") >= 3 and "not that the hubs form a hierarchy" not in TF and "not that the hubs form a hierarchy" not in dep and "not that the hubs of the frame form a hierarchy" in dep
        ...
        and c58.triplet_agree_big_vs_block < S84.loc["ALL", "triplet_agree_min"] and "(c) Within-model ceiling" in tm6 and f"mean over backbones & {S84.loc['ALL', 'ari_cut_mean']:.2f}" in tm6 and "largely shared" in bf and THESIS in bf
    ...
    chk("final (thesis after expR84): the new thesis twice, the abstract's sharing sentence and its S1 mirror with 0.77 and the triplet ceiling range from the files, contribution 3 mirrored, S5.4 with the three ceilings (range and mean) beside the cross-model values and the reading (topology within most of the ceiling, metric at about half)",
        ...
        and f"Across models the tree topology is largely shared and the metric is not: the naive comparison manufactures an island, triplet agreement reaches {c58.triplet_agree_big_vs_block:.2f} against a within-model ceiling of {b84.triplet_agree_mean.min():.2f}--{b84.triplet_agree_mean.max():.2f} once the clustering cut is controlled, and the self-supervised tree is angular." in abs_now
        and bf.count(f"triplet agreement reaches {c58.triplet_agree_big_vs_block:.2f} against a within-model ceiling of {b84.triplet_agree_mean.min():.2f}--{b84.triplet_agree_mean.max():.2f} once the clustering cut is controlled") == 2
        and "\\item \\textbf{The map of trees.} The island is an artifact of the cut; the tree topology is largely shared and the metric is not, and the self-supervised tree is angular." in bf and "sharing is graded and supervision-dependent" not in bf
