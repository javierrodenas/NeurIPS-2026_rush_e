...
def final_checks():
    ...
    HEADLINE = {"44 of 72", "18 of 24", "4 of 12", "0 of 60", "7 of 15", "0.48 to 2.5", "+0.9 to +1.3", "30 of 36", "47", "5 of 12", "210", "+0.41 against +0.28", "0 of 4", "4 of 5", "0 of 5", "0.03", "0.05", "60", "60\\%", "60%"}   # 2026-09-21: priorities 1b (4 of 5, 0 of 5), 2 (0.03, 0.05) and 1c (60%)   # 2026-09-20: the record is the centered Haar null (44 of 72; FMNIST 5 of 12); R6: relational alignment (0 of 4)
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "The test never fires on randomized hubs.": (3, 2), "A deep hierarchy at the real noise level is detected.": (2, 2)}   # the author's count sentences carry more than one number
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 9d, and no 'left open' or 'no power' wording anywhere",
        ...
        and bf.index("\\paragraph{No hierarchy above the superclasses is found.}") < bf.index("\\paragraph{A deep hierarchy at the real noise level is detected.}") < bf.index("\\paragraph{The test never fires on randomized hubs.}")
        and f"fires in {nd} of {len(deep)} seeds at the frame of record. A flat control with the same spectrum and ratio fires in {nf} of {len(flat)}." in bf and "The decoupling control fires on every deep seed, deeper than the intact cloud" in bf and "do not fire at either dimension" in bf
        ...
        and "(iii)~The depth test's power is measured for a three-level synthetic hierarchy with ViT-L's spectrum and noise, not for shallower ones" in bf and "left open" not in bf and "no power at this noise level" not in bf and "without power" not in bf and "\\paragraph{The two-level implant is missed at ImageNet's noise level.}" in bf,
