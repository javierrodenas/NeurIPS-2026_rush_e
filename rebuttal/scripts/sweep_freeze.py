...
def final_checks():
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 10d, and no 'left open' or 'no power' wording anywhere",
        ...
        and "The power follows the backbone, not its ratio or family." in bf and "left open" not in bf and "no power at this noise level" not in bf and "without power" not in bf and "\\paragraph{The two-level implant is missed" not in bf and "Figures~\\ref{fig:implant} and \\ref{fig:power} give the detection rates and the power." in bf,
    ...
    chk("final (priority 2): the S5.1 published-reading paragraph after the lead-in, its claims re-derived from expR78_khrulkov_replication_summary.csv (all four within 0.03, excess negative everywhere, p <= 0.05 on CIFAR-100 and MiniImageNet only), the new lead-in, limitation (vi), and Table 8(b) with the four published and reproduced values",
        ...
        and "The estimator of \\citet{Khrulkov_2020_CVPR}, run on our extraction, reproduces the raw $\\delta_{\\text{rel}}$ they report for ResNet-34 within 0.03." in bf and "Calibrated, only MiniImageNet \\citep{miniimagenet} is genuine. CIFAR-10 and CUB-200 \\citep{cub} are indistinguishable from a random cloud, and CIFAR-100 falls below it only before correction ($p=0.030$)." in bf and set(S78.index[S78.p_left_max > 0.05]) == {"cifar10", "cub"} and "2 datasets, one image budget and one published setting" in bf
    ...
    p81_ok = (len(cov) == 9 and len(unc) == 3 and cov == ["i21k_s", "i21k_b", "i21k_l", "dinov2_b", "dinov2_l", "dinov2_g", "clip_b", "clip_l", "siglip_b"] and unc == ["i21k_t", "dinov1_b", "dinov2_s"]
              ...
              and f"The 3 that cannot see a planted hierarchy, {_names_unc}, sit at ratios {rb.min():.1f} to {rb.max():.1f}, inside the {rc.min():.1f} to {rc.max():.1f} spanned by the 9 covered backbones. The power follows the backbone, not its ratio or family." in bf and rc.min() <= rb.min() and rb.max() <= rc.max()
