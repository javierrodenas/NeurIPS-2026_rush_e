...
def final_checks():
    ...
    THESIS = "Read correctly, foundation models organize classes into clustered structure that is moderately shared and does not converge to one common tree; no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven; and their raw tree-likeness is not evidence for hyperbolic geometry."   # brief of 2026-09-21 (priorities 1b and 2 integrated)
    ...
    EXEMPT = {"Most cells show structure beyond the second moments.": (4, 3), "The test never fires on randomized hubs.": (3, 2), "A deep hierarchy at the real noise level is detected.": (5, 2), "No hub hierarchy is found where the control has power.": (5, 2), "The power of the test follows the backbone, not its ratio or family.": (4, 2), "A trained hierarchy survives decoupling.": (3, 2)}   # the author's count sentences carry more than one number
    ...
    chk("final (author's decisions, 2026-09-20): the record is the centered Haar null (44 of 72; five constructions in the census table with the uncentered census as a column; Table 1, Figure 3 and the joint sensitivity from it); the decoupling result rewrites the depth claim with the author's wording (abstract, S1, contribution 2, thesis, S5.3, MERU, Figure 4 caption); 'certified' only for the decoupling-tested structure",
        ...
        and bf.count("The test misses an implanted two-level tree at this noise level but detects a three-level hierarchy with ViT-L's spectrum and noise in 4 of 5 seeds, and that detection survives orientation randomization, whereas none of the four real verdicts does") == 1 and "whereas an implanted hierarchy, synthetic or trained in, survives it; no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven." in bf and "The test misses an implanted two-level tree at this noise level but detects a three-level hierarchy" in bf and "no hierarchy above the superclasses is certified" not in bf and "clusters oriented toward their hubs in a few, no hub hierarchy in the five backbones where the control detects an implanted one, a blind test in the other seven, and a hyperbolic backbone" in bf
        and "\\textbf{The depth test certifies in 4 of 12 ImageNet backbones that clusters are oriented toward their hubs, not that the hubs form a hierarchy; no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven.}" in bf and "hub-aligned" not in bf and bf.count("clusters are oriented toward their hubs") >= 3 and "A model--dataset cell is genuine when" in bf and "certified against its matched star" in bf and "no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven" in bf and "where a deep synthetic one is detected" not in bf and "left open" not in bf and "no power at this noise level" not in bf
    ...
    chk("final (priority 1b): the S5.3 deep-hierarchy paragraph after the decoupling paragraph with its counts re-derived from expR79_synthetic_deep_poincare.csv (4 of 5 deep, 0 of 5 flat, every deep seed under decoupling and deeper, Poincare never), the author's sentence in S5.3 and S1, the abstract sentence, the thesis, limitation (iii), Table 9d, and no 'left open' or 'no power' wording anywhere",
        ...
        and "(iii)~The depth test's power for a three-level hierarchy follows the backbone rather than its within/between ratio or family" in bf and "left open" not in bf and "no power at this noise level" not in bf and "without power" not in bf and "\\paragraph{The two-level implant is missed" not in bf and "Figure~\\ref{fig:depth}b the detection rates" in bf,
    ...
    cov = [m for m in ORD if S81.loc[m, "power"] >= 0.8]; unc = [m for m in ORD if S81.loc[m, "power"] < 0.8]
    p81_ok = (len(cov) == 5 and len(unc) == 7 and cov == ["i21k_l", "dinov2_l", "dinov2_g", "clip_l", "siglip_b"]
              and f"The control detects an implanted hierarchy in ViT-L, DINOv2-L, DINOv2-G, CLIP-L and SigLIP-B (power {S81.loc[cov, 'power'].min():.2f}--{S81.loc[cov, 'power'].max():.2f}), while ViT-T/S/B, DINO-B, DINOv2-S/B and CLIP-B are not covered (power {S81.loc[unc, 'power'].min():.2f}--{S81.loc[unc, 'power'].max():.2f}), so no hub hierarchy is found in the five and the test is blind in the other seven." in bf
              and all(S81.loc[m, "power"] == 0.0 for m in ("i21k_t", "i21k_s", "i21k_b")) and f"Three supervised ViTs at ratios {r64[['i21k_t', 'i21k_s', 'i21k_b']].min():.1f} to {r64[['i21k_t', 'i21k_s', 'i21k_b']].max():.1f} miss the implant, whereas DINOv2-L and DINOv2-G detect it at {r64['dinov2_l']:.1f}." in bf
              and "(e) Power per backbone" in pw and all(int(S81.loc[m, "n_seeds"]) == 20 for m in ("dinov2_s", "dinov2_b", "dinov2_l", "dinov2_g")) and f"It is {S81.loc[cov, 'power'].min():.2f} or more in five backbones and {S81.loc[unc, 'power'].max():.2f} or less in the other seven" in bf)
    ...
    chk("final (7th review): headline scoped in the abstract (twice), S1, contribution 2, S5.3 lead-in, Figure 4 and Table 8 captions and the thesis; decoupled flat control false alarms (8 of 50) and the observation sentence in S5.3 from the files; limitation (iii) with ViT-L's ratio 1.9 and the DINOv2 range from expR64b; Figure 1 caption 'read alike, all low'; no unproved extension in the proof of Proposition 1(b)",
        ...
        and ("no hub hierarchy is found in the five backbones where the control detects an implanted one, and the test is blind in the other seven") in dep, f"fa_dec {fa_dec} obs {dobs['kind']}")
