#!/usr/bin/env python3
"""expR30 (mock-review W3): does the GPT-2 scale emergence survive the template?
...
each) per (model, template). Output: expR30_template_nulls.csv."""
import os, sys, time
...
def specnull(C, rep):
    ...
    return (G*S)@Vt + mu
@torch.no_grad()
def encode_causal(hf, texts, fp16, bs=16):
...
def main():
    ...
    csv_path = OUT/"expR30_template_nulls.csv"
