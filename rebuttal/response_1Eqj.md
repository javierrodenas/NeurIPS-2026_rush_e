...
**A2.** Both remarks are fair catches on the §3.2 sentence at L145.
- **(a) Sources:** our sentence attributed all three reference values to [Gromov, 1987], where they are indeed not tabulated; the revision re-cites them properly: $\ln(1+\sqrt2)$ is the thin-triangles constant of $\mathbb{H}^2$ (Bridson–Haefliger, Ch. III.H), and the binary-tree and $S^{99}$ values are our own numerical calibrations, with protocol stated. On the latter: the printed $0.127$ came from the original estimator settings; under the fully declared protocol used in this response (1000 uniform points, 500K quadruples × multi-seed) the sphere sits at $0.143$ (chord) / $0.179$ (geodesic). Both versions lie above every ImageNet value printed in Table 1 (.066–.123), so no conclusion depends on the difference; the revision reports the declared-protocol value.
- **(b) The 0,38:** our text wrote "log" without specifying the base; with the natural logarithm the constant is $\ln(1+\sqrt2)=\operatorname{arccosh}(\sqrt2)\approx0.881$ (read as $\log_{10}$ it yields the 0.38 in the review); we apologize for the ambiguity, and the final version writes "ln" explicitly.
- **(c) The counterintuitive ordering** is a **units problem in our text**: $\approx0.881$ is an **absolute** constant, while $0.127$ is a **normalized** value $\hat\delta=\delta_{\max}/\mathrm{diam}$, two different scales placed in one sentence. Calibrating our exact estimator (500K quadruples, multi-seed) on reference geometries:
...
5. **Experiments**: rewrite §4.1 to state its conclusion first and reference the new main-text table; enlarge Figure 2; fix the broken figure reference at L242; move "Practical protocol" next to the setup as suggested; expand A1–A4 into claim + evidence + interpretation, with descriptive names for the four interventions.
