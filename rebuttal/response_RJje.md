...
- **Concession and fix**: trained centroids sit *above* the iid-Gaussian null, i.e., low absolute $\hat\delta$ alone cannot be read as "semantically hierarchical" (the null is a degenerate star). The most informative control is a **spectrum-matched Gaussian null** (identical covariance spectrum, so all second-order structure is equated): supervised ViTs sit clearly *below* it (excess −0.019/−0.015/−0.023/−0.031 for T/S/B/L — genuine higher-order tree structure, growing with scale), as do CLIP/SigLIP (−0.021/−0.020/−0.017), whereas DINOv2 sits essentially *at* its null (+0.011..−0.011); an independent PC-permutation null (exact spectrum and marginals) reproduces the split with larger margins (supervised −0.035..−0.040; CLIP/SigLIP −0.036..−0.047; DINOv2 −0.004..+0.017). DINOv2's very low raw $\hat\delta$ is thus largely carried by its covariance spectrum and extreme cluster compactness. This concords with the WordNet-alignment inversion in §3 below — two independent controls giving the same refined picture. Dimension-matched comparisons depend on the matching method: under Gaussian random projection to $d{=}192$ the family ordering survives (DINOv2-B/L/G lowest, .094–.100, below the .104 iid null; CLIP highest), while PCA truncation attenuates it; DINOv2-S sits well above B/L/G in every variant, so the clean part of the within-family claim is S vs {B,L,G} rather than a monotone trend. All absolute-level and cross-dimension statements (including the "45%" headline) will be restated as **excess over matched nulls**; the within-architecture ablations (random weights, fine-tuning, layer-wise) are same-$d$ contrasts and unaffected.
