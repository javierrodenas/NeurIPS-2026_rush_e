...
import os as _osmod
_t = _osmod.environ["TARGET"].replace(".","_")
OUT = ROOT / ("results/sweep_curv_t" + _t)
...
def s4_params(X_train, target_p95=None):
    mu = X_train.mean(0)
    if target_p95 is None:
        import os
        target_p95 = float(os.environ["TARGET"])
...
def task_E(X_tr, y_tr, X_te, y_te, ds, n_episodes=1000, n_way=5, k_shot=5, q_query=15, seed=42):
    ...
    import os as _os; scale = 2 * np.arctanh(float(_os.environ["TARGET"])) / max(p95, 1e-7)
...
def main():
    ...
    for model in ALL_MODELS:
        for ds in ALL_DATASETS:
            ...
            row = {**base,
                   ...
                   "time_s":time.time()-t0}
            rows.append(row)
            pd.DataFrame(rows).to_csv(OUT/"sweep_curv.csv", index=False)
    ...
    df.to_csv(OUT/"sweep_curv.csv", index=False)
    print(f"\nSaved: {OUT/'sweep_curv.csv'}")
